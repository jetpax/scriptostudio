print("Starting iperf3 ScriptO...")

# === START_CONFIG_PARAMETERS ===

dict(

    timeout = 0,    # Delay in seconds before showing the interrupt button to the user.
                    # A value of 0 will never display the button.
    
    info    = dict(
        # ----------------------------------------------------------------------
        name        = 'iperf3 Server',                          # Name is mandatory
        ...
        description =                                           # Description is mandatory
                      ''' Run iperf3 network performance tests.
                          Supports Server and Client modes, TCP/UDP, and Reverse mode.
                      ''',
        author      = 'Damien P. George / Mod by Antigravity',  # Author is mandatory
        # ----------------------------------------------------------------------
    ),
    
    args    = dict(              # Label and type are mandatory
        # ----------------------------------------------------------------------
        mode               = dict( label    = 'Mode:',
                                   type     = dict,
                                   items    = dict( server = "Server Mode",
                                                    client = "Client Mode" ),
                                   value    = 'server'),
        # ----------------------------------------------------------------------
        host               = dict( label    = 'Server IP (Client mode only):',
                                   type     = str,
                                   value    = "192.168.1.100"),
        # ----------------------------------------------------------------------
        udp                = dict( label    = 'UDP Mode:',
                                   type     = bool,
                                   value    = False ),
        # ----------------------------------------------------------------------
        reverse            = dict( label    = 'Reverse Mode:',
                                   type     = bool,
                                   value    = False ),
        # ----------------------------------------------------------------------
        bandwidth          = dict( label    = 'Bandwidth (UDP only, bits/sec):',
                                   type     = int,
                                   value    = 10485760 ),  # 10 Mbps
        # ----------------------------------------------------------------------
    )
...
def server_once():
    ...
    try:
        ...
        # Get parameters
        len_bytes = recvn(s_ctrl, 4)
        n = struct.unpack(">I", len_bytes)[0]
        print(f"DEBUG: Protocol received param length: {n} (bytes: {len_bytes.hex()})")
        if n > 16384: # 16KB limit
            raise ValueError(f"Parameter length {n} is too large. Possible protocol mismatch or garbage data.")
...
if args.mode == 'server':
    ...
    try:
        client(args.host, udp=args.udp, reverse=args.reverse, bandwidth=args.bandwidth)
    except KeyboardInterrupt:
        print("\nClient stopped.")
