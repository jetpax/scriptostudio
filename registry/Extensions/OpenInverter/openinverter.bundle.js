...
// BUILD_ID: 2026-02-04T18-13-33

console.log('%c[OpenInverter] BUILD 2026-02-04T18-13-33', 'color: #ff8c00; font-weight: bold');
    ...
    <div class="parameter-item" style="display: flex; flex-direction: column; gap: 0.25rem;">
      ...
      <div class="parameter-input-group" style="display: flex; flex-direction: column; gap: 0.25rem;">
        ${s?this.html`
          <select
            value="${r}"
            onchange=${a=>ke.call(this,e,t,parseFloat(a.target.value))}
...
${d}`)}a===0&&n===0&&alert("No valid parameters found in file")}catch(i){console.error("[OI Parameters] Import error:",i),alert(`Failed to import: ${i.message}`),t.value="",this.state.isImporting=!1,this.emit("render")}}async function Ne(){if(!this.state.oiDeviceConnected){alert("Not connected to device");return}let e=parseInt(this.state.nodeId);if(isNaN(e)||e<1||e>127){alert("Node ID must be a number between 1 and 127");return}try{let t=this.state.discoveredDevices?.find(s=>s.serial===this.state.selectedDeviceSerial);t&&(t.nodeId=e,this.state.selectedNodeId=e),alert(`Node ID set to ${e}`),this.emit("render")}catch(t){console.error("[OI Parameters] Save Node ID error:",t),alert(`Failed to save Node ID: ${t.message}`)}}async function Oe(){if(!this.state.oiDeviceConnected){alert("Not connected to device");return}try{let e=await this.device.execute("from lib.ext.openinverter.OI_helpers import saveToFlash; saveToFlash()"),t=this.device.parseJSON(e);t&&t.success?alert("Parameters saved to flash successfully"):alert("Failed to save parameters to flash")}catch(e){console.error("[OI Parameters] Save to flash error:",e),alert(`Failed to save to flash: ${e.message}`)}}function Ae(e){return e?e.split("_").map(t=>t.charAt(0).toUpperCase()+t.slice(1).toLowerCase()).join(" "):""}function Te(e){return e==null?"0":String(Math.round(Number(e)))}function x(){if(!this.state.oiDeviceConnected)return this.html`
    ...
    `;if(this.state.spotValueRefreshTimer||te.call(this),this.state.isLoadingOiSpotValues)return this.html`
      <div style="padding: 60px 20px; text-align: center;">
        ...
        <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
          ...
          <div style="display: flex; align-items: center; gap: 8px;">
            ...
            <input 
              ...
              onfocus=${t=>{this.state.spotValueRefreshTimer&&V.call(this)}}
              onchange=${t=>{ee.call(this,t.target.value)}}
              onblur=${t=>{ee.call(this,t.target.value)}}
              min="100"
              max="10000"
              step="100"
              style="
  ...
  `}function Fe(e){this.state.selectedChartParams||(this.state.selectedChartParams=new Set);let t=this.state.selectedChartParams.has(e.name),s=k(e.value,e.unit),i=Ee.call(this,e,s.value),r=this.state.spotValueHistory[e.name]||[],a=`spot-card-${e.name.replace(/[^a-zA-Z0-9]/g,"-")}`,n=()=>{console.log("[OI] Clicked spot value card:",e.name),Ve.call(this,e.name)};return this.html`
    <div 
      id="${a}"
  ...
  `}function ze(e,t,s){let a={top:10,right:10,bottom:40,left:50},n=500-a.left-a.right,o=150-a.top-a.bottom;if(t.length<2)return this.html`
    ...
    `;let h=t.map(c=>c.value),d=t.map(c=>c.timestamp),p=Math.min(...h),l=Math.max(...h),g=Math.min(...d),M=Math.max(...d),N=l-p||1,b=M-g||1,O=c=>a.left+(c-g)/b*n,A=c=>a.top+o-(c-p)/N*o,B=5,ue=Array.from({length:B},(c,v)=>p+N*v/(B-1)),j=6,me=Array.from({length:j},(c,v)=>g+b*v/(j-1)),ge=t.map((c,v)=>{let u=O(c.timestamp),y=A(c.value);return`${v===0?"M":"L"} ${u} ${y}`}).join(" "),_=Le(e);return this.html`
  ...
  `}function te(){this.state.spotValueRefreshTimer&&clearInterval(this.state.spotValueRefreshTimer),Z.call(this),this.state.spotValueRefreshTimer=setInterval(()=>{Z.call(this)},this.state.autoRefreshInterval)}function V(){this.state.spotValueRefreshTimer&&(clearInterval(this.state.spotValueRefreshTimer),this.state.spotValueRefreshTimer=null)}async function Z(){if(!this.state.oiDeviceConnected){V.call(this);return}try{this.state.isLoadingOiSpotValues=!0;let e=await J.call(this);if(e){this.state.oiSpotValues=e;let t=Date.now();Object.entries(e).forEach(([s,i])=>{if(this.state.selectedChartParams&&this.state.selectedChartParams.has(s)){this.state.spotValueHistory[s]||(this.state.spotValueHistory[s]=[]);let r=k(i.value,i.unit);this.state.spotValueHistory[s].push({timestamp:t,value:r.value}),this.state.spotValueHistory[s].length>100&&this.state.spotValueHistory[s].shift()}})}this.state.isLoadingOiSpotValues=!1,this.state.activeDeviceTab==="telemetry"&&this.emit("render")}catch(e){console.error("[OI] Failed to refresh spot values:",e),this.state.isLoadingOiSpotValues=!1}}function ee(e){let t=parseInt(e,10);!isNaN(t)&&t>=100&&t<=1e4&&(this.state.autoRefreshInterval=t,V.call(this),te.call(this))}function Re(){this.state.spotValueHistory={},this.emit("render")}function Ve(e){if(this.state.selectedChartParams||(this.state.selectedChartParams=new Set),this.state.selectedChartParams.has(e))this.state.selectedChartParams.delete(e),delete this.state.spotValueHistory[e];else if(this.state.selectedChartParams.add(e),this.state.spotValueHistory[e]||(this.state.spotValueHistory[e]=[]),this.state.oiSpotValues&&this.state.oiSpotValues[e]){let t=this.state.oiSpotValues[e],s=k(t.value,t.unit);this.state.spotValueHistory[e].push({timestamp:Date.now(),value:s.value})}this.emit("render")}function Le(e){let t=0;for(let i=0;i<e.length;i++)t=e.charCodeAt(i)+((t<<5)-t);let s=["var(--oi-blue)","var(--oi-orange)","var(--oi-green)","#9333ea","#ec4899","#14b8a6","#f59e0b","#ef4444"];return s[Math.abs(t)%s.length]}function k(e,t){if(!t)return{value:e,unit:""};let s=t.match(/^(\d+)(.*)$/);if(s){let i=parseInt(s[1],10),r=s[2];return{value:e*i,unit:r}}return{value:e,unit:t}}function Ee(e,t){let s=k(t,e.unit),i;return s.value===0?i="0":Math.abs(s.value)<.01?i=s.value.toExponential(2):Math.abs(s.value)<100?i=s.value.toFixed(2):i=s.value.toFixed(0),s.unit?`${i} ${s.unit}`:i}function w(){if(!this.state.canMappings&&!this.state.isLoadingCanMappings&&this.state.oiDeviceConnected&&setTimeout(()=>L.call(this),0),!this.state.allParamsWithIds&&!this.state.isLoadingAllParamsWithIds&&this.state.oiDeviceConnected&&setTimeout(()=>_e.call(this),0),!this.state.oiDeviceConnected)return this.html`
      <div class="system-panel">
...
`,n=await this.device.execute(a),o=this.device.parseJSON(n);o&&o.error?alert(`Failed to send CAN message: ${o.error}`):alert(`CAN message sent: ID 0x${s.toString(16).toUpperCase()}`)}catch(r){console.error("[CAN] Send error:",r),alert("Failed to send CAN message: "+r.message)}}function Ke(e){let t=this.state.canMessages.periodicMessages[e];t.active=!t.active,this.emit("render")}function Xe(e){confirm("Delete this periodic message?")&&(this.state.canMessages.periodicMessages.splice(e,1),this.emit("render"))}function I(){this.state.firmwareUpgrade||(this.state.firmwareUpgrade={selectedFile:null,serialNumber:"",recoveryMode:!1,inProgress:!1,progress:0,status:"",error:null});let e=this.state.firmwareUpgrade;return this.html`
  ...
  `):this.html`
    ...
    `}function S(e){this.state.activeDeviceTab=e,this.emit("render")}function at(){switch(this.state.activeDeviceTab){case"telemetry":return x.call(this);case"parameters":return $.call(this);case"canmapping":return w.call(this);case"canmessages":return C.call(this);case"ota":return I.call(this);default:return this.html`<div class="panel-message">Unknown tab</div>`}}function rt(e){let t=this.state.oiDeviceConnected&&this.state.currentDeviceSerial===e.serial&&this.state.selectedNodeId===e.nodeId,s=this.state.editingDeviceName===e.serial;return this.html`
    <div 
      class="device-card"
      style="
...
print(json.dumps(results))
    `),s=this.device.parseJSON(t);s&&Array.isArray(s)?(this.state.canScanResults=s,this.state.scanMessage=`Found ${s.length} device${s.length!==1?"s":""}`):(this.state.canScanResults=[],this.state.scanMessage="No devices found")}catch(t){console.error("[OI] Scan error:",t),this.state.scanMessage="Scan failed: "+t.message,this.state.canScanResults=[]}this.state.isScanning=!1,this.emit("render")}function nt(e){let t=this.state.discoveredDevices.find(s=>s.nodeId===e.nodeId||s.serial===e.serialNumber);if(t){alert(`Device already added: ${t.name||`Node ${t.nodeId}`}`);return}this.state.discoveredDevices.push({nodeId:e.nodeId,serial:e.serialNumber,name:`Device ${e.nodeId}`,online:!0,firmware:null}),this.emit("render")}function ot(e){this.state.discoveredDevices.find(i=>i.nodeId===e.nodeId||i.serial===e.serialNumber)||this.state.discoveredDevices.push({nodeId:e.nodeId,serial:e.serialNumber,name:`Device ${e.nodeId}`,online:!0,firmware:null}),E.call(this,e.nodeId,e.serialNumber);let s=e.serialNumber||`NODE_${e.nodeId}`;this.emit("change-extension-panel",{extensionId:"openinverter",panelId:`device-${s}`})}function ce(){let e=this.state.selectedNodeId,t=this.state.manualSerial;t||(e>127?t=`MOCK-${String(e).padStart(3,"0")}-${Date.now().toString(36).slice(-4).toUpperCase()}`:t=`MANUAL_${e}`);let s=this.state.discoveredDevices.find(r=>r.nodeId===e||r.serial===t);if(s){alert(`Device already added: ${s.name||`Node ${s.nodeId}`}`);return}let i={nodeId:e,serial:t,name:e>127?`Test Inverter ${e}`:`Device ${e}`,online:!0,firmware:e>127?"v4.2.0-mock":null};this.state.discoveredDevices.push(i),this.state.manualSerial="",this.emit("render"),console.log("[OI] Device added manually:",i)}function lt(){ce.call(this);let e=this.state.selectedNodeId,t=this.state.discoveredDevices.find(s=>s.nodeId===e);t&&(E.call(this,t.nodeId,t.serial),this.emit("change-extension-panel",{extensionId:"openinverter",panelId:`device-${t.serial}`}))}async function E(e,t){if(console.log("[OI] Connecting to device:",{nodeId:e,serial:t}),this.state.selectedNodeId=e,e>127){let s=t||`MOCK-${String(e).padStart(3,"0")}-${Date.now().toString(36).slice(-4).toUpperCase()}`;this.state.discoveredDevices||(this.state.discoveredDevices=[]);let i={nodeId:e,serial:s,name:`Test Inverter ${e}`,firmware:"v4.2.0-mock",online:!0},r=this.state.discoveredDevices.findIndex(a=>a.nodeId===e||a.serial===s);r>=0?this.state.discoveredDevices[r]=i:this.state.discoveredDevices.push(i),this.state.oiDeviceConnected=!0,this.state.currentDeviceSerial=s,this.state.oiParameters=null,console.log("[OI] Mock device connected:",i),this.getMenuItems&&this.emit("update-extension-menu");return}this.state.currentDeviceSerial=t,this.state.oiDeviceConnected=!0,this.state.oiParameters=null,this.getMenuItems&&this.emit("update-extension-menu")}function ct(e){E.call(this,e.nodeId,e.serial),this.emit("change-extension-panel",{extensionId:"openinverter",panelId:`device-${e.serial}`})}function dt(e){this.state.editingDeviceName=e.serial,this.state.editingDeviceNameValue=e.name||"",this.emit("render")}function oe(e){let t=this.state.editingDeviceNameValue?.trim();t&&(e.name=t),this.state.editingDeviceName=null,this.emit("render")}function pt(e){if(!confirm(`Delete ${e.name||`Device ${e.nodeId}`}?`))return;let t=this.state.discoveredDevices.findIndex(s=>s.serial===e.serial);t!==-1&&(this.state.discoveredDevices.splice(t,1),this.state.currentDeviceSerial===e.serial&&(this.state.oiDeviceConnected=!1,this.state.currentDeviceSerial=null,this.state.oiParameters=null),this.getMenuItems&&this.emit("update-extension-menu")),this.emit("render")}function de(){return this.state.canScanResults||(this.state.canScanResults=[]),this.state.selectedNodeId||(this.state.selectedNodeId=1),this.state.oiDeviceConnected===void 0&&(this.state.oiDeviceConnected=!1),this.state.isScanning===void 0&&(this.state.isScanning=!1),this.html`
