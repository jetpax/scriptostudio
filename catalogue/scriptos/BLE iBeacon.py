...
dict(
    ...
    info = dict(
        ...
        description = 'Initializes the Bluetooth Low Energy radio and simulates an Apple iBeacon object. ' \
                    + 'The advertising message broadcasts an UUID corresponding to "B.SCRIPTO_BEACON" in binary ' \
                    + 'with the short name "ScriptOmator" while the iBeacon is the response to an active scanner.\n' \
...
def _getAdvCompleteNameData() :
    ...
    uuidBytes = b'B.SCRIPTO_BEACON' # 16 bytes or str UUID mandatory
    data      = _getAdvDataPart(boradcastFlagType,    bytes([incomp128bitsUUIDS])) \
              + _getAdvDataPart(CompleteNameDataType, 'ScriptOmator'.encode()) \
