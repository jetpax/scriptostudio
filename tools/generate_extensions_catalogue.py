...
<head>
    ...
    <style>
        * {{
        ...
        .header {{
            background: linear-gradient(135deg, #008184 0%, #00a8ab 100%);
            color: white;
            padding: 2rem;
            text-align: center;
...
<body>
    <div class="header">
        ...
        <p>Full-featured applications for ScriptO Studio</p>
    </div>
...
{chr(10).join(generate_card_html(ext) for ext in extensions)}
    ...
    <script>
        ...
        function generateCardHTML(ext) {{
            ...
            const hasLib = ext.mipPackage ? '<span class="extension-has-lib">📦 Device Library</span>' : '';
...
def generate_card_html(extension):
    ...
    has_lib = '<span class="extension-has-lib">📦 Device Library</span>' if extension.get('mipPackage') else ''
    ...
    if mip_package:
        lib_html = f"""
            <div class="detail-section">
                ...
                <p>This extension includes Python libraries that run on your ESP32 device.</p>
...
def generate_html_catalogue(index, output_dir='extensions-catalogue', extensions_dir='Extensions'):
    ...
    if extensions_source.exists():
        for extension in extensions:
            ...
            if filename:
                # Find the extension file in the source directory
                ext_files = list(extensions_source.glob(f"**/{filename}"))
                if ext_files:
                    source_file = ext_files[0]
                    # Copy to a subdirectory matching the extension ID
