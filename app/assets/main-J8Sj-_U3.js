...
`}),l+=`${n}_ds.st()
`,l}async function instrumentCodeForExec(n,i={}){const{watches:o={},conditionalBP:r={},fileName:s="main.py"}=i,a=performance.now(),c=await identifyCodeRows(n);console.log(`[Debugger] Identified ${c.size} code rows in ${performance.now()-a}ms`);let l=n.split(/\r?\n/);const u=new Map,d=o[""]||[],p=o[s]||[],f=[...new Set([...d,...p])],h=r[""]||[],m=r[s]||[],v=[...new Set([...h,...m])],y=Array.from(c.keys()).sort((k,S)=>k-S);for(const k of y){const S=l[k],_=c.get(k);if(S===void 0)continue;const E=/# ●/.test(S),P=i.breakpoints&&i.breakpoints[s]?i.breakpoints[s][k+1]:null;(E||P)&&console.log(`[Debugger] Breakpoint detected at line ${k+1}: ${S}`);const I=S.match(/^(\s*)/)[1];let $=generateDebugBlock(I,E,s,k+1,f,v,P);_==="function_definition"?$+=`${I}@_ds.wrap
`:_==="async_function_definition"&&($+=`${I}@_ds.awrap
`),E&&console.log(`[Debugger] Generated instrumentation for row ${k+1} (${_})`),u.set(k,$)}let w=[];for(let k=0;k<l.length;k++)u.has(k)&&w.push(u.get(k)),w.push(l[k]);const b=w.join(`
`),C=`${DEBUG_STATE_MODULE}

# Execute isolated code
_ds.exec("""${b.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/'/g,"\\'")}""")
`,x=performance.now()-a;return console.log(`[Debugger] Instrumentation complete in ${x.toFixed(0)}ms`),C}console.log("[Libs] ES modules loaded");function renderIcon(n,i={}){const{className:o="",size:r=24,color:s="currentColor"}=i;return html$1`
  ...
  `}const DiskFileList=generateFileList("disk"),BoardFileList=generateFileList("board");function generateFileList(n){return function(o,r){function s(h){h.key.toLowerCase()==="enter"&&h.target.blur(),h.key.toLowerCase()==="escape"&&(h.target.value=null,h.target.blur())}const a=html`
    ...
    `;function l(h,m){const v=html`
        <input type="text"
          value=${h.fileName}
          onkeydown=${s}
          onblur=${E=>r("finish-renaming-file",E.target.value)}
          onclick=${E=>!1}
          ondblclick=${E=>!1}
          />
      `,y=o.selectedFiles.find(E=>E.fileName===h.fileName&&E.source===n);function w(E){return E.preventDefault(),r("rename-file",n,h),!1}function b(){o.renamingFile||r(`navigate-${n}-folder`,h.fileName)}function C(){o.renamingFile||r("open-file",n,h)}let x=h.fileName;const k=o.selectedFiles.find(E=>E.fileName===x);o.renamingFile==n&&k&&(x=v);function S(E){if(E==null)return"";if(E===0)return"0 B";const P=1024,I=["B","KB","MB","GB"],$=Math.floor(Math.log(E)/Math.log(P));return parseFloat((E/Math.pow(P,$)).toFixed(1))+" "+I[$]}const _=h.type==="file"?S(h.size):"";return h.type==="folder"?html`
          <div
            class="item ${y?"selected":""}"
            onclick=${E=>r("toggle-file-selection",h,n,E)}
            ondblclick=${b}
            >
            ${IconSprite.renderIcon("folder",{className:"icon"})}
    ...
    <div class="panel-section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <span class="info-value info-mono">${"1258c84"}</span>
        </div>
        <div class="info-item">
  ...
  `}function NTPPanel(n,i){n.isConnected&&!n.ntpConfigLoaded&&!n.isLoadingNtpConfig&&i("load-ntp-config");const o=n.ntpConfig||{server:"pool.ntp.org",tzOffset:0,timezone:"UTC",autoDetect:!1},r=n.ntpSyncResult||null,s=d=>{if(!d)return"--:--:--";const{year:p,month:f,day:h,hour:m,minute:v,second:y}=d;return`${String(m).padStart(2,"0")}:${String(v).padStart(2,"0")}:${String(y).padStart(2,"0")}`},a=d=>{if(!d)return"";const{year:p,month:f,day:h}=d;return`${p}-${String(f).padStart(2,"0")}-${String(h).padStart(2,"0")}`},c=[{value:"UTC",offset:0,label:"UTC (Coordinated Universal Time)"},{value:"EST",offset:-5,label:"EST (Eastern Standard Time)"},{value:"CST",offset:-6,label:"CST (Central Standard Time)"},{value:"MST",offset:-7,label:"MST (Mountain Standard Time)"},{value:"PST",offset:-8,label:"PST (Pacific Standard Time)"},{value:"GMT",offset:0,label:"GMT (Greenwich Mean Time)"},{value:"CET",offset:1,label:"CET (Central European Time)"},{value:"EET",offset:2,label:"EET (Eastern European Time)"},{value:"JST",offset:9,label:"JST (Japan Standard Time)"},{value:"AEST",offset:10,label:"AEST (Australian Eastern Standard Time)"}],l=o.timezone,u=[...c];return l&&!c.find(d=>d.value===l)&&u.unshift({value:l,offset:o.tzOffset||0,label:`${l} (Detected)`}),html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${async d=>{d.preventDefault();const p=new FormData(d.target),f=p.get("server")||"pool.ntp.org",h=p.get("timezone")||"UTC",m=p.get("auto_detect")==="on",v=u.find(b=>b.value===h),y=v?v.offset:0,w={server:f,tz_offset:y,timezone:h,auto_detect:m};try{await new Promise((b,C)=>{const x=setTimeout(()=>C(new Error("Save timeout")),1e4),k=()=>{clearTimeout(x),window.appInstance.emitter.removeListener("ntp-config-saved",S),window.appInstance.emitter.removeListener("ntp-config-save-error",_)},S=()=>{k(),b()},_=E=>{k(),C(E)};window.appInstance.emitter.once("ntp-config-saved",S),window.appInstance.emitter.once("ntp-config-save-error",_),i("save-ntp-config",w)}),i("sync-ntp-time",f,y,m)}catch(b){console.error("[NTP] Failed to save config before sync:",b),alert(`Failed to save NTP configuration: ${b.message}`)}}}>
          <div class="form-group">
...
`)){const a=s.match(/^(\w+)\s*:\s*(.+)$/);a&&(i.meta[a[1].trim()]=a[2].trim())}i.body=n.slice(o[0].length)}return i.body=i.body.replace(/^ScriptO for\s+\[.*?\]\(.*?\)\.?\s*Run on device\s*\(MicroPython\)\.?\s*\n?/im,""),i.body=i.body.trim(),i}function SkillSidebar(n,i){if(!n.isSkillSidebarOpen||!n.skillSidebarContent)return html`<div></div>`;const{name:o,skillMd:r}=n.skillSidebarContent,{meta:s,body:a}=parseSkillMd(r);let c="";try{a&&(c=g.parse(a,{breaks:!0}))}catch(f){console.error("[SkillSidebar] Failed to parse markdown:",f),c=`<pre>${a}</pre>`}const l=[];s.description&&l.push(`<p class="skill-meta-description">${s.description}</p>`);const u=[];s.category&&u.push(`<span class="skill-meta-tag">${s.category}</span>`),s.author&&u.push(`<span class="skill-meta-tag">by ${s.author}</span>`),u.length&&l.push(`<div class="skill-meta-tags">${u.join("")}</div>`);const d=l.length?`<div class="skill-meta-block">${l.join("")}</div>`:"",p=document.createElement("div");return p.className="skill-sidebar-content skill-md-content",p.innerHTML=d+c,html`
  ...
  `,w=S=>{const _=S.icon||"settings";return IconSprite.renderIcon(_,{className:"",size:20})},b=(n.installedExtensions||[]).map(S=>{const _=n.expandedExtensions[S.id],E=n.activeExtension===S.id,P=S.devices===!0;let I=null;if(P&&_)if(!n.loadedExtensions[S.id])console.warn("[System] Extension not loaded:",S.id);else{const $=n.loadedExtensions[S.id];if(!$.instance)try{const T=$.data.content,O=new Function("DeviceAPI","html","emit","state",`
            ...
            `)(DeviceAPI,html$1,i,n),F=new DeviceAPI(BridgeDevice),D=new O(F,i,n,html$1);n.loadedExtensions[S.id].instance=D}catch(T){console.error(`[System] Error instantiating extension ${S.id}:`,T)}if($.instance&&typeof $.instance.getMenuItems=="function")try{const T=$.instance.getMenuItems();T&&Array.isArray(T)&&(S.menu=T)}catch(T){console.error(`[System] Error getting menu items for ${S.id}:`,T)}if($.instance&&typeof $.instance.renderSidebarDevices=="function")try{I=$.instance.renderSidebarDevices()}catch(T){console.error(`[System] Error rendering sidebar devices for ${S.id}:`,T)}else console.warn(`[System] renderSidebarDevices not available for ${S.id}`)}if(_&&n.loadedExtensions[S.id]?.instance){const $=n.loadedExtensions[S.id];if($.instance&&typeof $.instance.getMenuItems=="function")try{const T=$.instance.getMenuItems();T&&Array.isArray(T)&&(S.menu=T)}catch(T){console.error(`[System] Error getting menu items for ${S.id}:`,T)}}return html$1`
      <div class="system-sidebar-extension">
        <div 
...
${l.message}`)}});async function a(c,l,u,d,p){const f=c.id,h=Array.isArray(c.version)?c.version.join("."):String(c.version);s(`[Extensions] Running onInstall for ${c.name}...`),showStyledModal({variant:"",icon:"packages",title:"Installing Extension",subtitle:c.name,body:`<p>Installing extension files to device...</p>
             ...
             </div>`,buttons:[]});try{stopStatusInfoPolling();const m=(y,...w)=>y.reduce((b,C,x)=>b+C+(w[x]||""),""),v=instantiateExtension(l.content,u,d.emit.bind(d),m);if(typeof v.onInstall=="function"){updateModalBody(`<p>Writing files to device...</p>
          <div style="margin-top: 16px; color: var(--text-secondary);">
