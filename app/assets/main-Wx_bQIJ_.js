const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/tree-sitter-C57HRO-g.js","assets/vendor-XVACtT3C.js","assets/vendor-YJ71nRLj.js","assets/vendor-CkB03O3T.css","assets/index-D8uXHUWg.js","assets/index-BvC6oQL-.js","assets/index-CEIVfw6W.js","assets/index-Ckq7-Rzu.js","assets/index-BR6z4vJu.js","assets/index-Bvni1Ih1.js","assets/index-CzizjDTO.js","assets/xterm-DOrYoP_4.css"])))=>i.map(i=>d[i]);
...
`}),a+=`${e}_ds.st()
`,a}async function instrumentCodeForExec(e,n={}){const{watches:i={},conditionalBP:s={},fileName:o="main.py"}=n,r=performance.now(),c=await identifyCodeRows(e);console.log(`[Debugger] Identified ${c.size} code rows in ${performance.now()-r}ms`);let a=e.split(/\r?\n/);const d=new Map,l=i[""]||[],u=i[o]||[],p=[...new Set([...l,...u])],f=s[""]||[],h=s[o]||[],m=[...new Set([...f,...h])],v=Array.from(c.keys()).sort(($,b)=>$-b);for(const $ of v){const b=a[$],k=c.get($);if(b===void 0)continue;const x=/# ●/.test(b),P=n.breakpoints&&n.breakpoints[o]?n.breakpoints[o][$+1]:null;(x||P)&&console.log(`[Debugger] Breakpoint detected at line ${$+1}: ${b}`);const T=b.match(/^(\s*)/)[1];let E=generateDebugBlock(T,x,o,$+1,p,m,P);k==="function_definition"?E+=`${T}@_ds.wrap
`:k==="async_function_definition"&&(E+=`${T}@_ds.awrap
`),x&&console.log(`[Debugger] Generated instrumentation for row ${$+1} (${k})`),d.set($,E)}let y=[];for(let $=0;$<a.length;$++)d.has($)&&y.push(d.get($)),y.push(a[$]);const w=y.join(`
  ...
  `}const DiskFileList=generateFileList("disk"),BoardFileList=generateFileList("board");function generateFileList(e){return function(i,s){function o(f){f.key.toLowerCase()==="enter"&&f.target.blur(),f.key.toLowerCase()==="escape"&&(f.target.value=null,f.target.blur())}const r=html`
    ...
    `;function a(f,h){const m=html`
        <input type="text"
          ...
          ondblclick=${x=>!1}
          />
      `,v=i.selectedFiles.find(x=>x.fileName===f.fileName&&x.source===e);function y(x){return x.preventDefault(),s("rename-file",e,f),!1}function w(){i.renamingFile||s(`navigate-${e}-folder`,f.fileName)}function S(){i.renamingFile||s("open-file",e,f)}let C=f.fileName;const $=i.selectedFiles.find(x=>x.fileName===C);i.renamingFile==e&&$&&(C=m);function b(x){if(x==null)return"";if(x===0)return"0 B";const P=1024,T=["B","KB","MB","GB"],E=Math.floor(Math.log(x)/Math.log(P));return parseFloat((x/Math.pow(P,E)).toFixed(1))+" "+T[E]}const k=f.type==="file"?b(f.size):"";return f.type==="folder"?html`
          <div
    ...
    <div class="panel-section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <span class="info-value info-mono">${"724c746"}</span>
        </div>
        <div class="info-item">
          <span class="info-label">${e("sysinfo.studioBuildDate")}</span>
          <span class="info-value">${"2026-03-10"}</span>
        </div>
      </div>
    </div>
...
`)){const r=o.match(/^(\w+)\s*:\s*(.+)$/);r&&(n.meta[r[1].trim()]=r[2].trim())}n.body=e.slice(i[0].length)}return n.body=n.body.replace(/^ScriptO for\s+\[.*?\]\(.*?\)\.?\s*Run on device\s*\(MicroPython\)\.?\s*\n?/im,""),n.body=n.body.trim(),n}function SkillSidebar(e,n){if(!e.isSkillSidebarOpen||!e.skillSidebarContent)return html`<div></div>`;const{name:i,skillMd:s}=e.skillSidebarContent,{meta:o,body:r}=parseSkillMd(s);let c="";try{r&&(c=g.parse(r,{breaks:!0}))}catch(p){console.error("[SkillSidebar] Failed to parse markdown:",p),c=`<pre>${r}</pre>`}const a=[];o.description&&a.push(`<p class="skill-meta-description">${o.description}</p>`);const d=[];o.category&&d.push(`<span class="skill-meta-tag">${o.category}</span>`),o.author&&d.push(`<span class="skill-meta-tag">by ${o.author}</span>`),d.length&&a.push(`<div class="skill-meta-tags">${d.join("")}</div>`);const l=a.length?`<div class="skill-meta-block">${a.join("")}</div>`:"",u=document.createElement("div");return u.className="skill-sidebar-content skill-md-content",u.innerHTML=l+c,html`
  ...
  `,f=e.expandedNetworks!==!1,h=html$1`
    <div class="system-sidebar-extension">
      ...
      ${f?html$1`
        <div class="system-sidebar-submenu">
          ${c.map(b=>{const k=e.activeNetworkPanel===b.id;return html$1`
              <div 
                class="system-sidebar-subitem ${k?"active":""}"
...
`),u=typeof l=="object"?l:JSON.parse(l.trim());e.boardConfig={board_id:u.board_id,board_name:u.board_name,chip:u.chip,version:u.version,description:u.description,hardware:u.hardware};try{const f=new URL(r).hostname,h=u.board_id||f.replace(/\\./g,"-");await BridgeDisk.addOnboardedDevice(h,{hostname:f,url:r,board_id:u.board_id,board_name:u.board_name,chip:u.chip,version:u.version,connectedAt:new Date().toISOString()}),console.log("[Connection] Device saved to /onboarded/"),e.needsOnboarding=!1}catch(p){console.warn("[Connection] Could not save device to /onboarded/:",p)}n.emit("render")}catch(l){console.error("[BoardConfig] Failed to load board config:",l)}try{const l=(await __vitePreload(async()=>{const{DeviceAPI:y}=await Promise.resolve().then(()=>deviceApi);return{DeviceAPI:y}},void 0)).DeviceAPI,{updateDeviceExtensionRegistry:u,needsInstall:p}=await __vitePreload(async()=>{const{updateDeviceExtensionRegistry:y,needsInstall:w}=await Promise.resolve().then(()=>deviceRegistry);return{updateDeviceExtensionRegistry:y,needsInstall:w}},void 0),f=new l(BridgeDevice),h=await readDeviceExtensionRegistry(f),m=new Set(Object.keys(h)),v=localStorage.getItem("pending-extension-id");for(const y in e.loadedExtensions)e.loadedExtensions[y].instance=null;console.log(`[Extensions] Device registry has ${Object.keys(h).length} extensions, sidebar showing: ${e.installedExtensions.map(y=>y.id).join(", ")||"(none)"}`),n.emit("render"),v&&!h[v]&&(console.log(`[Extensions] Pending extension ${v} not in device registry — running onInstall`),(async()=>{try{const{ExtensionRegistry:y}=await __vitePreload(async()=>{const{ExtensionRegistry:T}=await Promise.resolve().then(()=>extensionLoader);return{ExtensionRegistry:T}},void 0),S=await new y().getExtension(v);if(!S||!S.content){console.warn(`[Extensions] No cached data for ${v}, skipping onInstall`);return}const C=decodeDeviceFiles(S.content);if(Object.keys(C).length===0){console.log(`[Extensions] ${v} has no device files, skipping onInstall`);const T=S.config?.version,E=Array.isArray(T)?T.join("."):String(T||"0.0.0");await u(f,v,E);return}const $=new Blob([S.content],{type:"text/javascript"}),b=URL.createObjectURL($);let k;try{k=(await import(b)).default}finally{URL.revokeObjectURL(b)}if(!k){console.warn(`[Extensions] No default export in ${v} bundle`);return}const{html:x}=await __vitePreload(async()=>{const{html:T}=await import("./vendor-XVACtT3C.js");return{html:T}},__vite__mapDeps([1,2,3])),P=new k(f,n.emit.bind(n),e,x);if(P.deviceFiles=C,typeof P.onInstall=="function"){if(console.log(`[Extensions] Running onInstall for ${v}...`),await P.onInstall()===!1){console.error(`[Extensions] onInstall returned false for ${v}`);return}const E=S.config?.version,_=Array.isArray(E)?E.join("."):String(E||"0.0.0");await u(f,v,_),console.log(`[Extensions] onInstall complete for ${v}`)}localStorage.removeItem("pending-extension-id")}catch(y){console.error(`[Extensions] Deferred onInstall failed for ${v}:`,y)}})())}catch(l){console.warn("[Extensions] Could not read device extension registry:",l)}})(),n.emit("render")}catch(d){let l="Connection failed";d instanceof Error||d&&d.message?l=d.message:d&&d.type==="error"&&(l=`WebSocket connection failed. Check if device is available at ${r}`),console.log("✗ Connection failed:",l),e.isConnecting=!1,e.isConnected=!1,updateOverlayDirectly(e),n.emit("render")}}),n.on("open-reset-dialog",()=>{e.isResetDialogOpen=!0,n.emit("render")}),n.on("close-reset-dialog",()=>{e.isResetDialogOpen=!1,n.emit("render")}),n.on("trigger-reset",async r=>{e.isResetDialogOpen=!1,n.emit("render");try{r===1?(console.log("[Connection] Triggering Hard Reset"),e.isResettingHard=!0,await i.reset(1),console.log("[Connection] Hard reset sent, waiting for disconnect...")):(console.log("[Connection] Triggering Soft Reset"),await i.reset(0))}catch(c){console.error("[Connection] Reset failed:",c),e.isResettingHard=!1,alert("Reset failed: "+c.message)}}),n.on("bind-terminal-input",()=>{const r=e.cache(XTerm,"terminal");!r||!r.term||r.bindInput(e,i)&&console.log("[Connection] Bound terminal input after view change")}),n.on("terminal-focus",async()=>{if(!e.isConnected||e.bannerDisplayed)return;const r=e.cache(XTerm,"terminal");if(!r||!r.term)return;const c=r.term;try{const a=await i.exec(`
import os, json
u = os.uname()
