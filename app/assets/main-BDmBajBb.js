      ...
      <div class="scriptos-content">
        ...
        `:html`
          <div class="scriptos-categories">
            ${p.map(y=>{const b=e.scriptOsCategoryCollapse[y]===!0;return html`
    ...
    <div class="panel-section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <span class="info-value info-mono">${"3c2fcf6"}</span>
        </div>
        <div class="info-item">
  ...
  `}function NTPPanel(e,n){e.isConnected&&!e.ntpConfigLoaded&&!e.isLoadingNtpConfig&&n("load-ntp-config");const i=e.ntpConfig||{server:"pool.ntp.org",tzOffset:0,timezone:"UTC",autoDetect:!1},s=e.ntpSyncResult||null,o=l=>{if(!l)return"--:--:--";const{year:p,month:u,day:f,hour:h,minute:m,second:v}=l;return`${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}:${String(v).padStart(2,"0")}`},r=l=>{if(!l)return"";const{year:p,month:u,day:f}=l;return`${p}-${String(u).padStart(2,"0")}-${String(f).padStart(2,"0")}`},d=[{value:"UTC",offset:0,label:"UTC (Coordinated Universal Time)"},{value:"EST",offset:-5,label:"EST (Eastern Standard Time)"},{value:"CST",offset:-6,label:"CST (Central Standard Time)"},{value:"MST",offset:-7,label:"MST (Mountain Standard Time)"},{value:"PST",offset:-8,label:"PST (Pacific Standard Time)"},{value:"GMT",offset:0,label:"GMT (Greenwich Mean Time)"},{value:"CET",offset:1,label:"CET (Central European Time)"},{value:"EET",offset:2,label:"EET (Eastern European Time)"},{value:"JST",offset:9,label:"JST (Japan Standard Time)"},{value:"AEST",offset:10,label:"AEST (Australian Eastern Standard Time)"}],a=i.timezone,c=[...d];return a&&!d.find(l=>l.value===a)&&c.unshift({value:a,offset:i.tzOffset||0,label:`${a} (Detected)`}),html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${async l=>{l.preventDefault();const p=new FormData(l.target),u=p.get("server")||"pool.ntp.org",f=p.get("timezone")||"UTC",h=p.get("auto_detect")==="on",m=c.find(b=>b.value===f),v=m?m.offset:0,y={server:u,tz_offset:v,timezone:f,auto_detect:h};try{await new Promise((b,S)=>{const x=setTimeout(()=>S(new Error("Save timeout")),1e4),C=()=>{clearTimeout(x),window.appInstance.emitter.removeListener("ntp-config-saved",w),window.appInstance.emitter.removeListener("ntp-config-save-error",k)},w=()=>{C(),b()},k=E=>{C(),S(E)};window.appInstance.emitter.once("ntp-config-saved",w),window.appInstance.emitter.once("ntp-config-save-error",k),n("save-ntp-config",y)}),n("sync-ntp-time",u,v,h)}catch(b){console.error("[NTP] Failed to save config before sync:",b),alert(`Failed to save NTP configuration: ${b.message}`)}}}>
          <div class="form-group">
...
Please check your internet connection and try again.`)}}),n.on("select-scriptos",async r=>{if(o("select-scriptos:",r.displayName||r.slug),r.slug&&!r.content){const d=r.slug;console.log("[Registry] Fetching skill from ScriptoHub:",d);try{const a=await getSkill(d,e.scriptohubApiUrl);if(!a||!a.latestVersion)throw new Error(`Skill "${d}" has no published version`);const l=(await getSkillVersionFiles(d,a,e.scriptohubApiUrl)).files.find(h=>h.path.endsWith(".py"));if(!l)throw new Error(`Skill "${d}" has no .py file`);const[p,u]=await Promise.all([getSkillFile(d,l.path,e.scriptohubApiUrl),getSkillFile(d,"SKILL.md",e.scriptohubApiUrl).catch(()=>null)]),f=parseScriptOsConfig(p);if(!f)throw new Error("Failed to parse ScriptO configuration");console.log("[Registry] Fetched skill:",d,"py:",l.path,"skill.md:",!!u),r={slug:d,filename:l.path,displayName:r.displayName||a.skill?.displayName,content:p,config:f,skillMd:u}}catch(a){console.error("[Registry] Error fetching skill:",a),alert(`Failed to load skill:
