...
`),x&&console.log(`[Debugger] Generated instrumentation for row ${$+1} (${T})`),d.set($,k)}let w=[];for(let $=0;$<l.length;$++)d.has($)&&w.push(d.get($)),w.push(l[$]);const S=w.join(`
      ...
      <div class="scriptos-content">
        ...
        `:p.length<=1&&u.length===0?html`
          ...
          <div class="scriptos-grid">
            ${l.map(f=>ScriptOsCard(f,i,s.length<=1))}
          </div>
        `:html`
...
`)).match(/CREDS:(\{.*\})/);if(o){const r=JSON.parse(o[1]);panelState.credentials={hostname:r.h+".local",password:r.p,ssid:r.s},addLog(`Found: ${r.h}.local`,"success")}else throw new Error("Could not read settings - device may need firmware");triggerRerender()}catch(n){addLog(`Error: ${n.message}`,"error"),triggerRerender()}}async function performWifiSetup(){clearLog();try{let l=function(d){return new Promise((u,p)=>{const h=document.createElement("div");h.className="fw-modal-overlay active",h.innerHTML=`
        ...
        `,document.body.appendChild(h);const f=y=>{const w=y.target.closest(".fw-modal-network-item");if(w){const S=parseInt(w.dataset.index);v(),u(S)}},m=()=>{v(),p(new Error("Setup cancelled"))},v=()=>{h.remove()};h.querySelector(".fw-network-modal-list").addEventListener("click",f),h.querySelector(".fw-modal-cancel").addEventListener("click",m)})};addLog("Connecting to REPL...");const n=await transitionToREPL();if(!n.success){await handleBootFailure(n.error);return}addLog("Scanning for WiFi networks...");const o=(await sendREPLCommand(currentPort,`
import network
import json
...
${l.message}`)}});async function a(c,l,d,u,p){const h=c.id,f=Array.isArray(c.version)?c.version.join("."):String(c.version);s(`[Extensions] Running onInstall for ${c.name}...`),showStyledModal({variant:"",icon:"packages",title:"Installing Extension",subtitle:c.name,body:`<p>Installing extension files to device...</p>
             ...
             </div>`,buttons:[]});try{stopStatusInfoPolling();const m=(y,...w)=>y.reduce((S,E,C)=>S+E+(w[C]||""),""),v=instantiateExtension(l.content,d,u.emit.bind(u),m);if(typeof v.onInstall=="function"){updateModalBody(`<p>Writing files to device...</p>
          <div style="margin-top: 16px; color: var(--text-secondary);">
