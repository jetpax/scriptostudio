...
`),p+=`# Configuration
`,p+=`class args:
`;for(const f in i.args){const h=i.args[f];let m=o[f];m==null&&(m=h.value);let v;m==null?v="None":typeof m=="string"?v=`'${escapePythonString(m)}'`:typeof m=="boolean"?v=m?"True":"False":typeof m=="number"?v=m.toString():Array.isArray(m)?v="["+m.map(y=>typeof y=="string"?`'${escapePythonString(y)}'`:y).join(", ")+"]":v="None",p+=`    ${f} = ${v}
`}return p+=`
`,u&&(p+=u),p}function escapePythonString(n){return String(n).replace(/\\/g,"\\\\").replace(/'/g,"\\'").replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/\t/g,"\\t")}class IndexedDBFileBridge{constructor(){this.DB_NAME="scripto-studio-files",this.DB_VERSION=1,this.STORE_FILES="files",this.rootPath="/"}async _initDB(){return new Promise((i,o)=>{const r=indexedDB.open(this.DB_NAME,this.DB_VERSION);r.onerror=()=>o(r.error),r.onsuccess=()=>i(r.result),r.onupgradeneeded=s=>{const a=s.target.result;a.objectStoreNames.contains(this.STORE_FILES)||a.createObjectStore(this.STORE_FILES)}})}_normalizePath(i){return i?(i=i.replace(this.rootPath,""),i.startsWith("/")||(i="/"+i),i!=="/"&&i.endsWith("/")&&(i=i.slice(0,-1)),i):"/"}async _getFilesInDirectory(i){const o=this._normalizePath(i),r=o==="/"?"/":o+"/",s=await this._initDB();return new Promise((a,c)=>{const d=s.transaction([this.STORE_FILES],"readonly").objectStore(this.STORE_FILES).openCursor(),p=[],f=new Set;d.onsuccess=h=>{const m=h.target.result;if(m){const v=m.key,y=m.value;if(v.startsWith(r)){const w=v.substring(r.length);if(v.endsWith("/")&&y.type==="folder"){const b=w.slice(0,-1);b&&!b.includes("/")&&!f.has(b)&&(f.add(b),p.push({path:b,type:"folder"}))}else if(w&&!w.includes("/"))p.push({path:w,type:y.type||"file",content:y.content,timestamp:y.timestamp,size:y.size});else if(w.includes("/")){const b=w.split("/")[0];f.has(b)||(f.add(b),p.push({path:b,type:"folder"}))}}m.continue()}else p.sort((v,y)=>v.type===y.type?v.path.localeCompare(y.path):v.type==="folder"?-1:1),a(p)},d.onerror=()=>c(d.error)})}async initialize(){try{return await this._initDB(),!0}catch(i){return console.error("[IDB FS] Error initializing:",i),!1}}isSupported(){return"indexedDB"in window}async openFolder(){const i=await this.ilistFiles("/");return{folder:this.rootPath,files:i}}async ilistFiles(i){try{return(await this._getFilesInDirectory(i)).map(r=>{let s=r.size;return s===void 0&&r.type==="file"&&(r.content?typeof r.content=="string"?s=new TextEncoder().encode(r.content).length:r.content instanceof ArrayBuffer?s=r.content.byteLength:r.content instanceof Uint8Array?s=r.content.length:r.content instanceof Blob&&(s=r.content.size):s=0),{path:r.path,type:r.type,size:s}})}catch(o){return console.error("[IDB FS] Error listing files:",o),[]}}async ilistAllFiles(i){try{const o=this._normalizePath(i),r=o==="/"?"/":o+"/",s=await this._initDB();return new Promise((a,c)=>{const d=s.transaction([this.STORE_FILES],"readonly").objectStore(this.STORE_FILES).openCursor(),p=[],f=new Set;d.onsuccess=h=>{const m=h.target.result;if(m){const v=m.key,y=m.value;if(v.startsWith(r)){const w=v.substring(r.length);if(w){const b=w.split("/");let C=r.slice(0,-1);for(let E=0;E<b.length;E++)C+="/"+b[E],f.has(C)||(f.add(C),E===b.length-1&&y.type==="file"?p.push({path:C,type:"file"}):E<b.length-1&&p.push({path:C,type:"folder"}))}}m.continue()}else a(p)},d.onerror=()=>c(d.error)})}catch(o){return console.error("[IDB FS] Error listing all files:",o),[]}}async loadFile(i){try{const o=this._normalizePath(i),r=await this._initDB();return new Promise((s,a)=>{const u=r.transaction([this.STORE_FILES],"readonly").objectStore(this.STORE_FILES).get(o);u.onsuccess=()=>{const d=u.result;if(!d||d.type!=="file"){a(new Error(`File not found: ${i}`));return}const p=d.content||"",h=new TextEncoder().encode(p).buffer;console.log("[IDB FS] Loaded file:",i,"(",h.byteLength,"bytes)"),s(h)},u.onerror=()=>a(u.error)})}catch(o){throw console.error("[IDB FS] Error loading file:",o),new Error(`Failed to load file: ${o.message}`)}}async saveFileContent(i,o){try{const r=this._normalizePath(i);let s=o;o instanceof Uint8Array?s=new TextDecoder().decode(o):o instanceof ArrayBuffer?s=new TextDecoder().decode(new Uint8Array(o)):o instanceof Blob?s=await o.text():typeof o!="string"&&(s=String(o));const a=await this._initDB();return new Promise((c,l)=>{const p=a.transaction([this.STORE_FILES],"readwrite").objectStore(this.STORE_FILES).put({type:"file",content:s,timestamp:Date.now(),size:new TextEncoder().encode(s).length},r);p.onsuccess=()=>{console.log("[IDB FS] Saved file:",i),c(!0)},p.onerror=()=>l(p.error)})}catch(r){throw console.error("[IDB FS] Error saving file:",r),new Error(`Failed to save file: ${r.message}`)}}async importFiles(i="/"){return new Promise((o,r)=>{const s=document.createElement("input");s.type="file",s.multiple=!0,s.accept="*/*",s.onchange=async a=>{try{const c=Array.from(a.target.files);if(c.length===0){o([]);return}const l=[];for(const u of c){const d=await u.text(),p=this.getFullPath(i,"",u.name);await this.saveFileContent(p,d),l.push({name:u.name,path:p,size:u.size}),console.log("[IDB FS] Imported file:",u.name,"→",p)}o(l)}catch(c){console.error("[IDB FS] Error importing files:",c),r(c)}},s.oncancel=()=>{o([])},s.click()})}async fileExists(i){try{const o=this._normalizePath(i),r=await this._initDB();return new Promise((s,a)=>{const u=r.transaction([this.STORE_FILES],"readonly").objectStore(this.STORE_FILES).get(o);u.onsuccess=()=>{const d=u.result;s(d&&d.type==="file")},u.onerror=()=>a(u.error)})}catch{return!1}}async folderExists(i){try{const o=this._normalizePath(i),r=o.endsWith("/")?o:o+"/",s=o==="/"?"/":o+"/",a=await this._initDB();return new Promise((c,l)=>{const d=a.transaction([this.STORE_FILES],"readonly").objectStore(this.STORE_FILES),p=d.get(r);p.onsuccess=()=>{if(p.result&&p.result.type==="folder"){c(!0);return}const f=d.openCursor();f.onsuccess=h=>{const m=h.target.result;if(m){const v=m.key;if(v.startsWith(s)&&v!==r){c(!0);return}m.continue()}else c(!1)},f.onerror=()=>l(f.error)},p.onerror=()=>l(p.error)})}catch{return!1}}async removeFile(i){try{const o=this._normalizePath(i),r=await this._initDB();return new Promise((s,a)=>{const u=r.transaction([this.STORE_FILES],"readwrite").objectStore(this.STORE_FILES).delete(o);u.onsuccess=()=>{console.log("[IDB FS] Removed file:",i),s(!0)},u.onerror=()=>a(u.error)})}catch(o){throw console.error("[IDB FS] Error removing file:",o),new Error(`Failed to remove file: ${o.message}`)}}async renameFile(i,o){try{const r=this._normalizePath(i),s=this._normalizePath(o),a=await this.loadFile(i);return await this.saveFileContent(o,a),await this.removeFile(i),console.log("[IDB FS] Renamed file:",i,"->",o),!0}catch(r){throw console.error("[IDB FS] Error renaming file:",r),new Error(`Failed to rename file: ${r.message}`)}}async createFolder(i){try{const o=this._normalizePath(i),r=o.endsWith("/")?o:o+"/",s=await this._initDB();return new Promise((a,c)=>{const u=s.transaction([this.STORE_FILES],"readwrite").objectStore(this.STORE_FILES),d=u.get(r);d.onsuccess=()=>{if(d.result){console.log("[IDB FS] Folder already exists:",i),a(!0);return}const p=u.put({type:"folder",timestamp:Date.now()},r);p.onsuccess=()=>{console.log("[IDB FS] Created folder:",i),a(!0)},p.onerror=()=>c(p.error)},d.onerror=()=>c(d.error)})}catch(o){throw console.error("[IDB FS] Error creating folder:",o),new Error(`Failed to create folder: ${o.message}`)}}async removeFolder(i){try{const o=this._normalizePath(i),r=o==="/"?"/":o+"/",s=o.endsWith("/")?o:o+"/",a=await this._initDB();return new Promise((c,l)=>{const p=a.transaction([this.STORE_FILES],"readwrite").objectStore(this.STORE_FILES).openCursor(),f=[];p.onsuccess=h=>{const m=h.target.result;if(m){const v=m.key;(v.startsWith(r)||v===o||v===s)&&f.push(m.delete()),m.continue()}else Promise.all(f).then(()=>{console.log("[IDB FS] Removed folder:",i),c(!0)}).catch(l)},p.onerror=()=>l(p.error)})}catch(o){throw console.error("[IDB FS] Error removing folder:",o),new Error(`Failed to remove folder: ${o.message}`)}}async listFiles(i){return this.ilistFiles(i)}getFullPath(i,o,r){let s=i||"/";return o&&o!=="/"&&(s+=(s.endsWith("/")?"":"/")+o.replace(/^\//,"")),r&&(s+=(s.endsWith("/")?"":"/")+r),s.startsWith("/")||(s="/"+s),s}getNavigationPath(i,o){if(o===".."){const r=i.split("/").filter(s=>s);return r.pop(),"/"+r.join("/")}return i==="/"?"/"+o:i+"/"+o}async getAppPath(){return this.rootPath}async clearWorkspace(){try{const i=await this._initDB();return new Promise((o,r)=>{const c=i.transaction([this.STORE_FILES],"readwrite").objectStore(this.STORE_FILES).clear();c.onsuccess=()=>{console.log("[IDB FS] Workspace cleared"),o()},c.onerror=()=>r(c.error)})}catch(i){console.error("[IDB FS] Error clearing workspace:",i)}}async listScriptOsFiles(){try{const i="/ScriptOs",o=await this.ilistFiles(i),r=[];console.log(`[IDB FS] Found ${o.length} items in ScriptOs directory`);for(const s of o)if(s.type==="file"&&s.path.endsWith(".py"))try{const a=i+"/"+s.path,c=await this.loadFile(a),l=new TextDecoder().decode(new Uint8Array(c)),u=parseScriptOsConfig(l);u?(r.push({filename:s.path,fullPath:a,content:l,config:u}),console.log(`[IDB FS] Loaded ScriptO: ${u.info?.name||s.path}`)):console.warn(`[IDB FS] No valid config found in: ${s.path}`)}catch(a){console.error(`[IDB FS] Error loading ScriptO ${s.path}:`,a)}return console.log(`[IDB FS] Successfully loaded ${r.length} ScriptOs`),r}catch(i){return console.error("[IDB FS] Error listing ScriptOs files:",i),[]}}async hasOnboardedDevices(){try{return(await this.ilistFiles("/onboarded")).some(o=>o.type==="file"&&o.path.endsWith(".json"))}catch{return!1}}async getOnboardedDevices(){try{const i=await this.ilistFiles("/onboarded"),o=[];for(const r of i)if(r.type==="file"&&r.path.endsWith(".json"))try{const s=await this.loadFile("/onboarded/"+r.path),a=new TextDecoder().decode(new Uint8Array(s));o.push(JSON.parse(a))}catch(s){console.warn("[IDB FS] Error parsing device file:",r.path,s)}return o}catch(i){return console.error("[IDB FS] Error listing onboarded devices:",i),[]}}async addOnboardedDevice(i,o){try{await this.createFolder("/onboarded");const s="/onboarded/"+(i.replace(/:/g,"")+".json");await this.saveFileContent(s,JSON.stringify(o,null,2)),console.log("[IDB FS] Added onboarded device:",o.hostname||i)}catch(r){throw console.error("[IDB FS] Error adding onboarded device:",r),r}}}const BridgeDisk=new IndexedDBFileBridge,AIAgentSystemPrompt=`You are an expert MicroPython developer specializing in ESP32 microcontrollers.
...
`,s.forEach(u=>{const d=u.replace(/"/g,'\\"');l+=`${n}try:
`,l+=`${n}    _ds.d["w"]["${d}"] = str(${u})
`,l+=`${n}except Exception as _debug_e:
`,l+=`${n}    _ds.d["w"]["${d}"] = str(_debug_e)
`}),l+=`${n}_ds.st()
`,l}async function instrumentCodeForExec(n,i={}){const{watches:o={},conditionalBP:r={},fileName:s="main.py"}=i,a=performance.now(),c=await identifyCodeRows(n);console.log(`[Debugger] Identified ${c.size} code rows in ${performance.now()-a}ms`);let l=n.split(/\r?\n/);const u=new Map,d=o[""]||[],p=o[s]||[],f=[...new Set([...d,...p])],h=r[""]||[],m=r[s]||[],v=[...new Set([...h,...m])],y=Array.from(c.keys()).sort(($,S)=>$-S);for(const $ of y){const S=l[$],_=c.get($);if(S===void 0)continue;const x=/# ●/.test(S),I=i.breakpoints&&i.breakpoints[s]?i.breakpoints[s][$+1]:null;(x||I)&&console.log(`[Debugger] Breakpoint detected at line ${$+1}: ${S}`);const P=S.match(/^(\s*)/)[1];let k=generateDebugBlock(P,x,s,$+1,f,v,I);_==="function_definition"?k+=`${P}@_ds.wrap
`:_==="async_function_definition"&&(k+=`${P}@_ds.awrap
`),x&&console.log(`[Debugger] Generated instrumentation for row ${$+1} (${_})`),u.set($,k)}let w=[];for(let $=0;$<l.length;$++)u.has($)&&w.push(u.get($)),w.push(l[$]);const b=w.join(`
`),C=`${DEBUG_STATE_MODULE}

# Execute isolated code
_ds.exec("""${b.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/'/g,"\\'")}""")
`,E=performance.now()-a;return console.log(`[Debugger] Instrumentation complete in ${E.toFixed(0)}ms`),C}console.log("[Libs] ES modules loaded");function renderIcon(n,i={}){const{className:o="",size:r=24,color:s="currentColor"}=i;return html$1`
  ...
  `}const IconSprite={renderIcon};await __vitePreload(()=>import("./vendor-D5iZ5CP7.js"),__vite__mapDeps([1,2,3]));console.log("[Views] Vendor loaded, Component available:",!!window.Component);function Button(n){const{first:i=!1,size:o="",square:r=!1,icon:s="link",onClick:a=$=>{},disabled:c=!1,active:l=!1,tooltip:u,label:d,background:p}=n;let f=html``;u&&(f=html`<div class="tooltip">${u}</div>`),f=html``;let h=l?"active":"",m=l?"selected":"",v=p?"inverted":"",y=i?"first":"",w=r?"square":"",b=c?"inactive":"active",C=o==="small"?"":html`<div class="label ${b} ${m}">${d}</div>`;const E=IconSprite.renderIcon(s,{className:""});return html`
     <div class="button ${y}">
...
`}catch{return null}}toggleBreakpoint(i){if(!this.view)return;const o=this.view.state.doc.line(i+1),r=o.text,s=/# ●/.test(r);let a;s?a=r.replace(/\s*# ●.*/,""):a=r.trimEnd()+" # ●",this.view.dispatch({changes:{from:o.from,to:o.to,insert:a}})}syncBreakpointsFromStore(){if(!this.view||!this.fileName)return;const i=window.appState?.debugger?.breakpoints?.[this.fileName]||{},o=this.view.state.doc,r=[];for(let s=1;s<=o.lines;s++){const a=o.line(s),c=a.text,l=i[s],u=/# ●/.test(c);if(l&&!u)r.push({from:a.to,to:a.to,insert:" # ●"});else if(!l&&u){const d=c.match(/\s*# ●.*/);if(d){const p=a.from+c.indexOf(d[0]);r.push({from:p,to:a.to,insert:""})}}}r.length>0&&this.view.dispatch({changes:r})}}const editor=Object.freeze(Object.defineProperty({__proto__:null,CodeMirrorEditor},Symbol.toStringTag,{value:"Module"}));function Tab(n){const{text:i="undefined",icon:o="device-desktop",onSelectTab:r=()=>!1,onCloseTab:s=()=>!1,onStartRenaming:a=()=>!1,onFinishRenaming:c=()=>!1,active:l=!1,renaming:u=!1,hasChanges:d=!1}=n;if(l)if(u){let h=function(v){c(v.target.value)},m=function(v){v.key.toLowerCase()==="enter"&&v.target.blur(),v.key.toLowerCase()==="escape"&&(v.target.value=null,v.target.blur())};return html`
      ...
      `}else{let h=function(m){m.stopPropagation(),s(m)};return html`
        <div class="tab active" tabindex="0">
          ${IconSprite.renderIcon(o,{className:"icon"})}
      ...
      `}function p(h){h.target.classList.contains("close-tab")||r(h)}function f(h){h.stopPropagation(),s(h)}return html`
    <div
      class="tab"
      tabindex="1"
      onclick=${p}
...
`)}}n.write(TERMINAL_PROMPT)}else if(r==="	"){if(i.isConnected&&o&&typeof o.requestCompletion=="function")try{const s=await o.requestCompletion(i.currentLine);if(!s||s.length===0){n.write("\x07");return}const a=d=>d&&d.length>=2&&/^[A-Z][A-Z0-9_]*$/.test(d),c=s.filter(d=>!a(d));if(c.length===0){n.write("\x07");return}let l="",u=[];if(c.length===1)l=c[0];else{const d=i.currentLine;let p=c[0];for(let f=1;f<c.length;f++){let h=0;for(;h<p.length&&h<c[f].length&&p[h]===c[f][h];)h++;p=p.slice(0,h)}p.length>d.length&&(l=p.slice(d.length)),u=c}if(l&&(i.currentLine=i.currentLine.slice(0,i.cursorPos)+l+i.currentLine.slice(i.cursorPos),i.cursorPos+=l.length,redrawLine(n,i.currentLine,i.cursorPos,o)),u.length>0){n.write(`\r
`);const d=80;let p="";for(const f of u){const h=p.length+f.length+(p?4:0);p&&h>d?(n.write(p+`\r
`),p=f):p=p?p+"    "+f:f}p&&n.write(p+`\r
    ...
    <div class="dialog-content webrepl-dialog">
      ...
      <div class="dialog-body">
        ...
        <div class="dialog-footer">
          <button class="connect-button" onclick=${u} disabled=${n.isConnecting}>
...
`;if(n.isNewFileDialogOpen){const h=f.querySelector("#dialog-new-file .dialog-content > input");return h&&h.focus(),f}}function ScriptOsModal(n,i){return n.isScriptOsModalOpen?html`
  ...
  `:html`<div></div>`}function ScriptOsLibraryView(n,i){if(n.scriptOsCategoryCollapse||(n.scriptOsCategoryCollapse={}),n.isLoadingRegistry)return html`
    ...
    `;const o=w=>{if(w.slug){const b=[];return w.category&&b.push(w.category),w.skillType&&b.push(w.skillType==="skill"?"📦 Package":"🔧 Tool"),{name:w.displayName||w.slug,description:w.summary||"",tags:b,author:"",version:w.latestVersion?.version||"1.0.0"}}else return w.config&&w.config.info?{name:w.config.info.name||w.filename,description:w.config.info.description||"",tags:w.config.info.category?[w.config.info.category]:[],author:w.config.info.author||"",version:w.config.info.version||[1,0,0]}:{name:w.filename||"Unknown",description:"",tags:[],author:"",version:[1,0,0]}},r=new Set;n.scriptOsList.forEach(w=>{o(w).tags.forEach(C=>r.add(C))});const s=Array.from(r).sort(),a=(n.scriptOsSearchQuery||"").toLowerCase(),c=n.scriptOsFilterTags||[],l=n.scriptOsList.filter(w=>{const b=o(w),C=b.name.toLowerCase(),E=b.description.toLowerCase(),$=b.tags.join(" ").toLowerCase(),S=!a||C.includes(a)||E.includes(a)||$.includes(a),_=c.length===0||c.every(x=>b.tags.includes(x));return S&&_}),u={},d=[];l.forEach(w=>{const b=o(w),C=b.tags.length>0?b.tags[0]:null;C?(u[C]||(u[C]=[]),u[C].push(w)):d.push(w)});const p=Object.keys(u).sort(),f=w=>{n.scriptOsCategoryCollapse[w]=!n.scriptOsCategoryCollapse[w],i("render")},h=()=>{n.scriptOsFiltersCollapsed=!n.scriptOsFiltersCollapsed,i("render")},m=()=>{n.scriptOsViewMode=n.scriptOsViewMode==="cards"?"list":"cards",i("render")},v=n.scriptOsFiltersCollapsed,y=n.scriptOsViewMode||"cards";return html`
    <div class="scriptos-library">
          ...
          ${s.length>1&&!v?html`
            <div class="scriptos-tag-filters">
              <div class="scriptos-tag-filters-list">
                ${s.map(w=>{const b=c.includes(w);return html`
                    <button 
        ...
        `:p.length<=1&&d.length===0?html`
          ...
          `:html`
            <div class="scriptos-grid">
              ${l.map(w=>ScriptOsCard(w,i,s.length<=1))}
            </div>
          `}
        `:html`
          <div class="scriptos-categories">
            ${p.map(w=>{const b=n.scriptOsCategoryCollapse[w]===!0;return html`
                <div class="scriptos-category ${b?"collapsed":""}">
                  <h3 class="scriptos-category-title" onclick=${()=>f(w)}>
                    ...
                    <span>${w}</span>
                    <span class="scriptos-category-count">${u[w].length}</span>
                  </h3>
                  ${b?"":y==="list"?html`
                    <div class="scriptos-list">
                      ${u[w].map(C=>ScriptOsListRow(C,i))}
                    </div>
                  `:html`
                    <div class="scriptos-grid">
                      ${u[w].map(C=>ScriptOsCard(C,i,!1))}
                    </div>
                  `}
                </div>
              `})}
            ${d.length>0?html`
              <div class="scriptos-category ${n.scriptOsCategoryCollapse.Other?"collapsed":""}">
                ...
                ${n.scriptOsCategoryCollapse.Other?"":y==="list"?html`
                  <div class="scriptos-list">
                    ${d.map(w=>ScriptOsListRow(w,i))}
                  </div>
                `:html`
                  <div class="scriptos-grid">
                    ${d.map(w=>ScriptOsCard(w,i,!1))}
                  </div>
                `}
              </div>
            `:""}
  ...
  `}function ScriptOsCard(n,i,o=!1){let r,s,a,c,l;if(n.slug)r=n.displayName||n.slug,s=n.summary||"",c=n.tags?Object.keys(n.tags):[],a=n.latestVersion?.version?`v${n.latestVersion.version}`:"v1.0.0",l="";else if(n.config&&n.config.info){const u=n.config.info;r=u.name||n.filename,s=u.description||"",c=u.category?[u.category]:[],l=u.author||"",u.version?a=Array.isArray(u.version)?`v${u.version.join(".")}`:`v${u.version}`:a="v1.0.0"}else r=n.filename||"Unknown",s="",c=[],a="v1.0.0",l="";return html`
    <div 
      class="scriptos-card"
      ...
      ${c.length>0&&!o?html`
        <div class="scriptos-card-tags">
          ${c.slice(0,3).map(u=>html`<span class="scriptos-tag-badge">${u}</span>`)}
  ...
  `}function ScriptOsListRow(n,i){let o,r,s;if(n.slug)o=n.displayName||n.slug,r=n.summary||"",s=n.latestVersion?.version?`v${n.latestVersion.version}`:"v1.0.0";else if(n.config&&n.config.info){const a=n.config.info;o=a.name||n.filename,r=a.description||"",a.version?s=Array.isArray(a.version)?`v${a.version.join(".")}`:`v${a.version}`:s="v1.0.0"}else o=n.filename||"Unknown",r="",s="v1.0.0";return html`
    <div class="scriptos-list-row" onclick=${()=>i("select-scriptos",n)}>
      <span class="scriptos-list-name">${o}</span>
      <span class="scriptos-list-version">${s}</span>
      <span class="scriptos-list-desc">${r}</span>
    </div>
  ...
  `}function renderInputField(n,i,o,r){const s=i.type,a=o.scriptOsArgs[n],c=i.value!==void 0?i.value:null;switch(s){case"str":return html`
      ...
      `;case"dict":const u=i.items||{},d=Object.keys(u);return html`
        <select 
          id="arg-${n}"
          class="scriptos-select"
  ...
  `}function ExtensionsLibraryView(n,i){const o=n.availableExtensions||[],r=n.installedExtensions||[],s=new Set(r.map(l=>l.id)),a=o.filter(l=>!s.has(l.id)),c=(l,u)=>{for(let d=0;d<3;d++){const p=l[d]||0,f=u[d]||0;if(p>f)return!0;if(p<f)return!1}return!1};return o.length===0?html`
      <div class="scriptos-library">
        <div class="scriptos-header-sticky">
           <div class="scriptos-header">
            <h2>Extensions</h2>
      ...
      <div class="scriptos-content">
        ${r.length>0?html`
          <div class="scriptos-section" style="margin-bottom: 32px">
            ...
            <div class="scriptos-grid">
              ${r.map(l=>{const u=o.find(p=>p.id===l.id),d=u&&c(u.version,l.version);return html`
                  <div class="scriptos-card installed ${d?"has-update":""}">
                    ...
                    <div class="scriptos-card-header">
                      ...
                      <span class="scriptos-card-version">
                        v${l.version.join(".")}
                        ${d?html`<span class="scriptos-version-arrow">→ v${u.version.join(".")}</span>`:""}
                      </span>
                    </div>
                    ...
                    <div class="scriptos-card-actions" style="margin-top: auto; padding-top: 12px; border-top: 1px solid var(--border-color); display: flex; gap: 8px;">
                      ${d?html`
                        <button 
                          class="scriptos-update-btn"
                          onclick=${p=>{p.stopPropagation(),i("update-extension",{extension:l,newVersion:u})}}
                          title="Update to v${u.version.join(".")}">
                          Update
                        </button>
                      `:""}
                      <button 
...
Are you sure you want to proceed?`)&&i("trigger-reset",1)}n.isResetDialogOpen&&window.addEventListener("keydown",function l(u){u.key==="Escape"&&(i("close-reset-dialog"),window.removeEventListener("keydown",l))},{once:!0});const c=html`
...
`),a=n.debugger.active;return html`
  ...
  `}function BreakpointModal(n,i){if(!n.debugger||!n.debugger.breakpointModalOpen)return html`<div></div>`;const{file:o,line:r}=n.debugger.editingBreakpoint,s=n.debugger.breakpoints[o][r]||{condition:"",hitCount:"",enabled:!0},a=()=>{i("debugger:save-breakpoint",{file:o,line:r,config:s})},c=()=>{i("debugger:delete-breakpoint",{file:o,line:r})},l=()=>{i("debugger:close-breakpoint-modal")};return html`
    <div class="scriptos-modal-overlay" onclick=${u=>{u.target.classList.contains("scriptos-modal-overlay")&&l()}}>
      <div class="scriptos-modal breakpoint-modal">
        <div class="breakpoint-modal-header">
          ...
          <div class="switch-container">
            <label class="switch">
              <input type="checkbox" checked=${s.enabled} onchange=${u=>{s.enabled=u.target.checked,i("render")}}>
        ...
        <div class="breakpoint-field">
          ...
          <input 
            ...
            value=${s.condition}
            oninput=${u=>{s.condition=u.target.value}}
          >
        </div>
        ...
        <div class="breakpoint-field">
          ...
          <input 
            ...
            value=${s.hitCount}
            oninput=${u=>{s.hitCount=u.target.value}}
          >
        </div>
...
`),console.log(`[device-registry] Updated ${i} to v${o}`)}catch(r){throw console.error("[device-registry] Error updating registry:",r),r}}function needsInstall(n,i){if(!i)return!0;const o=Array.isArray(n)?n.join("."):String(n),r=i.version||"";return o!==r}function hasOnInstallMethod(n){return n?/async\s+onInstall\s*\(/.test(n):!1}let lastExtensionDOM=null,lastExtensionKey=null;function shouldPreserveInput(n){const i=document.activeElement;if(i&&(i.tagName==="INPUT"||i.tagName==="TEXTAREA")){if(i.classList.contains("msg-input"))return!1;if(i.closest('.system-panel, .msg-chat, [class*="extension"]'))return!0}return!1}async function checkAndRunOnInstall(n,i,o,r,s,a){const c=o.data.config,l=Array.isArray(c.version)?c.version.join("."):String(c.version||"0.0.0");try{const d=(await readDeviceExtensionRegistry(r))[n];if(!needsInstall(l,d))return;s.loadedExtensions[n].installing=!0,a("render");try{await i.onInstall(),await updateDeviceExtensionRegistry(r,n,l)}catch(p){console.error("[ExtensionContainer] onInstall failed:",p),s.loadedExtensions[n].installError=p.message}finally{s.loadedExtensions[n].installing=!1,a("render")}}catch(u){console.warn("[ExtensionContainer] Could not check device registry:",u)}}async function instantiateExtension$1(extensionId,loadedExtension,state,emit,html){if(loadedExtension.data.styles&&!loadedExtension.stylesInjected){const n=`extension-styles-${extensionId}`;let i=document.getElementById(n);i||(i=document.createElement("style"),i.id=n,i.textContent=loadedExtension.data.styles,document.head.appendChild(i)),loadedExtension.stylesInjected=!0}const extensionCode=loadedExtension.data.content,filesMatch=extensionCode.match(/export\s+const\s+__DEVICE_FILES__\s*=\s*(\{[\s\S]*?\});/);let deviceFiles={};if(filesMatch)try{const rawFiles=eval("("+filesMatch[1]+")");for(const[n,i]of Object.entries(rawFiles))try{const o=atob(i),r=new Uint8Array(o.length);for(let s=0;s<o.length;s++)r[s]=o.charCodeAt(s);deviceFiles[n]=new TextDecoder("utf-8").decode(r)}catch{deviceFiles[n]=i}}catch(n){console.warn("[ExtensionContainer] Device files parse failed:",n)}const blob=new Blob([extensionCode],{type:"text/javascript"}),blobUrl=URL.createObjectURL(blob);let ExtensionClass;try{ExtensionClass=(await import(blobUrl)).default}finally{URL.revokeObjectURL(blobUrl)}if(!ExtensionClass)throw new Error("No default export found in extension bundle");const deviceAPI=new DeviceAPI$1(BridgeDevice),extensionInstance=new ExtensionClass(deviceAPI,emit,state,html);extensionInstance.deviceFiles=deviceFiles,loadedExtension.instance=extensionInstance,loadedExtension.instantiating=!1,emit("render"),state.isConnected&&typeof extensionInstance.onInstall=="function"&&checkAndRunOnInstall(extensionId,extensionInstance,loadedExtension,deviceAPI,state,emit)}function ExtensionContainer(n,i,o){const r=n.activeExtension,s=n.activeExtensionPanel,a=`${r}:${s}`;if(shouldPreserveInput()&&lastExtensionDOM&&lastExtensionKey===a)return console.debug("[ExtensionContainer] Preserving DOM during input focus"),lastExtensionDOM;if(!r||!s)return lastExtensionDOM=null,lastExtensionKey=null,o`
    ...
    `;const c=n.loadedExtensions[r];if(!c.instance&&!c.instantiating)return c.instantiating=!0,instantiateExtension$1(r,c,n,i,o).catch(d=>{console.error("[ExtensionContainer] Failed to instantiate extension:",d),c.instantiateError=d.message,c.instantiating=!1,i("render")}),lastExtensionDOM=null,lastExtensionKey=null,o`
      <div class="system-panel">
        <div class="panel-message">
          <p>Loading extension...</p>
        </div>
    ...
    `;const l=c.instance,u=`render${s.charAt(0).toUpperCase()+s.slice(1)}`;if(typeof l[u]!="function")return lastExtensionDOM=null,lastExtensionKey=null,o`
      <div class="system-panel">
        <div class="panel-message error">
          <p>Extension panel not found: ${s}</p>
          <p>Looking for method: ${u}</p>
        </div>
      </div>
    `;try{const d=l[u]();return lastExtensionDOM=d,lastExtensionKey=a,d}catch(d){return console.error("[ExtensionContainer] Render error:",d),lastExtensionDOM=null,lastExtensionKey=null,o`
      <div class="system-panel">
        <div class="panel-message error">
...
`;this.term.write(m),this.state.logs?.autoScroll!==!1&&this.term.scrollToBottom()}cleanup(){this.writeHandler&&(window.removeEventListener("log-terminal-write",this.writeHandler),this.writeHandler=null),this.clearHandler&&(window.removeEventListener("log-terminal-clear",this.clearHandler),this.clearHandler=null),this.resizeObserver&&(this.resizeObserver.disconnect(),this.resizeObserver=null),this.term&&(this.term.dispose(),this.term=null),this.fitAddon=null}}let logTerminalComponent=null;function LogSidebar(n,i){if(!n.logs.isOpen)return logTerminalComponent=null,null;logTerminalComponent||(logTerminalComponent=new LogTerminalComponent("log-terminal",n,i));const o=logTerminalComponent.render(n),r=n.logSidebarWidth||350;return html`
  ...
  `}const DiskFileList=generateFileList("disk"),BoardFileList=generateFileList("board");function generateFileList(n){return function(o,r){function s(h){h.key.toLowerCase()==="enter"&&h.target.blur(),h.key.toLowerCase()==="escape"&&(h.target.value=null,h.target.blur())}const a=html`
      <div class="item">
        ${IconSprite.renderIcon("file",{className:"icon"})}
        <div class="text">
          <input type="text" onkeydown=${s} onblur=${h=>r("finish-creating-file",h.target.value)}/>
        </div>
      </div>
    `,c=html`
      <div class="item">
        ...
        <div class="text">
          <input type="text" onkeydown=${s} onblur=${h=>r("finish-creating-folder",h.target.value)}/>
        </div>
      </div>
    `;function l(h,m){const v=html`
        <input type="text"
          ...
          ondblclick=${x=>!1}
          />
      `,y=o.selectedFiles.find(x=>x.fileName===h.fileName&&x.source===n);function w(x){return x.preventDefault(),r("rename-file",n,h),!1}function b(){o.renamingFile||r(`navigate-${n}-folder`,h.fileName)}function C(){o.renamingFile||r("open-file",n,h)}let E=h.fileName;const $=o.selectedFiles.find(x=>x.fileName===E);o.renamingFile==n&&$&&(E=v);function S(x){if(x==null)return"";if(x===0)return"0 B";const I=1024,P=["B","KB","MB","GB"],k=Math.floor(Math.log(x)/Math.log(I));return parseFloat((x/Math.pow(I,k)).toFixed(1))+" "+P[k]}const _=h.type==="file"?S(h.size):"";return h.type==="folder"?html`
          <div
            class="item ${y?"selected":""}"
            onclick=${x=>r("toggle-file-selection",h,n,x)}
            ondblclick=${b}
            >
            ${IconSprite.renderIcon("folder",{className:"icon"})}
        ...
        `}const u=o[`${n}Files`].sort((h,m)=>{const v=h.fileName.toUpperCase(),y=m.fileName.toUpperCase();if(h.type==="folder"&&m.type==="file")return-1;if(h.type===m.type){if(v<y)return-1;if(v>y)return 1}return 0}),d=html`<div class="item"
  ...
  `}function LanguageSelector(n,i){const o=window.html||(()=>{}),r=n.locale||"en",s=window.i18n?window.i18n.getAvailableLocales():["en","de","es","fr"],a={en:"English",de:"Deutsch",es:"Español",fr:"Français"},c=window.i18n?window.i18n.t("language"):"Language";return o`
    <div class="language-selector">
      ...
      <select
        ...
        onchange=${u=>{const d=u.target.value;i("change-locale",d)}}
      >
        ${s.map(u=>o`
          <option value=${u} selected=${u===r}>
            ${a[u]||u}
          </option>
        `)}
      </select>
    ...
    <div class="panel-section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <span class="info-value info-mono">${"99d39be"}</span>
        </div>
        <div class="info-item">
  ...
  `}function SysInfoPanel(n,i){const o=window.i18n?window.i18n.t:s=>s;if(!n.systemInfo&&n.isConnected&&!n.isLoadingSystemInfo&&!n.systemInfoAttempted&&(n.systemInfoAttempted=!0,i("refresh-system-info")),!n.systemInfo)return html`
      <div class="panel-container">
  ...
  `:""}function EthernetPanel(n,i){if(!n.networksInfo&&n.isConnected&&!n.isLoadingNetworks&&i("refresh-networks"),n.isConnected&&!n.ethConfigLoaded&&!n.isLoadingEthConfig&&i("load-eth-config"),n.isConnected&&!n.networkInterfacesConfig&&!n.isLoadingNetworkInterfacesConfig&&i("load-network-interfaces-config"),!n.networksInfo)return html`
    ...
    `;const o=n.networksInfo.eth,r=n.ethStatus,s=r&&r.initialized?r:o&&o.mac?o:r||o,a=n.ethConfig||{},c=o!==null,l=n.networkInterfacesConfig?.ethernet===!1,u=s&&(s.mac||s.enabled||s.enable||s.initialized),d=s&&s.ip&&s.ip!=="0.0.0.0",p=s&&s.linkup===!0,f=d;if(!c)return html`
      <div class="panel-container">
        <div class="panel-header">
          <h2>Ethernet Configuration</h2>
        </div>
    ...
    `;const h=l?"status-disabled":f?"status-active":p?"status-warning":"status-inactive",m=l?"Disabled":f?"Connected":p?"Link Up (No IP)":u?"No Link":"Not Initialized";return html`
    <div class="panel-container">
      <div class="panel-header">
        <h2>Ethernet Configuration</h2>
      </div>
  ...
  `}function VPNPanel(n,i){n.isConnected&&!n.vpnConfigLoaded&&!n.isLoadingVpnConfig&&i("load-vpn-config");const o=n.vpnConfig||{hostname:"",join_code:"",auto_connect:!1},r=n.networksInfo?.vpn,s=n.isConnected,a=n.networksInfo&&r!==void 0;!a||r.available;const c=a&&r.active;return html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${l=>{l.preventDefault();const u=new FormData(l.target),d=u.get("hostname")||"",p=u.get("join_code")||"",f=u.get("auto_connect")==="on";if(!d.trim()){alert("Please enter a hostname for this device");return}if(!p.trim()){alert("Please enter a Husarnet join code");return}i("vpn-connect",{hostname:d.trim(),join_code:p.trim(),auto_connect:f})}}>
          <div class="form-group">
          ...
          <div class="config-actions">
            ...
            `:html`
              ...
              <button 
                ...
                disabled=${!s}
                onclick=${l=>{l.preventDefault();const u=l.target.closest("form"),d=new FormData(u);i("vpn-save-config",{hostname:d.get("hostname")||"",join_code:d.get("join_code")||"",auto_connect:d.get("auto_connect")==="on",enabled:!1})}}
              >
                Save Only
              </button>
            `}
  ...
  `:html`
    ...
    <div class="panel-container">
          ...
          <div class="toggle-row">
            <label class="toggle-switch ${a.connected?"ppp-active":a.connecting?"ppp-connecting":""}">
              <input 
                ...
                onchange=${u=>{u.target.checked?i("enable-mobile-data"):i("disable-mobile-data")}}
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${u=>{u.preventDefault();const d=new FormData(u.target),p={apn:d.get("apn")||"",username:d.get("username")||"",password:d.get("password")||"",auto_init_modem:d.get("auto_init_modem")==="on",mobile_data_enabled:r.mobile_data_enabled||!1};i("save-wwan-config",p)}}>
          <div class="form-group">
            <label for="wwan-apn">
              GPRS APN
              <span class="label-tooltip">
  ...
  `}function NTPPanel(n,i){n.isConnected&&!n.ntpConfigLoaded&&!n.isLoadingNtpConfig&&i("load-ntp-config");const o=n.ntpConfig||{server:"pool.ntp.org",tzOffset:0,timezone:"UTC",autoDetect:!1},r=n.ntpSyncResult||null,s=d=>{if(!d)return"--:--:--";const{year:p,month:f,day:h,hour:m,minute:v,second:y}=d;return`${String(m).padStart(2,"0")}:${String(v).padStart(2,"0")}:${String(y).padStart(2,"0")}`},a=d=>{if(!d)return"";const{year:p,month:f,day:h}=d;return`${p}-${String(f).padStart(2,"0")}-${String(h).padStart(2,"0")}`},c=[{value:"UTC",offset:0,label:"UTC (Coordinated Universal Time)"},{value:"EST",offset:-5,label:"EST (Eastern Standard Time)"},{value:"CST",offset:-6,label:"CST (Central Standard Time)"},{value:"MST",offset:-7,label:"MST (Mountain Standard Time)"},{value:"PST",offset:-8,label:"PST (Pacific Standard Time)"},{value:"GMT",offset:0,label:"GMT (Greenwich Mean Time)"},{value:"CET",offset:1,label:"CET (Central European Time)"},{value:"EET",offset:2,label:"EET (Eastern European Time)"},{value:"JST",offset:9,label:"JST (Japan Standard Time)"},{value:"AEST",offset:10,label:"AEST (Australian Eastern Standard Time)"}],l=o.timezone,u=[...c];return l&&!c.find(d=>d.value===l)&&u.unshift({value:l,offset:o.tzOffset||0,label:`${l} (Detected)`}),html`
    <div class="panel-container">
      <div class="panel-header">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${async d=>{d.preventDefault();const p=new FormData(d.target),f=p.get("server")||"pool.ntp.org",h=p.get("timezone")||"UTC",m=p.get("auto_detect")==="on",v=u.find(b=>b.value===h),y=v?v.offset:0,w={server:f,tz_offset:y,timezone:h,auto_detect:m};try{await new Promise((b,C)=>{const E=setTimeout(()=>C(new Error("Save timeout")),1e4),$=()=>{clearTimeout(E),window.appInstance.emitter.removeListener("ntp-config-saved",S),window.appInstance.emitter.removeListener("ntp-config-save-error",_)},S=()=>{$(),b()},_=x=>{$(),C(x)};window.appInstance.emitter.once("ntp-config-saved",S),window.appInstance.emitter.once("ntp-config-save-error",_),i("save-ntp-config",w)}),i("sync-ntp-time",f,y,m)}catch(b){console.error("[NTP] Failed to save config before sync:",b),alert(`Failed to save NTP configuration: ${b.message}`)}}}>
          <div class="form-group">
          ...
          <div class="signal-display">
            <div class="signal-bars">
              ${[1,2,3,4,5].map(h=>html`
                <div class="signal-bar ${h<=l?"active":""}" style="--bar-color: ${d}"></div>
              `)}
            </div>
            <div class="signal-info">
              ${c!=null&&c!==-999?html`
                <span class="signal-dbm" style="color: ${d}">${c} dBm</span>
                <span class="signal-quality">${u}</span>
              `:html`
  ...
  `}function formatBytes(n){if(!n||n===0)return"0 B";const i=1024,o=["B","KB","MB","GB"],r=Math.floor(Math.log(n)/Math.log(i));return Math.round(n/Math.pow(i,r)*100)/100+" "+o[r]}const REGISTRY_BASE="/boards",CHIP_VARIANT_MAP={"ESP32-S3":{base:"ESP32_S3_8",variants:["ESP32_S3_8","ESP32_S3_8_OCT","ESP32_S3_16"]},"ESP32-P4":{base:"ESP32_P4_16",variants:["ESP32_P4_16","ESP32_P4_32"]}};async function fetchFirmwareInfo(){try{const n=await fetch(`${REGISTRY_BASE}/firmware/latest.json`);if(!n.ok)throw new Error(`Registry error: ${n.status}`);return await n.json()}catch(n){throw console.error("[firmware-flasher] Failed to fetch firmware info:",n),n}}async function selectFirmwareVariant(n,i=0,o=null){const r=CHIP_VARIANT_MAP[n];if(!r)return console.warn(`[firmware-flasher] No firmware mapping for chip: ${n}`),null;const s=typeof i=="string"?parseInt(i):i;let a=r.base;n==="ESP32-S3"?s>=16?a="ESP32_S3_16":o==="octal"&&(a="ESP32_S3_8_OCT"):n==="ESP32-P4"&&s>=32&&(a="ESP32_P4_32");try{const l=await fetchFirmwareInfo(),u=(l.variants||[]).find(d=>d.id===a);if(u&&u.url)return{id:u.id,file:u.file,url:u.url,displayName:u.id.replace(/_/g," "),version:l.version}}catch(l){console.warn("[firmware-flasher] Could not fetch latest.json, using fallback URL:",l)}const c=`${a}-merged.bin`;return{id:a,file:c,url:`${REGISTRY_BASE}/firmware/${c}`,displayName:a.replace(/_/g," ")}}async function fetchBoardManifests(n=null){try{const i=await fetch(`${REGISTRY_BASE}/manifests/index.json`);if(!i.ok)throw new Error(`Registry error: ${i.status}`);let r=(await i.json()).boards||[];return n&&(r=r.filter(s=>s.chip===n)),r}catch(i){throw console.error("[firmware-flasher] Failed to fetch board manifests:",i),i}}async function fetchBoardManifest(n){try{const i=await fetch(`${REGISTRY_BASE}/manifests/${n}.json`);if(!i.ok)throw new Error(`Manifest not found: ${n}`);return await i.json()}catch(i){throw console.error(`[firmware-flasher] Failed to fetch manifest for ${n}:`,i),i}}async function downloadFirmware(n,i=()=>{},o=0){try{const r=await fetch(n);if(!r.ok){const f=n.match(/([A-Z0-9_]+)-merged\.bin/),h=f?f[1]:"requested";throw r.status===404||r.status===503?new Error(`Firmware "${h}" is not available yet. The build may still be in progress — please try again in a few minutes.`):new Error(`Failed to download firmware: HTTP ${r.status}`)}const s=r.headers.get("content-length"),a=s?parseInt(s,10):0;if(!r.body){const f=await r.arrayBuffer();return i(100),f}const c=r.body.getReader(),l=[];let u=0;for(;;){const{done:f,value:h}=await c.read();if(f)break;l.push(h),u+=h.length;const m=o||a;m>0?i(Math.round(u/m*100)):i(Math.round(10+u/1e6%80))}const d=new Uint8Array(u);let p=0;for(const f of l)d.set(f,p),p+=f.length;return console.log(`[firmware-flasher] Downloaded ${(u/1024/1024).toFixed(1)} MB`),i(100),d.buffer}catch(r){throw console.error("[firmware-flasher] Download error:",r),r}}function showStyledModal({variant:n="",icon:i,title:o,subtitle:r,body:s,buttons:a}){const c=l=>l?l.includes("<")||/[\u{1F300}-\u{1F9FF}]/u.test(l)?l:`<svg class="icon icon-tabler" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><use href="#tabler-${l}"></use></svg>`:"";return new Promise(l=>{const u=document.createElement("div");u.className="fw-modal-overlay active",u.innerHTML=`
...
`)).match(/CREDS:(\{.*\})/);if(o){const r=JSON.parse(o[1]);panelState.credentials={hostname:r.h+".local",password:r.p,ssid:r.s},addLog(`Found: ${r.h}.local`,"success")}else throw new Error("Could not read settings - device may need firmware");triggerRerender()}catch(n){addLog(`Error: ${n.message}`,"error"),triggerRerender()}}async function performWifiSetup(){clearLog();try{let l=function(u){return new Promise((d,p)=>{const f=document.createElement("div");f.className="fw-modal-overlay active",f.innerHTML=`
        ...
        `,document.body.appendChild(f);const h=y=>{const w=y.target.closest(".fw-modal-network-item");if(w){const b=parseInt(w.dataset.index);v(),d(b)}},m=()=>{v(),p(new Error("Setup cancelled"))},v=()=>{f.remove()};f.querySelector(".fw-network-modal-list").addEventListener("click",h),f.querySelector(".fw-modal-cancel").addEventListener("click",m)})};addLog("Connecting to REPL...");const n=await transitionToREPL();if(!n.success){await handleBootFailure(n.error);return}addLog("Scanning for WiFi networks...");const o=(await sendREPLCommand(currentPort,`
import network
import json
...
`)).match(/JSON:(\[.*\])/);if(!o)throw new Error("Failed to parse network scan - no JSON found");let r;try{r=JSON.parse(o[1])}catch(u){throw new Error("Failed to parse network JSON: "+u.message)}if(r.length===0)throw new Error("No WiFi networks found");addLog(`Found ${r.length} networks`);let s=!1,a=null,c=null;for(;!s;){const u=await l(r),d=r[u],p=prompt(`Enter password for "${d.ssid}":`);if(p===null)continue;addLog(`Connecting to "${d.ssid}"...`);const h=(await sendREPLCommand(currentPort,`
import time
try:
    wlan.disconnect()
except:
...
`,15e3,!1)).match(/CONNECTED:(\d+\.\d+\.\d+\.\d+)/);if(h){a=h[1],s=!0,addLog(`Connected! IP: ${a}`),addLog("Saving settings...");const v=`pybot-${panelState.deviceInfo?.mac?panelState.deviceInfo.mac.replace(/:/g,"").slice(-4).toLowerCase():"xxxx"}`;if(await sendREPLCommand(currentPort,`
from lib.sys import settings
...
Click OK to try again, or Cancel to abort.`))throw new Error("Connection cancelled")}}catch(n){addLog(`Error: ${n.message}`,"error"),triggerRerender()}}async function continueToProvision(n){try{const i=n+".local";addLog("Generating HTTPS certificate..."),window.forge||await new Promise((l,u)=>{const d=document.createElement("script");d.src="https://cdn.jsdelivr.net/npm/node-forge@1.3.1/dist/forge.min.js",d.onload=l,d.onerror=()=>u(new Error("Failed to load forge")),document.head.appendChild(d)}),addLog(`Hostname: ${i}`);const{certPem:o,keyPem:r}=generateSelfSignedCert(i);if(addLog(`Certificate generated (${o.length} bytes)`),addLog("Installing certificate..."),await uploadCertsToDevice(currentPort,o,r),panelState.selectedBoard)try{addLog(`Installing board config: ${panelState.selectedBoard.name}...`);const l=await fetchBoardManifest(panelState.selectedBoard.id),u=JSON.stringify(l).replace(/\\/g,"\\\\").replace(/'/g,"\\'");await sendREPLCommand(currentPort,`
import json
try:
    import os
    os.mkdir('/settings')
...
`,5e3,!1),addLog("Certificates installed!")}function FirmwarePanel(n,i){appState=n;const o=panelState.view;return html$1`
  ...
  `}function renderNewDeviceView(n,i){const o=panelState.flashStep,r=panelState.flashProgress,s=panelState.selectedFirmware;if(o==="complete")return html$1`
      <div class="fw-view active">
        ...
        <div class="fw-device-card">
          ...
          <div style="text-align: center; padding: 2rem 0;">
            ...
            ${s?.version?html$1`<p style="color: var(--brand); font-size: 0.85rem; font-weight: 500; margin-bottom: 0.25rem;">pyDirect ${s.version}</p>`:""}
    ...
    `;if(o==="downloading"||o==="flashing"){const a=t(o==="downloading"?"firmware.downloading":"firmware.flashing"),c=o==="flashing"&&r>90?html$1`<p style="text-align: center; color: var(--text-muted); margin-top: 0.5rem; font-size: 0.8rem;">
        ...
        <div class="fw-device-card">
          ...
          <div style="padding: 1.5rem 0;">
            <div class="fw-progress-bar">
              <div class="fw-progress-fill" style="width: ${r}%"></div>
            </div>
        ...
        ${s?html$1`
            ...
            <div class="fw-form-group">
              <select class="fw-select" onchange=${a=>{const c=panelState.firmwareOptions.find(l=>l.id===a.target.value);c&&(panelState.selectedFirmware=c,triggerRerender())}}>
                ${panelState.firmwareOptions.map(a=>html$1`
                  <option value="${a.id}" selected=${a.id===s.id}>
                    ${a.displayName}
                  </option>
                `)}
              </select>
          ...
          `:html$1`
            <div class="fw-firmware-info">
              <p style="font-weight: 500; margin-bottom: 0.25rem;">${s.displayName} <span style="font-size: 0.8rem; color: var(--text-secondary);">
                ${s.version?` ( Based on MicroPython ${s.version} )`:""}
                ${s.version&&s.size?" · ":""}
                ${s.size?`${(s.size/1024/1024).toFixed(1)} MB`:s.version?"":"Ready to flash"}
              </span></p>
            </div>
          `}
  ...
  `)}function parseSkillMd(n){const i={meta:{},body:n},o=n.match(/^---\s*\n([\s\S]*?)\n---\s*\n?/);if(o){const r=o[1];for(const s of r.split(`
`)){const a=s.match(/^(\w+)\s*:\s*(.+)$/);a&&(i.meta[a[1].trim()]=a[2].trim())}i.body=n.slice(o[0].length)}return i.body=i.body.replace(/^ScriptO for\s+\[.*?\]\(.*?\)\.?\s*Run on device\s*\(MicroPython\)\.?\s*\n?/im,""),i.body=i.body.trim(),i}function SkillSidebar(n,i){if(!n.isSkillSidebarOpen||!n.skillSidebarContent)return html`<div></div>`;const{name:o,skillMd:r}=n.skillSidebarContent,{meta:s,body:a}=parseSkillMd(r);let c="";try{a&&(c=g.parse(a,{breaks:!0}))}catch(f){console.error("[SkillSidebar] Failed to parse markdown:",f),c=`<pre>${a}</pre>`}const l=[];s.description&&l.push(`<p class="skill-meta-description">${s.description}</p>`);const u=[];s.category&&u.push(`<span class="skill-meta-tag">${s.category}</span>`),s.author&&u.push(`<span class="skill-meta-tag">by ${s.author}</span>`),u.length&&l.push(`<div class="skill-meta-tags">${u.join("")}</div>`);const d=l.length?`<div class="skill-meta-block">${l.join("")}</div>`:"",p=document.createElement("div");return p.className="skill-sidebar-content skill-md-content",p.innerHTML=d+c,html`
    <div class="skill-sidebar">
      ...
      <div class="skill-sidebar-header">
        <h3>${o||s.name||"Skill Documentation"}</h3>
      </div>
  ...
  `}function ReconnectDialog(n,i){if(!n.isReconnecting)return;const o=n.reconnectAttempt||0;function r(){i("reconnect-cancel")}return html$1`
    ...
    <div class="dialog-content webrepl-dialog" style="max-width: 400px;">
      ...
      <div class="dialog-body">
        ...
        <div class="dialog-footer" style="justify-content: center; margin-top: 16px;">
           <button class="scriptos-btn scriptos-btn-secondary" style="min-width: 100px;" onclick=${r}>
             Cancel
           </button>
        </div>
      </div>
  ...
  `,o.querySelector(".iframe-modal-close").addEventListener("click",()=>o.remove()),o.addEventListener("click",r=>{r.target===o&&o.remove()}),document.addEventListener("keydown",function r(s){s.key==="Escape"&&(o.remove(),document.removeEventListener("keydown",r))}),document.body.appendChild(o)}function SystemView(n,i){n.sidebarIconRotated===void 0&&(n.sidebarIconRotated=!1);const o=n.connectionMode==="webrepl"?"#00FF7F":n.connectionMode==="usb"?"#FF9500":"var(--text-secondary)",r=html$1`
  ...
  `;return html$1`
    ...
    ${ExtensionsModal(n,i)}
    ${BreakpointModal(n,i)}
  `}function SystemSidebar(n,i){const o=window.i18n?window.i18n.t:S=>S,r=[{id:"home",label:o("sidebar.home"),icon:"home"}],s=[{id:"editor",label:o("sidebar.editor"),icon:"code"},{id:"file-manager",label:o("sidebar.files"),icon:"folder"}],a=[{id:"about",label:o("sidebar.about"),icon:"info-circle"},{id:"settings",label:o("sidebar.settings"),icon:"adjustments-alt"},{id:"ai-agent",label:o("sidebar.aiAgent"),icon:"robot-face"},{id:"firmware",label:o("sidebar.firmware"),icon:"file-download"}],c=[{id:"wifi",label:o("sidebar.wifi"),icon:"wifi"},{id:"ethernet",label:o("sidebar.ethernet"),icon:"cloud-network"},{id:"vpn",label:o("sidebar.vpn"),icon:"shield-chevron"},{id:"btle",label:o("sidebar.btle"),icon:"bluetooth"},{id:"wwan",label:o("sidebar.wwan"),icon:"cell"},{id:"mqtt",label:o("sidebar.mqtt"),icon:"message-2"},{id:"ntp",label:o("sidebar.ntp"),icon:"clock-cog"},{id:"can",label:o("sidebar.can"),icon:"car-crash"}],l=[{id:"gps",label:o("sidebar.gps"),icon:"gps"},{id:"4g-modem",label:o("sidebar.modem"),icon:"signal-4g"},{id:"sdcard",label:o("sidebar.sdcard"),icon:"device-sd-card"}],u=r.map(S=>renderSidebarItem(S,n,i)),d=s.map(S=>renderSidebarItem(S,n,i)),p=n.expandedSystem!==!1,f=html$1`
  ...
  `,w=S=>{if(S.iconSvg)try{const P=new DOMParser().parseFromString(S.iconSvg,"image/svg+xml").querySelector("svg");if(P){const k=P.getAttribute("viewBox")||"0 0 24 24",T=Array.from(P.querySelectorAll("path"));return html$1`
            <svg 
            ...
            >
              ${T.map(N=>{const O=N.getAttribute("d")||"",F=N.getAttribute("fill")||"none",D=N.getAttribute("stroke");return O==="M0 0h24v24H0z"||D==="none"?html$1`<path d="${O}" stroke="none" fill="none" />`:html$1`<path d="${O}" stroke="currentColor" fill="${F}" />`})}
            </svg>
          `}}catch(x){console.warn("[System] Failed to parse extension icon SVG:",x)}const _=S.icon||"settings";return IconSprite.renderIcon(_,{className:"",size:20})},b=(n.installedExtensions||[]).map(S=>{const _=n.expandedExtensions[S.id],x=n.activeExtension===S.id,I=S.devices===!0;let P=null;if(I&&_)if(!n.loadedExtensions[S.id])console.warn("[System] Extension not loaded:",S.id);else{const k=n.loadedExtensions[S.id];if(!k.instance)try{const T=k.data.content,O=new Function("DeviceAPI","html","emit","state",`
              ${T}
            ...
            `)(DeviceAPI,html$1,i,n),F=new DeviceAPI(BridgeDevice),D=new O(F,i,n,html$1);n.loadedExtensions[S.id].instance=D}catch(T){console.error(`[System] Error instantiating extension ${S.id}:`,T)}if(k.instance&&typeof k.instance.getMenuItems=="function")try{const T=k.instance.getMenuItems();T&&Array.isArray(T)&&(S.menu=T)}catch(T){console.error(`[System] Error getting menu items for ${S.id}:`,T)}if(k.instance&&typeof k.instance.renderSidebarDevices=="function")try{P=k.instance.renderSidebarDevices()}catch(T){console.error(`[System] Error rendering sidebar devices for ${S.id}:`,T)}else console.warn(`[System] renderSidebarDevices not available for ${S.id}`)}if(_&&n.loadedExtensions[S.id]?.instance){const k=n.loadedExtensions[S.id];if(k.instance&&typeof k.instance.getMenuItems=="function")try{const T=k.instance.getMenuItems();T&&Array.isArray(T)&&(S.menu=T)}catch(T){console.error(`[System] Error getting menu items for ${S.id}:`,T)}}return html$1`
      <div class="system-sidebar-extension">
        <div 
        ...
        ${_?html$1`
          <div class="system-sidebar-submenu">
            ${S.menu.map(k=>{const T=n.activeExtension===S.id&&n.activeExtensionPanel===k.id,N=k.disabled===!0;return html$1`
                <div 
                  class="system-sidebar-subitem ${T?"active":""} ${N?"disabled":""}"
                  onclick=${N?null:()=>i("change-extension-panel",{extensionId:S.id,panelId:k.id})}
...
print('Hello, ')
print('ScriptO!') # ●
`;async function sleep(n){return new Promise(i=>setTimeout(i,n))}async function confirmDialog(n,i,o){return confirm(n)?0:1}function updateStatusBarDirectly(n,i){const o=document.getElementById("status-bar");if(!o)return;const r=i||localStorage.getItem("temperatureUnit")||"degC",s=buildStatusBarModel?buildStatusBarModel(n,r):null;if(!s||!s.connected){o.className="disconnected";const f=o.querySelector(".status-bar-center");f?f.textContent=s&&s.disconnectedText||DISCONNECTED_STATUS_TEXT:o.textContent=s&&s.disconnectedText||DISCONNECTED_STATUS_TEXT;return}o.className="";const a=o.querySelector(".status-bar-center")||o;function c(f){let h=a.querySelector(`.status-item.${f}`);if(!h){h=document.createElement("div"),h.className=`status-item ${f}`;const m=document.createElement("span");m.className="status-label";const v=document.createElement("span");v.className="status-value",h.appendChild(m),h.appendChild(v),a.appendChild(h)}return h}const l=c("ram");l.querySelector(".status-label").textContent="RAM",l.querySelector(".status-value").textContent=s.ram;const u=a.querySelector(".status-item.temp");if(s.temp){const f=c("temp");f.querySelector(".status-label").textContent="TEMP",f.querySelector(".status-value").textContent=s.temp}else u&&u.remove();const d=c("uptime");d.querySelector(".status-label").textContent="UPTIME",d.querySelector(".status-value").textContent=s.uptime;const p=a.querySelector(".status-item.wifi-rssi");if(s.rssi){const f=c("wifi-rssi");f.querySelector(".status-label").textContent="RSSI",f.querySelector(".status-value").textContent=s.rssi}else p&&p.remove()}function updateOverlayDirectly(n){const i=document.getElementById("overlay");if(!i)return;let o=!1,r="";if(n.diskFiles==null)o=!0,r="<p>Loading files...</p>";else if(n.isRemoving)o=!0,r="<p>Removing...</p>";else if(n.isConnecting)o=!0,r="<p>Connecting...</p>";else if(n.isLoadingFiles)o=!0,r="<p>Loading files...</p>";else if(n.isSaving)o=!0,r=`<p>Saving file... ${n.savingProgress||0}</p>`;else if(n.isTransferring){o=!0;const s=String(n.transferringProgress||""),a=s.match(/(\d+)%?$/),c=a?parseInt(a[1]):0,l=s.match(/^(.+?):/);r=`
      <div class="transfer-overlay-content">
        <div class="transfer-title">Transferring File</div>
...
`,l=await o.exec(c);if(l&&l.success)n.ntpConfig={server:s.server||"pool.ntp.org",tzOffset:s.tz_offset??s.tzOffset??0,timezone:s.timezone||"UTC",autoDetect:s.auto_detect??s.autoDetect??!1},i.emit("render"),i.emit("ntp-config-saved");else{const u=new Error(l?.error||"Save failed");throw i.emit("ntp-config-save-error",u),u}}catch(a){throw console.error("[NTP] Failed to save config:",a),alert(`Failed to save NTP configuration: ${a.message}`),i.emit("ntp-config-save-error",a),a}}),i.on("sync-ntp-time",async(s,a,c)=>{if(r("sync-ntp-time",s,a,c),!n.isConnected){console.warn("[NTP] Not connected to device");return}try{const u=`
from lib.sys.utils import sync_ntp
sync_ntp('${s}', ${a}, ${c?"True":"False"}, force=True)
`,d=await o.exec(u);if(o.onNtpSync){const p={autoDetect:c,server:s,tzOffset:a};o.onNtpSync(d,p)}}catch(l){console.error("[NTP] Failed to sync time:",l),alert(`Failed to sync NTP time: ${l.message}`)}}),i.on("load-mqtt-config",async()=>{if(r("load-mqtt-config"),!n.isConnected){console.warn("[MQTT] Not connected to device");return}if(n.isLoadingMqttConfig){console.log("[MQTT] Already loading config, skipping");return}try{n.isLoadingMqttConfig=!0;const a=await o.exec(`
from lib.sys import settings
import json
...
`),p=typeof d=="object"?d:JSON.parse(d.trim());n.boardConfig={board_id:p.board_id,board_name:p.board_name,chip:p.chip,version:p.version,description:p.description,hardware:p.hardware};try{const h=new URL(a).hostname,m=p.board_id||h.replace(/\\./g,"-");await BridgeDisk.addOnboardedDevice(m,{hostname:h,url:a,board_id:p.board_id,board_name:p.board_name,chip:p.chip,version:p.version,connectedAt:new Date().toISOString()}),console.log("[Connection] Device saved to /onboarded/"),n.needsOnboarding=!1}catch(f){console.warn("[Connection] Could not save device to /onboarded/:",f)}i.emit("render")}catch(d){console.error("[BoardConfig] Failed to load board config:",d)}try{const d=(await __vitePreload(async()=>{const{DeviceAPI:m}=await Promise.resolve().then(()=>deviceApi);return{DeviceAPI:m}},void 0)).DeviceAPI,p=new d(BridgeDevice),f=await readDeviceExtensionRegistry(p),h=new Set(Object.keys(f));n.installedExtensions=n.allCachedExtensions.filter(m=>h.has(m.id)),n.loadedExtensions={},console.log(`[Extensions] Device registry has ${h.size} extensions, showing: ${n.installedExtensions.map(m=>m.id).join(", ")||"(none)"}`),i.emit("render")}catch(d){console.warn("[Extensions] Could not read device extension registry:",d),n.installedExtensions=[],i.emit("render")}})(),i.emit("render")}catch(u){let d="Connection failed";u instanceof Error||u&&u.message?d=u.message:u&&u.type==="error"&&(d=`WebSocket connection failed. Check if device is available at ${a}`),console.log("✗ Connection failed:",d),n.isConnecting=!1,n.isConnected=!1,updateOverlayDirectly(n),i.emit("render")}}),i.on("open-reset-dialog",()=>{n.isResetDialogOpen=!0,i.emit("render")}),i.on("close-reset-dialog",()=>{n.isResetDialogOpen=!1,i.emit("render")}),i.on("trigger-reset",async a=>{n.isResetDialogOpen=!1,i.emit("render");try{a===1?(console.log("[Connection] Triggering Hard Reset"),n.isResettingHard=!0,await o.reset(1),console.log("[Connection] Hard reset sent, waiting for disconnect...")):(console.log("[Connection] Triggering Soft Reset"),await o.reset(0))}catch(c){console.error("[Connection] Reset failed:",c),n.isResettingHard=!1,alert("Reset failed: "+c.message)}}),i.on("bind-terminal-input",()=>{const a=n.cache(XTerm,"terminal");!a||!a.term||a.bindInput(n,o)&&console.log("[Connection] Bound terminal input after view change")}),i.on("terminal-focus",async()=>{if(!n.isConnected||n.bannerDisplayed)return;const a=n.cache(XTerm,"terminal");if(!a||!a.term)return;const c=a.term;try{const l=await o.exec(`
import os, json
u = os.uname()
print(json.dumps({"version": u.version, "machine": u.machine}))
`);if(l){const u=`MicroPython ${l.version} on ${l.machine}`;c.write(`\r
\x1B[1;32m`+u+`\x1B[0m\r
`),c.write(`\x1B[0mType "help()" for more information.\r
`)}}catch(l){console.debug("[Connection] Failed to fetch version info:",l.message)}c.write(TERMINAL_PROMPT),c.scrollToBottom(),n.bannerDisplayed=!0,console.log("[Connection] Welcome banner displayed")})}function registerFileOperationHandlers(n,i,o,r,s,a,c){i.on("save",async()=>{if(console.log("save"),canSave({isConnected:n.isConnected,openFiles:n.openFiles,editingFile:n.editingFile})==!1){console.log("can't save");return}n.isSaving=!0,updateOverlayDirectly(n);let u=n.openFiles.find(v=>v.id===n.editingFile),d=!1;const p=u.parentFolder,f=p===null;f&&(u.source=="board"?u.parentFolder=n.boardNavigationPath:u.source=="disk"&&(u.parentFolder=n.diskNavigationPath));let h=!1;if(u.source=="board"?h=await fileExists(BridgeDevice,getFullPath(n.boardNavigationRoot,u.parentFolder,u.fileName)):u.source=="disk"&&(h=await r.fileExists(r.getFullPath(n.diskNavigationRoot,u.parentFolder,u.fileName))),(f||!h)&&(u.source=="board"?(u.parentFolder=n.boardNavigationPath,d=await fileExists(BridgeDevice,getFullPath(n.boardNavigationRoot,u.parentFolder,u.fileName))):u.source=="disk"&&(u.parentFolder=n.diskNavigationPath,d=await r.fileExists(r.getFullPath(n.diskNavigationRoot,u.parentFolder,u.fileName)))),d&&await confirmDialog(`You are about to overwrite the file ${u.fileName} on your ${u.source}.

 Are you sure you want to proceed?`)!==0){n.isSaving=!1,u.parentFolder=p,updateOverlayDirectly(n),i.emit("render");return}let m=u.editor.content||"";if(u.fileName&&u.fileName.toLowerCase().endsWith(".json"))try{const v=m.trim();if(v&&(v[0]==="{"||v[0]==="[")){const y=JSON.parse(v),w=JSON.stringify(y,null,2)+`
`;w!==m&&(m=w)}}catch{}try{if(u.source=="board"){if(["main.py","boot.py"].includes(u.fileName)&&await confirmDialog(`⚠️ Warning: Saving '${u.fileName}' to device may cause disconnection.
...
Continue saving to device?`,"Cancel","OK")!==0){n.isSaving=!1,updateOverlayDirectly(n),i.emit("render");return}await o.saveFile(getFullPath(n.boardNavigationRoot,u.parentFolder,u.fileName),m,{progressCallback:y=>{n.savingProgress=y,i.emit("render")}})}else u.source=="disk"&&await r.saveFileContent(r.getFullPath(n.diskNavigationRoot,u.parentFolder,u.fileName),m)}catch(v){console.log("error",v)}u.hasChanges=!1,n.isSaving=!1,n.savingProgress=0,updateOverlayDirectly(n),i.emit("refresh-files"),i.emit("render")}),i.on("select-tab",l=>{if(console.log("select-tab",l),!n.openFiles.find(d=>d.id===l)){console.warn("[select-tab] Tab not found:",l);return}n.editingFile=l,i.emit("render")}),i.on("close-tab",async l=>{console.log("close-tab",l);const u=n.openFiles.find(d=>d.id===l);if(!u){console.warn("[close-tab] Tab not found:",l);return}if(u.hasChanges&&await confirmDialog("Your file has unsaved changes. Are you sure you want to proceed?")!==0)return!1;n.openFiles=n.openFiles.filter(d=>d.id!==l),n.openFiles.length>0?n.editingFile=n.openFiles[0].id:await a("disk"),i.emit("render")}),i.on("refresh-board-files",async()=>{if(console.log("refresh-board-files"),n.isConnected)try{n.boardFiles=await getBoardFiles(getFullPath(n.boardNavigationRoot,n.boardNavigationPath,""))}catch{n.boardFiles=[]}else n.boardFiles=[];i.emit("refresh-selected-files"),i.emit("render")}),i.on("refresh-disk-files",async()=>{console.log("refresh-disk-files");try{n.diskFiles=await getDiskFiles(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,""))}catch(l){console.error("[FS] Error refreshing disk files:",l),n.diskNavigationRoot=null,n.diskNavigationPath="/"}i.emit("refresh-selected-files"),i.emit("render")}),i.on("refresh-files",async()=>{if(console.log("refresh-files"),n.isLoadingFiles)return;n.isLoadingFiles=!0;const l=document.getElementById("overlay");l?(l.classList.remove("closed"),l.classList.add("open"),l.innerHTML="<p>Loading files...</p>"):i.emit("render");try{await Promise.all([(async()=>{if(n.isConnected)try{n.boardFiles=await getBoardFiles(getFullPath(n.boardNavigationRoot,n.boardNavigationPath,""))}catch{n.boardFiles=[]}else n.boardFiles=[]})(),(async()=>{try{n.diskFiles=await getDiskFiles(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,""))}catch(u){console.error("[FS] Error refreshing files:",u),n.diskNavigationRoot=null,n.diskNavigationPath="/"}})()])}finally{n.isLoadingFiles=!1,i.emit("refresh-selected-files");const u=document.getElementById("overlay");u&&(u.classList.remove("open"),u.classList.add("closed")),n.systemSection==="file-manager"&&i.emit("render")}}),i.on("refresh-selected-files",()=>{n.selectedFiles=n.selectedFiles.filter(l=>l.source==="board"?n.isConnected?n.boardFiles.find(u=>l.fileName===u.fileName):!1:n.diskFiles.find(u=>l.fileName===u.fileName)),i.emit("render")}),i.on("create-new-tab",async(l,u=null)=>{const d=l=="board"?n.boardNavigationPath:n.diskNavigationPath;console.log("create-new-tab",l,u,d),await a(l,u,d)&&(i.emit("close-new-file-dialog"),i.emit("render"))}),i.on("create-file",(l,u=null)=>{console.log("create-file",l),n.creatingFile===null&&(n.creatingFile=l,n.creatingFolder=null,u!=null&&i.emit("finish-creating-file",u),i.emit("render"))}),i.on("finish-creating-file",async l=>{if(console.log("finish-creating",l),!n.creatingFile)return;if(!l){n.creatingFile=null,i.emit("render");return}if(n.creatingFile=="board"&&n.isConnected){if(await checkBoardFile({root:n.boardNavigationRoot,parentFolder:n.boardNavigationPath,fileName:l})&&await confirmDialog(`You are about to overwrite the file ${l} on your board.
...
Are you sure you want to proceed?`)!==0){n.creatingFolder=null,i.emit("render");return}await r.removeFolder(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,l))}await r.createFolder(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,l))}setTimeout(()=>{n.creatingFolder=null,i.emit("refresh-files"),i.emit("render")},200)}}),i.on("remove-files",async()=>{console.log("remove-files");let l=n.selectedFiles.filter(m=>m.source==="board").map(m=>m.fileName),u=n.selectedFiles.filter(m=>m.source==="disk").map(m=>m.fileName),d=`You are about to delete the following files:

`;if(l.length&&(d+=`From your board:
`,l.forEach(m=>d+=`${m}
`),d+=`
`),u.length&&(d+=`From your disk:
`,u.forEach(m=>d+=`${m}
`),d+=`
`),d+="Are you sure you want to proceed?",await confirmDialog(d)!==0){i.emit("render");return}let f=!1,h=!1;for(let m in n.selectedFiles){const v=n.selectedFiles[m];v.type=="folder"?v.source==="board"?(await removeBoardFolder(getFullPath(n.boardNavigationRoot,n.boardNavigationPath,v.fileName)),f=!0):(await r.removeFolder(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,v.fileName)),h=!0):v.source==="board"?(await deleteFile(o,getFullPath(n.boardNavigationRoot,n.boardNavigationPath,v.fileName)),f=!0):(await r.removeFile(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,v.fileName)),h=!0)}f&&h?i.emit("refresh-files"):f?i.emit("refresh-board-files"):h&&i.emit("refresh-disk-files"),n.selectedFiles=[],i.emit("render")}),i.on("rename-file",(l,u)=>{console.log("rename-file",l,u),n.renamingFile=l,i.emit("render")}),i.on("finish-renaming-file",async l=>{console.log("finish-renaming-file",l);const u=n.selectedFiles[0];if(!l||u.fileName==l){n.renamingFile=null,i.emit("render");return}if(n.isSaving=!0,updateOverlayDirectly(n),n.renamingFile=="board"&&n.isConnected){if((await checkOverwrite({fileNames:[l],parentPath:r.getFullPath(n.boardNavigationRoot,n.boardNavigationPath,""),source:"board"})).length>0){let p=`You are about to overwrite the following file/folder on your board:

`;if(p+=`${l}

`,p+="Are you sure you want to proceed?",await confirmDialog(p)!==0){n.isSaving=!1,n.renamingFile=null,updateOverlayDirectly(n),i.emit("render");return}u.type=="folder"?await removeBoardFolder(getFullPath(n.boardNavigationRoot,n.boardNavigationPath,l)):u.type=="file"&&await deleteFile(o,getFullPath(n.boardNavigationRoot,n.boardNavigationPath,l))}}else if(n.renamingFile=="disk"&&(await checkOverwrite({fileNames:[l],parentPath:r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,""),source:"disk"})).length>0){let p=`You are about to overwrite the following file/folder on your disk:

`;if(p+=`${l}

`,p+="Are you sure you want to proceed?",await confirmDialog(p)!==0){n.isSaving=!1,n.renamingFile=null,updateOverlayDirectly(n),i.emit("render");return}u.type=="folder"?await r.removeFolder(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,l)):u.type=="file"&&await r.removeFile(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,l))}try{n.renamingFile=="board"?await renameFile(o,getFullPath(n.boardNavigationRoot,n.boardNavigationPath,u.fileName),getFullPath(n.boardNavigationRoot,n.boardNavigationPath,l)):await r.renameFile(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,u.fileName),r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,l));const d=n.openFiles.findIndex(p=>p.fileName===u.fileName&&p.source===u.source&&p.parentFolder===u.parentFolder);d>-1&&(n.openFiles[d].fileName=l,i.emit("render"))}catch{alert(`The file ${u.fileName} could not be renamed to ${l}`)}n.isSaving=!1,n.renamingFile=null,updateOverlayDirectly(n),i.emit("refresh-files"),i.emit("render")}),i.on("rename-tab",l=>{console.log("rename-tab",l),n.renamingTab=l,i.emit("render")}),i.on("finish-renaming-tab",async l=>{console.log("finish-renaming-tab",l);const u=n.openFiles.find(v=>v.id===n.renamingTab);if(!l||u.fileName==l){n.renamingTab=null,n.isSaving=!1,updateOverlayDirectly(n),i.emit("render");return}n.isSaving=!0,updateOverlayDirectly(n);const d=u.parentFolder,p=u.fileName;u.fileName=l;const f=d===null;let h=!1;f||(u.source=="board"?h=await fileExists(BridgeDevice,getFullPath(n.boardNavigationRoot,u.parentFolder,p)):u.source=="disk"&&(h=await r.fileExists(r.getFullPath(n.diskNavigationRoot,u.parentFolder,p)))),(f||!h)&&(u.source=="board"?u.parentFolder=n.boardNavigationPath:u.source=="disk"&&(u.parentFolder=n.diskNavigationPath));let m=!1;if(u.source=="board"?m=await fileExists(BridgeDevice,getFullPath(n.boardNavigationRoot,u.parentFolder,u.fileName)):u.source=="disk"&&(m=await r.fileExists(r.getFullPath(n.diskNavigationRoot,u.parentFolder,u.fileName))),m&&await confirmDialog(`You are about to overwrite the file ${u.fileName} on your ${u.source}.

 Are you sure you want to proceed?`)!==0){n.renamingTab=null,n.isSaving=!1,u.fileName=p,i.emit("render");return}if(h){if(u.hasChanges){const v=u.editor.content||"";try{if(u.source=="board"){if(["main.py","boot.py"].includes(p)&&await confirmDialog(`⚠️ Warning: Saving '${p}' to device may cause disconnection.

This file is running on the device. Overwriting it may crash the connection.

Recommended: Save locally instead (to disk), then reconnect.

Continue saving to device?`,"Cancel","OK")!==0){n.renamingTab=null,n.isSaving=!1,u.fileName=p,updateOverlayDirectly(n),i.emit("render");return}await o.saveFile(getFullPath(n.boardNavigationRoot,u.parentFolder,p),v,{progressCallback:w=>{n.savingProgress=w,i.emit("render")}})}else u.source=="disk"&&await r.saveFileContent(r.getFullPath(n.diskNavigationRoot,u.parentFolder,p),v)}catch(y){console.log("error",y)}}try{u.source=="board"?await renameFile(o,getFullPath(n.boardNavigationRoot,u.parentFolder,p),getFullPath(n.boardNavigationRoot,u.parentFolder,u.fileName)):u.source=="disk"&&await r.renameFile(r.getFullPath(n.diskNavigationRoot,u.parentFolder,p),r.getFullPath(n.diskNavigationRoot,u.parentFolder,u.fileName))}catch(v){console.log("error",v)}}else if(!h){const v=u.editor.content||"";try{if(u.source=="board"){if(["main.py","boot.py"].includes(u.fileName)&&await confirmDialog(`⚠️ Warning: Saving '${u.fileName}' to device may cause disconnection.

This file is running on the device. Overwriting it may crash the connection.

Recommended: Save locally instead (to disk), then reconnect.

Continue saving to device?`,"Cancel","OK")!==0){n.renamingTab=null,n.isSaving=!1,u.fileName=p,updateOverlayDirectly(n),i.emit("render");return}await o.saveFile(getFullPath(n.boardNavigationRoot,u.parentFolder,u.fileName),v,{progressCallback:w=>{n.savingProgress=w,i.emit("render")}})}else u.source=="disk"&&await r.saveFileContent(r.getFullPath(n.diskNavigationRoot,u.parentFolder,u.fileName),v)}catch(y){console.log("error",y)}}u.hasChanges=!1,n.renamingTab=null,n.isSaving=!1,n.savingProgress=0,updateOverlayDirectly(n),i.emit("refresh-files"),i.emit("render")}),i.on("toggle-file-selection",(l,u,d)=>{console.log("toggle-file-selection",l,u,d);let p=u=="board"?n.boardNavigationPath:n.diskNavigationPath;if(d&&!d.ctrlKey&&!d.metaKey){n.selectedFiles=[{fileName:l.fileName,type:l.type,source:u,parentFolder:p}],i.emit("render");return}n.selectedFiles.find(h=>h.fileName===l.fileName&&h.source===u)?n.selectedFiles=n.selectedFiles.filter(h=>!(h.fileName===l.fileName&&h.source===u)):n.selectedFiles.push({fileName:l.fileName,type:l.type,source:u,parentFolder:p}),i.emit("render")}),i.on("open-selected-files",async()=>{console.log("open-selected-files");let l=[],u=[];if(!n.isLoadingFiles){n.isLoadingFiles=!0,i.emit("render");for(let d in n.selectedFiles){let p=n.selectedFiles[d];if(p.type=="folder")continue;const f=n.openFiles.find(h=>h.fileName==p.fileName&&h.source==p.source&&h.parentFolder==p.parentFolder);if(f)u.push(f);else{let h=null;if(p.source=="board"){const m=await o.loadFile(getFullPath(n.boardNavigationRoot,n.boardNavigationPath,p.fileName)),v=new Uint8Array(m),y=new TextDecoder("utf-8").decode(v);h=await s({parentFolder:n.boardNavigationPath,fileName:p.fileName,source:p.source,content:y}),h.editor.onChange=function(){h.hasChanges=!0,i.emit("render")}}else if(p.source=="disk"){const m=await r.loadFile(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,p.fileName)),v=new Uint8Array(m),y=new TextDecoder("utf-8").decode(v);h=await s({parentFolder:n.diskNavigationPath,fileName:p.fileName,source:p.source,content:y}),h.editor.onChange=function(){h.hasChanges=!0,i.emit("render")}}l.push(h)}}u.length>0&&(n.editingFile=u[0].id),l.length>0&&(n.editingFile=l[0].id),n.openFiles=n.openFiles.concat(l),n.selectedFiles=[],n.isLoadingFiles=!1,i.emit("change-view","editor"),i.emit("render")}}),i.on("open-file",(l,u)=>{console.log("open-file",l,u),n.selectedFiles=[{fileName:u.fileName,type:u.type,source:l,parentFolder:n[`${l}NavigationPath`]}],i.emit("open-selected-files")}),i.on("upload-files",async()=>{console.log("upload-files"),n.isTransferring=!0,i.emit("render");const l=await checkOverwrite({source:"board",fileNames:n.selectedFiles.map(u=>u.fileName),parentPath:getFullPath(n.boardNavigationRoot,n.boardNavigationPath,"")});if(l.length>0){let u=`You are about to overwrite the following files/folders on your board:

`;if(l.forEach(p=>u+=`${p.fileName}
`),u+=`
`,u+="Are you sure you want to proceed?",await confirmDialog(u)!==0){n.isTransferring=!1,i.emit("render");return}}try{for(let u in n.selectedFiles){const d=n.selectedFiles[u],p=r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,d.fileName),f=getFullPath(n.boardNavigationRoot,n.boardNavigationPath,d.fileName);if(d.type=="folder")await uploadFolder(p,f,(h,m)=>{n.transferringProgress=`${m}: ${h}`,i.emit("render")}),n.transferringProgress="";else{const h=await BridgeDisk.loadFile(p),m=new Uint8Array(h);await o.saveFile(f,m,{progressCallback:v=>{n.transferringProgress=`${d.fileName}: ${v}%`,i.emit("render")}}),n.transferringProgress=""}}n.selectedFiles=[],n.isTransferring=!1,n.transferringProgress="",updateOverlayDirectly(n),i.emit("refresh-files"),i.emit("render")}catch(u){console.error("[Upload] Transfer failed:",u),n.isTransferring=!1,n.transferringProgress="",updateOverlayDirectly(n),i.emit("render");let d=u.message;d&&d.includes("Transfer already in progress")&&(d="Transfer already in progress. The device may have stale TFTP state from a previous disconnected transfer. Please wait a moment and try again, or disconnect and reconnect."),alert(`Upload failed: ${d}`)}}),i.on("download-files",async()=>{console.log("download-files"),n.isTransferring=!0,updateOverlayDirectly(n),i.emit("render");const l=await checkOverwrite({source:"disk",fileNames:n.selectedFiles.map(u=>u.fileName),parentPath:r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,"")});if(l.length>0){let u=`You are about to overwrite the following files/folders on your disk:

`;if(l.forEach(p=>u+=`${p.fileName}
`),u+=`
`,u+="Are you sure you want to proceed?",await confirmDialog(u)!==0){n.isTransferring=!1,i.emit("render");return}}try{for(let u in n.selectedFiles){const d=n.selectedFiles[u],p=getFullPath(n.boardNavigationRoot,n.boardNavigationPath,d.fileName),f=r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,d.fileName);if(d.type=="folder")await downloadFolder(p,f,h=>{n.transferringProgress=h,i.emit("render")});else{const h=await o.loadFile(p,{progressCallback:m=>{n.transferringProgress=`${d.fileName}: ${m}%`,i.emit("render")}});await BridgeDisk.saveFileContent(f,h.buffer)}}n.isTransferring=!1,n.selectedFiles=[],updateOverlayDirectly(n),i.emit("refresh-files"),i.emit("render")}catch(u){console.error("[Download] Transfer failed:",u),n.isTransferring=!1,n.transferringProgress="",updateOverlayDirectly(n),i.emit("render");let d=u.message;d&&d.includes("Transfer already in progress")&&(d="Transfer already in progress. The device may have stale TFTP state from a previous disconnected transfer. Please wait a moment and try again, or disconnect and reconnect."),alert(`Download failed: ${d}`)}}),i.on("export-files",async()=>{console.log("export-files");for(const l of n.selectedFiles)if(l.type!=="folder")try{let u;if(l.source==="board"){const v=getFullPath(n.boardNavigationRoot,n.boardNavigationPath,l.fileName),y=await o.loadFile(v);u=new Uint8Array(y)}else{const v=r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,l.fileName),y=await r.loadFile(v);u=new Uint8Array(y)}const d=l.fileName.split(".").pop().toLowerCase(),p={png:"image/png",jpg:"image/jpeg",jpeg:"image/jpeg",webp:"image/webp",gif:"image/gif",py:"text/plain",txt:"text/plain",json:"application/json",md:"text/markdown"},f=new Blob([u],{type:p[d]||"application/octet-stream"}),h=URL.createObjectURL(f),m=document.createElement("a");m.href=h,m.download=l.fileName,document.body.appendChild(m),m.click(),document.body.removeChild(m),URL.revokeObjectURL(h)}catch(u){console.error(`[Export] Failed to export ${l.fileName}:`,u),alert(`Failed to export ${l.fileName}: ${u.message}`)}}),i.on("navigate-board-folder",l=>{console.log("navigate-board-folder",l),n.boardNavigationPath=getNavigationPath(n.boardNavigationPath,l),i.emit("refresh-files"),i.emit("render")}),i.on("navigate-board-parent",()=>{console.log("navigate-board-parent"),n.boardNavigationPath=getNavigationPath(n.boardNavigationPath,".."),i.emit("refresh-files"),i.emit("render")}),i.on("navigate-disk-folder",l=>{console.log("navigate-disk-folder",l),n.diskNavigationPath=r.getNavigationPath(n.diskNavigationPath,l),i.emit("refresh-files"),i.emit("render")}),i.on("navigate-disk-parent",()=>{console.log("navigate-disk-parent"),n.diskNavigationPath=r.getNavigationPath(n.diskNavigationPath,".."),i.emit("refresh-files"),i.emit("render")})}function debuggerStore(n,i){let o=null;console.log("[Debugger Store] Registering event handlers"),i.on("debugger:open-config",()=>{console.log("[Debugger] Opening config modal"),n.debugger.configOpen=!0,i.emit("render")}),i.on("debugger:close-config",()=>{n.debugger.configOpen=!1,i.emit("render")}),i.on("debugger:toggle-file",r=>{const s=n.debugger.debugFiles.indexOf(r);s>=0?n.debugger.debugFiles.splice(s,1):n.debugger.debugFiles.push(r),i.emit("render")}),i.on("debugger:set-watches",r=>{const s=r.split(`
...
${c.message}`)}}),i.on("close-extensions-modal",()=>{s("close-extensions-modal"),n.isExtensionsModalOpen=!1,i.emit("render")}),i.on("install-extension",async c=>{s("install-extension:",c.name);try{const l=await n.extensionRegistry.installExtension(c);n.installedExtensions=await n.extensionRegistry.getInstalledExtensions(),n.allCachedExtensions=[...n.installedExtensions],n.isExtensionsModalOpen=!1,s(`[Extensions] Installed ${c.name}`),i.emit("render");const u=await n.extensionRegistry.getExtension(c.id);hasOnInstallMethod(u?.content)?n.isConnected&&o?await a(c,u,n,i,o):(s("[Extensions] Device not connected - extension will install files when first opened"),await showStyledModal({variant:"warning",icon:"packages",title:"Extension Saved",subtitle:c.name,body:`<p>This extension needs to install files on your device.</p>
...
${l.message}`)}});async function a(c,l,u,d,p){const f=c.id,h=Array.isArray(c.version)?c.version.join("."):String(c.version);s(`[Extensions] Running onInstall for ${c.name}...`),showStyledModal({variant:"",icon:"packages",title:"Installing Extension",subtitle:c.name,body:`<p>Installing extension files to device...</p>
             <div style="margin-top: 16px; color: var(--text-secondary);">
               <span class="install-spinner">◐</span> Preparing...
             </div>`,buttons:[]});try{stopStatusInfoPolling();const m=(y,...w)=>y.reduce((b,C,E)=>b+C+(w[E]||""),""),v=instantiateExtension(l.content,u,d.emit.bind(d),m);if(typeof v.onInstall=="function"){updateModalBody(`<p>Writing files to device...</p>
          <div style="margin-top: 16px; color: var(--text-secondary);">
          ...
          </div>`);const y=new DeviceAPI$1(BridgeDevice);await updateDeviceExtensionRegistry(y,f,h),s(`[Extensions] onInstall complete for ${c.name}`)}closeStyledModal(),await showStyledModal({variant:"success",icon:"check",title:"Extension Ready",subtitle:c.name,body:"<p>Extension files have been installed on your device.</p>",buttons:[{id:"done",class:"fw-styled-modal-btn-primary",label:"Done"}]})}catch(m){console.error("[Extensions] onInstall failed:",m),closeStyledModal(),await showStyledModal({variant:"danger",icon:"alert-triangle",title:"Installation Failed",subtitle:c.name,body:`<p>Failed to install extension files:</p>
               <p style="color: var(--error-color)">${m.message}</p>
               <p style="margin-top: 12px; color: var(--text-secondary);">
                 You can try again by opening the extension panel.
               </p>`,buttons:[{id:"close",class:"fw-styled-modal-btn-cancel",label:"Close"}]})}finally{u.isConnected&&p&&startStatusInfoPolling(p,u,d)}}i.on("update-extension",async({extension:c,newVersion:l})=>{s("update-extension:",c.id,`v${c.version.join(".")} → v${l.version.join(".")}`);try{delete n.loadedExtensions[c.id];const u=`extension-styles-${c.id}`,d=document.getElementById(u);d&&(d.remove(),console.log(`[Extensions] Removed old styles for ${c.id}`)),await n.extensionRegistry.uninstallExtension(c.id),await n.extensionRegistry.installExtension(l),n.installedExtensions=await n.extensionRegistry.getInstalledExtensions(),n.allCachedExtensions=[...n.installedExtensions],n.activeExtension===c.id&&i.emit("change-extension-panel",{extensionId:c.id,panelId:n.activeExtensionPanel||l.menu[0].id}),s(`[Extensions] Updated ${c.name} to v${l.version.join(".")}`),l.mipPackage&&i.emit("prompt-upload-dependencies",{extensionId:c.id,extensionName:c.name}),i.emit("render")}catch(u){console.error("[Extensions] Update failed:",u),alert(`Failed to update extension:
${u.message}`)}}),i.on("uninstall-extension",async c=>{s("uninstall-extension:",c);try{await n.extensionRegistry.uninstallExtension(c),delete n.loadedExtensions[c];const l=`extension-styles-${c}`,u=document.getElementById(l);u&&(u.remove(),console.log(`[Extensions] Removed styles for ${c}`)),n.installedExtensions=await n.extensionRegistry.getInstalledExtensions(),n.allCachedExtensions=[...n.installedExtensions],n.activeExtension===c&&(n.activeExtension=null,n.activeExtensionPanel=null,n.systemSection="settings"),s(`[Extensions] Uninstalled ${c}`),i.emit("render")}catch(l){console.error("[Extensions] Uninstall failed:",l),alert(`Failed to uninstall extension:
${l.message}`)}}),i.on("prompt-upload-dependencies",async({extensionId:c,extensionName:l})=>{s("prompt-upload-dependencies:",c);try{const u=await n.extensionRegistry.getDependencies(c);if(!u||!u.mipPackage)return;n.dependencyPrompt={extensionId:c,extensionName:l,dependencies:u},i.emit("render")}catch(u){console.error("[Extensions] Error getting dependencies:",u)}}),i.on("close-dependency-prompt",()=>{s("close-dependency-prompt"),n.dependencyPrompt=null,i.emit("render")}),i.on("upload-extension-dependencies",async c=>{s("upload-extension-dependencies:",c),n.dependencyPrompt=null;try{const l=n.installedExtensions.find(p=>p.id===c);if(!l||!l.mipPackage){i.emit("render");return}if(!n.isConnected){alert("Please connect to device first"),i.emit("render");return}stopStatusInfoPolling(),n.installingDependencies={extensionName:l.name,mipPackage:l.mipPackage},i.emit("render");const u=`
import mip
try:
...
${c.message}`)}})}async function getSkillVersionFiles(n,i,o){const r=i.latestVersion?.version;if(!r)throw new Error(`Skill "${n}" has no version`);const s=await fetch(`${o}/api/v1/skills/${encodeURIComponent(n)}/versions/${encodeURIComponent(r)}`);if(!s.ok)throw new Error(`Failed to fetch version files: ${s.status}`);const a=await s.json();return a.version||a}console.log("[Stores] ES modules loaded");let CodeMirrorEditorClass=null;async function getCodeMirrorEditor(){return CodeMirrorEditorClass||(CodeMirrorEditorClass=(await __vitePreload(()=>Promise.resolve().then(()=>editor),void 0)).CodeMirrorEditor),CodeMirrorEditorClass}const log=console.log,device=BridgeDevice,disk=BridgeDisk,registryCache=new RegistryCache;async function store(n,i){async function o(a){const{source:c,parentFolder:l,fileName:u,content:d=newFileContent,hasChanges:p=!1}=a,f=generateHash(),h=await getCodeMirrorEditor(),m=n.cache(h,`editor_${f}`);return m.content=d,m.fileName=u,{id:f,source:c,parentFolder:l,fileName:u,editor:m,hasChanges:p}}async function r(a,c=null,l=null,u=null){a=="board"?n.boardNavigationPath:n.diskNavigationPath;const d=await o({fileName:c===null?generateFileName():c,parentFolder:l,source:a,hasChanges:!0,content:u||newFileContent});let p=!1;return l!=null&&(a=="board"?p=await fileExists(BridgeDevice,getFullPath$1(n.boardNavigationRoot,d.parentFolder,d.fileName)):a=="disk"&&(p=await disk.fileExists(disk.getFullPath(n.diskNavigationRoot,d.parentFolder,d.fileName)))),n.openFiles.find(h=>h.parentFolder===d.parentFolder&&h.fileName===d.fileName&&h.source===d.source)||p?(await confirmDialog(`File ${d.fileName} already exists on ${a}. Please choose another name.`),!1):(d.editor.onChange=function(){d.hasChanges=!0,i.emit("render")},n.openFiles.push(d),n.editingFile=d.id,!0)}await initializeState(n,i,r),initializeTheme(n,i),registerThemeHandlers(n,i);const s=()=>{};registerDialogHandlers(n,i,device),registerConnectionHandlers(n,i,device,s,bindTerminalOutput),registerFileOperationHandlers(n,i,device,disk,o,r),registerTerminalHandlers(n,i,device,XTerm),registerAIAgentHandlers(n,i,r),registerLogHandlers(n,i),document.addEventListener("connection-mode-change",a=>{const c=a.detail?.mode||"none";n.connectionMode=c,i.emit("render")}),document.addEventListener("firmware-panel-update",()=>{i.emit("render")}),i.on("navigate",a=>{i.emit("change-system-section",a)}),i.on("change-view",async a=>{a==="file-manager"&&(n.isConnected&&BridgeDevice&&BridgeDevice.isCommandRunning()&&(i.emit("stop"),await sleep(250)),n.filesLoadedOnce||(log("[File Manager] Loading files for first time..."),n.filesLoadedOnce=!0),n.isConnected?i.emit("refresh-files"):i.emit("refresh-disk-files")),n.systemSection!==a&&(n.selectedFiles=[],n.systemSection=a,n.activeNetworkPanel=null,n.activePeripheralsPanel=null,n.activeSystemPanel=null,n.activeExtension=null,n.activeExtensionPanel=null,a==="editor"&&n.isConnected&&setTimeout(()=>{bindTerminalOutput(n);const c=n.cache(XTerm,"terminal");c&&c.term&&(i.emit("bind-terminal-input"),i.emit("terminal-focus"))},100),i.emit("render"))}),i.on("launch-app",async(a,c)=>{window.launchApp(a,c)}),i.on("change-system-section",a=>{if(log("change-system-section",a),n.systemSection=a,a.startsWith("network:")){const c=a.split(":")[1];n.activeNetworkPanel=c,n.activeSystemPanel=null,n.activePeripheralsPanel=null,n.activeExtension=null,n.activeExtensionPanel=null}else if(a.startsWith("peripherals:")){const c=a.split(":")[1];n.activePeripheralsPanel=c,n.activeNetworkPanel=null,n.activeSystemPanel=null,n.activeExtension=null,n.activeExtensionPanel=null}else if(a.startsWith("system:")){const c=a.split(":")[1];n.activeSystemPanel=c,n.activeNetworkPanel=null,n.activePeripheralsPanel=null,n.activeExtension=null,n.activeExtensionPanel=null}else n.activeNetworkPanel=null,n.activePeripheralsPanel=null,n.activeSystemPanel=null,n.activeExtension=null,n.activeExtensionPanel=null;(a==="ai-agent"||a==="system:ai-agent")&&n.aiAgent.settings.provider==="openrouter"&&n.aiAgent.settings.apiKey&&n.aiAgent.openRouterModels.length===0&&i.emit("ai-fetch-openrouter-models"),i.emit("render")}),registerNetworkConfigHandlers(n,i,device),registerHardwareConfigHandlers(n,i,device),registerSystemConfigHandlers(n,i,device),registerExtensionHandlers(n,i,device),registerScriptOsHandlers(n,i,registryCache,r),typeof debuggerStore=="function"?debuggerStore(n,i):console.warn("[Store] Debugger store not loaded"),i.on("change-locale",a=>{n.locale=a,localStorage.setItem("locale",a),window.i18n&&window.i18n.setLocale(a),i.emit("render")}),window.i18n?(window.i18n.setLocale(n.locale),console.log("[i18n] Locale initialized to:",n.locale)):console.warn("[i18n] i18n module not available")}const PANEL_CLOSED=45,PANEL_TOO_SMALL=65,PANEL_DEFAULT=320;typeof window<"u"&&(window.PANEL_CLOSED=PANEL_CLOSED,window.PANEL_TOO_SMALL=PANEL_TOO_SMALL,window.PANEL_DEFAULT=PANEL_DEFAULT);function App(n,i){return n.isInitializing?html$1`
  ...
  `}async function initApp(){window.i18nReady&&(await window.i18nReady,console.log("[App] Translations loaded, starting app...")),console.log("%c ScriptO Studio v1.0.0 ","background: #9b59b6; color: #fff; font-size: 11px; padding: 2px 6px; border-radius: 3px;","build 99d39be (2026-03-01)");let n=Choo();n.use(store),n.route("*",App),n.mount("#app"),window.appState=n.state,window.appInstance=n,n.emitter.on("DOMContentLoaded",()=>{n.state.diskNavigationRoot&&n.emitter.emit("refresh-files");const i=new URLSearchParams(window.location.search),o=i.get("device");if(o){const c=`wss://${o}/webrepl`;localStorage.setItem("webrepl-url",c),console.log(`[App] Device URL from query param: ${c}`),setTimeout(()=>{n.emitter.emit("open-connection-dialog");const l=window.location.pathname;window.history.replaceState({},"",l)},500)}const r=i.get("configure");r&&setTimeout(()=>{n.emitter.emit("configure-scripto",r);const c=window.location.pathname;window.history.replaceState({},"",c)},500);const s=i.get("extension");if(s){localStorage.setItem("pending-extension-id",s),console.log(`[App] Extension from query param: ${s}`),i.delete("extension");const c=i.toString(),l=window.location.pathname+(c?"?"+c:"");window.history.replaceState({},"",l)}const a=localStorage.getItem("pending-extension-id");if(a&&setTimeout(async()=>{try{const u=(await new ExtensionRegistry().getInstalledExtensions()).find(d=>d.id===a);u&&u.menu&&u.menu.length>0&&(console.log(`[App] Auto-navigating to extension: ${a}`),localStorage.removeItem("pending-extension-id"),n.emitter.emit("change-extension-panel",{extensionId:a,panelId:u.menu[0].id}))}catch(c){console.warn("[App] Could not auto-navigate to extension:",c)}},1e3),!n.state.isConnected){let c=document.getElementById("hand-click-hint");if(!c){c=document.createElement("div"),c.id="hand-click-hint";const d=document.createElementNS("http://www.w3.org/2000/svg","svg");d.setAttribute("width","36"),d.setAttribute("height","36"),d.setAttribute("viewBox","0 0 24 24"),d.setAttribute("fill","none"),d.setAttribute("stroke","currentColor"),d.setAttribute("stroke-width","2"),d.setAttribute("stroke-linecap","round"),d.setAttribute("stroke-linejoin","round"),d.innerHTML=`
          ...
          <path d="M14 3l1 -1" />
          <path d="M15 6h1" />
        `,c.appendChild(d),document.body.appendChild(c)}const l=()=>{c.classList.add("animate"),setTimeout(()=>{c.classList.remove("animate")},7500)};setTimeout(l,1e3);const u=setInterval(l,1e4);n.emitter.on("connect",()=>{clearInterval(u);const d=document.getElementById("hand-click-hint");d&&d.remove()})}})}document.readyState==="complete"?initApp():window.addEventListener("load",initApp);export{__vitePreload as _};
