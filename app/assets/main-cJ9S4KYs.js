...
`),console.log(`[device-registry] Updated ${i} to v${o}`)}catch(r){throw console.error("[device-registry] Error updating registry:",r),r}}function needsInstall(n,i){if(!i)return!0;const o=Array.isArray(n)?n.join("."):String(n),r=i.version||"";return o!==r}function hasOnInstallMethod(n){return n?/async\s+onInstall\s*\(/.test(n):!1}async function checkAndRunOnInstall(n,i,o,r,s,a){const d=o.data.config,l=Array.isArray(d.version)?d.version.join("."):String(d.version||"0.0.0");try{const u=(await readDeviceExtensionRegistry(r))[n];if(!needsInstall(l,u))return;s.loadedExtensions[n].installing=!0,a("render");try{await i.onInstall(),await updateDeviceExtensionRegistry(r,n,l)}catch(p){console.error("[ExtensionContainer] onInstall failed:",p),s.loadedExtensions[n].installError=p.message}finally{s.loadedExtensions[n].installing=!1,a("render")}}catch(c){console.warn("[ExtensionContainer] Could not check device registry:",c)}}async function instantiateExtension$1(extensionId,loadedExtension,state,emit,html){if(loadedExtension.data.styles&&!loadedExtension.stylesInjected){const n=`extension-styles-${extensionId}`;let i=document.getElementById(n);i||(i=document.createElement("style"),i.id=n,i.textContent=loadedExtension.data.styles,document.head.appendChild(i)),loadedExtension.stylesInjected=!0}const extensionCode=loadedExtension.data.content,filesMatch=extensionCode.match(/export\s+const\s+__DEVICE_FILES__\s*=\s*(\{[\s\S]*?\});/);let deviceFiles={};if(filesMatch)try{const rawFiles=eval("("+filesMatch[1]+")");for(const[n,i]of Object.entries(rawFiles))try{deviceFiles[n]=atob(i)}catch{deviceFiles[n]=i}}catch(n){console.warn("[ExtensionContainer] Device files parse failed:",n)}const blob=new Blob([extensionCode],{type:"text/javascript"}),blobUrl=URL.createObjectURL(blob);let ExtensionClass;try{ExtensionClass=(await import(blobUrl)).default}finally{URL.revokeObjectURL(blobUrl)}if(!ExtensionClass)throw new Error("No default export found in extension bundle");const deviceAPI=new DeviceAPI$1(BridgeDevice),extensionInstance=new ExtensionClass(deviceAPI,emit,state,html);extensionInstance.deviceFiles=deviceFiles,loadedExtension.instance=extensionInstance,loadedExtension.instantiating=!1,emit("render"),state.isConnected&&typeof extensionInstance.onInstall=="function"&&checkAndRunOnInstall(extensionId,extensionInstance,loadedExtension,deviceAPI,state,emit)}function ExtensionContainer(n,i,o){const r=n.activeExtension,s=n.activeExtensionPanel;if(!r||!s)return o`
    ...
    `;try{return d[l]()}catch(c){return console.error("[ExtensionContainer] Render error:",c),o`
      <div class="system-panel">
        <div class="panel-message error">
...
`;this.term.write(h),this.state.logs?.autoScroll!==!1&&this.term.scrollToBottom()}cleanup(){this.writeHandler&&(window.removeEventListener("log-terminal-write",this.writeHandler),this.writeHandler=null),this.clearHandler&&(window.removeEventListener("log-terminal-clear",this.clearHandler),this.clearHandler=null),this.resizeObserver&&(this.resizeObserver.disconnect(),this.resizeObserver=null),this.term&&(this.term.dispose(),this.term=null),this.fitAddon=null}}let logTerminalComponent=null;function LogSidebar(n,i){if(!n.logs.isOpen)return logTerminalComponent=null,null;logTerminalComponent||(logTerminalComponent=new LogTerminalComponent("log-terminal",n,i));const o=logTerminalComponent.render(n),r=n.logSidebarWidth||350;return html`
  ...
  `:""}function EthernetPanel(n,i){if(!n.networksInfo&&n.isConnected&&!n.isLoadingNetworks&&i("refresh-networks"),n.isConnected&&!n.ethConfigLoaded&&!n.isLoadingEthConfig&&i("load-eth-config"),!n.networksInfo)return html`
    ...
    `;const o=n.networksInfo.eth,r=n.ethStatus,s=r&&r.initialized?r:o&&o.mac?o:r||o,a=n.ethConfig||{},d=o!==null,l=s&&(s.mac||s.enabled||s.enable||s.initialized),c=s&&s.ip&&s.ip!=="0.0.0.0",u=s&&s.linkup===!0,p=c;return d?html`
    <div class="panel-container">
      <div class="panel-header">
        <h2>Ethernet Configuration</h2>
      </div>
  ...
  `:html`
    ...
    `}function WWANPanel(n,i){n.isConnected&&!n.wwanConfigLoaded&&!n.isLoadingWwanConfig&&i("load-wwan-config"),n.isConnected&&!n.modemStatusLoaded&&!n.isLoadingModemStatus&&i("load-modem-status");const o=n.wwanConfig||{},r=n.modemStatus||{},s=r.ppp||{};let a="Disabled",d="status-disabled";return o.mobile_data_enabled&&(s.connected?(a="Connected",d="status-connected"):s.connecting?(a="Connecting...",d="status-connecting"):r.connected?(a="Standby (WiFi OK)",d="status-standby"):(a="Waiting for modem...",d="status-waiting")),html`
    <div class="panel-container">
      <div class="panel-header">
      ...
      <div class="panel-section">
        ...
        <div class="mobile-data-control">
          ...
          <div class="status-row">
            ...
            <span class="status-value ${d}">${a}</span>
          </div>
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${l=>{l.preventDefault();const c=new FormData(l.target),u={apn:c.get("apn")||"",username:c.get("username")||"",password:c.get("password")||"",auto_init_modem:c.get("auto_init_modem")==="on",mobile_data_enabled:o.mobile_data_enabled||!1};i("save-wwan-config",u)}}>
          <div class="form-group">
            <label for="wwan-apn">
              GPRS APN
              <span class="label-tooltip">
  ...
  `}function NTPPanel(n,i){n.isConnected&&!n.ntpConfigLoaded&&!n.isLoadingNtpConfig&&i("load-ntp-config");const o=n.ntpConfig||{server:"pool.ntp.org",timezone:"UTC",autoDetect:!1,autoSync:!1},r=n.ntpSyncResult||null,s=l=>{if(!l)return"--:--:--";const{year:c,month:u,day:p,hour:g,minute:f,second:h}=l;return`${String(g).padStart(2,"0")}:${String(f).padStart(2,"0")}:${String(h).padStart(2,"0")}`},a=l=>{if(!l)return"";const{year:c,month:u,day:p}=l;return`${c}-${String(u).padStart(2,"0")}-${String(p).padStart(2,"0")}`},d=[{value:"UTC",offset:0,label:"UTC (Coordinated Universal Time)"},{value:"EST",offset:-5,label:"EST (Eastern Standard Time)"},{value:"CST",offset:-6,label:"CST (Central Standard Time)"},{value:"MST",offset:-7,label:"MST (Mountain Standard Time)"},{value:"PST",offset:-8,label:"PST (Pacific Standard Time)"},{value:"GMT",offset:0,label:"GMT (Greenwich Mean Time)"},{value:"CET",offset:1,label:"CET (Central European Time)"},{value:"EET",offset:2,label:"EET (Eastern European Time)"},{value:"JST",offset:9,label:"JST (Japan Standard Time)"},{value:"AEST",offset:10,label:"AEST (Australian Eastern Standard Time)"}];return html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${async l=>{l.preventDefault();const c=new FormData(l.target),u=c.get("server")||"pool.ntp.org",p=c.get("timezone")||"UTC",g=c.get("auto_detect")==="on",f=c.get("auto_sync")==="on",h=d.find(y=>y.value===p),m=h?h.offset:0,v={server:u,tz_offset:m,timezone:p,auto_detect:g,auto_sync:f};try{await new Promise((y,b)=>{const x=setTimeout(()=>b(new Error("Save timeout")),1e4),S=()=>{clearTimeout(x),window.appInstance.emitter.removeListener("ntp-config-saved",$),window.appInstance.emitter.removeListener("ntp-config-save-error",w)},$=()=>{S(),y()},w=k=>{S(),b(k)};window.appInstance.emitter.once("ntp-config-saved",$),window.appInstance.emitter.once("ntp-config-save-error",w),i("save-ntp-config",v)}),i("sync-ntp-time",u,m,g,f)}catch(y){console.error("[NTP] Failed to save config before sync:",y),alert(`Failed to save NTP configuration: ${y.message}`)}}}>
          <div class="form-group">
  ...
  `}function rssiToBars(n){return n==null||n===-999?0:n>=-70?5:n>=-80?4:n>=-90?3:n>=-100?2:n>=-110?1:0}function rssiToQuality(n){return n==null||n===-999?"Unknown":n>=-70?"Excellent":n>=-80?"Good":n>=-90?"Fair":n>=-100?"Poor":"Very Poor"}function getSignalColor(n){return n==null||n===-999?"var(--text-secondary)":n>=-70||n>=-80?"#22c55e":n>=-90||n>=-100?"#f59e0b":"#ef4444"}function SDCardPanel(n,i){const r=n.boardConfig?.hardware?.sdcard;n.isConnected&&!n.sdcardConfigLoaded&&!n.isLoadingSdcardConfig&&i("load-sdcard-config"),r&&n.isConnected&&!n.sdcardInfo&&!n.isLoadingSdcardInfo&&i("sdcard-get-info");const s=n.sdcardConfig||{mountPoint:"/sd",autoMount:!1};return n.isConnected?r?html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${async a=>{a.preventDefault();const d=new FormData(a.target),l={mountPoint:d.get("mount_point")||"/sd",autoMount:d.get("auto_mount")==="on"};i("save-sdcard-config",l)}}>
          <div class="form-group">
...
Click OK to try again, or Cancel to abort.`))throw new Error("Connection cancelled")}}catch(n){addLog(`Error: ${n.message}`,"error"),triggerRerender()}}async function continueToProvision(n){try{const i=n+".local";addLog("Generating HTTPS certificate..."),window.forge||await new Promise((a,d)=>{const l=document.createElement("script");l.src="https://cdn.jsdelivr.net/npm/node-forge@1.3.1/dist/forge.min.js",l.onload=a,l.onerror=()=>d(new Error("Failed to load forge")),document.head.appendChild(l)}),addLog(`Hostname: ${i}`);const{certPem:o,keyPem:r}=generateSelfSignedCert(i);addLog(`Certificate generated (${o.length} bytes)`),addLog("Installing certificate..."),await uploadCertsToDevice(currentPort,o,r),addLog("Enabling setup mode..."),await sendREPLCommand(currentPort,`
from lib.sys import settings
...
`)).match(/HOST:(.+)/),r=o?o[1].trim():"pydirect-new";addLog(`Generating new certificate for ${r}.local...`),window.forge||await new Promise((c,u)=>{const p=document.createElement("script");p.src="https://cdn.jsdelivr.net/npm/node-forge@1.3.1/dist/forge.min.js",p.onload=c,p.onerror=u,document.head.appendChild(p)});const s=r+".local",{certPem:a,keyPem:d}=generateSelfSignedCert(s);addLog("Installing new certificate..."),await uploadCertsToDevice(currentPort,a,d),addLog("Enabling setup mode..."),await sendREPLCommand(currentPort,`
from lib.sys import settings
settings.set("setup_mode", True)
settings.save()
print("SETUP_MODE_SET")
...
`,5e3,!1),addLog("Certificates installed!")}function FirmwarePanel(n,i){appState=n;const o=panelState.view;return html$1`
        ...
        ${a?html$1`
          ${panelState.firmwareOptions.length>1?html$1`
            <div class="fw-form-group">
              <select class="fw-select" onchange=${d=>{const l=panelState.firmwareOptions.find(c=>c.productId===d.target.value);l&&(panelState.selectedFirmware=l,triggerRerender())}}>
                ${panelState.firmwareOptions.map(d=>html$1`
                  <option value="${d.productId}" selected=${d.productId===a.productId}>
                    ${d.displayName}
                  </option>
                `)}
              </select>
  ...
  `,f=n.expandedNetworks!==!1,h=html$1`
    <div class="system-sidebar-extension">
      ...
      ${f?html$1`
        <div class="system-sidebar-submenu">
          ${d.map(w=>{const k=n.activeNetworkPanel===w.id;return html$1`
              <div 
                class="system-sidebar-subitem ${k?"active":""}"
  ...
  `,y=w=>{if(w.iconSvg)try{const T=new DOMParser().parseFromString(w.iconSvg,"image/svg+xml").querySelector("svg");if(T){const E=T.getAttribute("viewBox")||"0 0 24 24",_=Array.from(T.querySelectorAll("path"));return html$1`
          ...
          `}}catch(C){console.warn("[System] Failed to parse extension icon SVG:",C)}const k=w.icon||"settings";return IconSprite.renderIcon(k,{className:"",size:20})},b=(n.installedExtensions||[]).map(w=>{const k=n.expandedExtensions[w.id],C=n.activeExtension===w.id,P=w.devices===!0;let T=null;if(P&&k)if(!n.loadedExtensions[w.id])console.warn("[System] Extension not loaded:",w.id);else{const E=n.loadedExtensions[w.id];if(!E.instance)try{const _=E.data.content,N=new Function("DeviceAPI","html","emit","state",`
              ${_}
