const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/tree-sitter-DhTlkRvW.js","assets/vendor-QvVPN6l_.js","assets/vendor-CeCKEaxg.js","assets/vendor-CbBHggdi.css","assets/index-BalQ1Nwo.js","assets/index-DXz9816u.js","assets/index-C67KyK7a.js","assets/index-BlIhKDWf.js","assets/index-DdUpN0Lu.js","assets/index-ByjkoLU-.js","assets/xterm-DOrYoP_4.css"])))=>i.map(i=>d[i]);
import{PANEL_TOO_SMALL as mt,PANEL_CLOSED as Y,PANEL_DEFAULT as qe}from"./vendor-QvVPN6l_.js";import{c as he,C as vt,h as b,a as yt,b as wt}from"./vendor-CeCKEaxg.js";(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))o(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const c of s.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&o(c)}).observe(document,{childList:!0,subtree:!0});function t(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(i){if(i.ep)return;i.ep=!0;const s=t(i);fetch(i.href,s)}})();function ce(e){const n="# === START_CONFIG_PARAMETERS ===",t="# === END_CONFIG_PARAMETERS ===",o=e.indexOf(n),i=e.indexOf(t);if(o===-1||i===-1)return console.warn("[ScriptOs Parser] Config markers not found"),null;const s=e.substring(o+n.length,i).trim();try{return bt(s)}catch(c){return console.error("[ScriptOs Parser] Failed to parse config:",c),null}}function bt(e){let n=e.trim();n.startsWith("dict(")&&n.endsWith(")")&&(n=n.substring(5,n.length-1).trim()),n=n.split(`
...
`,$=performance.now()-s;return console.log(`[Debugger] Instrumentation complete in ${$.toFixed(0)}ms`),P}console.log("[Libs] ES modules loaded");const zt="/app/";function Ut(e,n={}){const{className:t="",size:o=24,color:i="currentColor"}=n;return b`
...
`;if(e.isNewFileDialogOpen){const u=p.querySelector("#dialog-new-file .dialog-content > input");return u&&u.focus(),p}}function di(e,n){return e.isScriptOsModalOpen?html`
  ...
  `}function wi(e,n){const t=e.availableExtensions||[],o=e.installedExtensions||[],i=new Set(o.map(r=>r.id)),s=t.filter(r=>!i.has(r.id)),c=(r,a)=>{for(let l=0;l<3;l++){const d=r[l]||0,p=a[l]||0;if(d>p)return!0;if(d<p)return!1}return!1};return t.length===0?html`
      <div class="scriptos-library">
        <div class="scriptos-header-sticky">
           <div class="scriptos-header">
            <h2>Extensions</h2>
  ...
  `}function xn(e){const n={sliders:html`
    ...
    `};return n[e]||n.cpu}function bi(e,n){const t=e.isResetDialogOpen?"open":"closed";function o(r){r.target.id==="dialog-reset"&&n("close-reset-dialog")}function i(){n("trigger-reset",0)}async function s(){confirm(`HARD RESET WARNING:
...
`),s=e.debugger.active;return html`
  ...
  `}function Si(e,n,t){const o=e.activeExtension,i=e.activeExtensionPanel;if(!o||!i)return t`
    ...
    <div class="agent-sidebar">
      ...
      <div class="agent-sidebar-messages" id="agent-messages">
        ...
        `:e.aiAgent.messages.map(t=>ki(t,n))}
  ...
  `:html`<div></div>`}function ki(e,n){const t=new Date(e.timestamp).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});return e.role==="user"?html`
    ...
    `:e.role==="assistant"?html`
      <div class="agent-message agent-message-assistant">
        ...
        <div class="agent-message-content">
          <div class="agent-message-text">${$i(e.content)}</div>
          ${e.code?html`
...
`;this.term.write(f),this.state.logs?.autoScroll!==!1&&this.term.scrollToBottom()}cleanup(){this.writeHandler&&(window.removeEventListener("log-terminal-write",this.writeHandler),this.writeHandler=null),this.clearHandler&&(window.removeEventListener("log-terminal-clear",this.clearHandler),this.clearHandler=null),this.resizeObserver&&(this.resizeObserver.disconnect(),this.resizeObserver=null),this.term&&(this.term.dispose(),this.term=null),this.fitAddon=null}}let ye=null;function Pi(e,n){if(!e.logs.isOpen)return ye=null,null;ye||(ye=new Ti("log-terminal",e,n));const t=ye.render(e),o=e.logSidebarWidth||350;return html`
  ...
  `}function Ni(e,n){const{isConnected:t,selectedFiles:o}=e;return html`
  <div id="file-actions">
  ...
  ..
    ...
    `;return new MutationObserver(u=>{const f=d.querySelector("input");f&&f.focus()}).observe(d,{childList:!0,subtree:!0}),d}}function Fi(e,n){const t=()=>{e.panelHeight>Y?n("close-panel"):n("open-panel")};e.isPanelOpen;const o=e.panelHeight>mt?"visible":"hidden";let i="terminal-enabled";return(!e.isConnected||e.isNewFileDialogOpen)&&(i="terminal-disabled"),b`
  ...
  `}function Gi(e,n){return{0:"APP",1:"DATA"}[e]||`Type ${e}`}function Vi(e){if(!e)return"📦";const n=e.toLowerCase();return n.includes("ota")?"🔄":n.includes("nvs")?"💾":n.includes("www")?"🌐":n.includes("vfs")?"📁":n.includes("data")?"💿":n.includes("factory")?"🏭":"📦"}function ut(e){return e?e<1024?e+" B":e<1024*1024?(e/1024).toFixed(1)+" KB":(e/(1024*1024)).toFixed(2)+" MB":"N/A"}function kn(e,n){if(!e.networksInfo&&e.isConnected&&!e.isLoadingNetworks&&n("refresh-networks"),!e.networksInfo)return html`
      <div class="panel-container">
        <div class="panel-header">
...
Are you sure you want to continue?`)){x("Flash cancelled by user");return}try{m.flashStep="downloading",m.flashProgress=0,T(),x(`Downloading ${m.selectedFirmware.name}...`);const n=await Co(m.selectedFirmware.downloadUrl,t=>{m.flashProgress=t,t%20===0&&x(`Download: ${t}%`),T()});x(`Downloaded ${(n.byteLength/1024/1024).toFixed(1)} MB`),m.flashStep="flashing",m.flashProgress=0,T(),x("Syncing with device...");try{await A.sync()}catch{await A.initialize()}x("Flashing firmware...");try{await A.flashData(n,(t,o)=>{const i=Math.floor(t/o*100);m.flashProgress=i,i%10===0&&x(`Progress: ${i}%`),T()},0,!0)}catch(t){if((t?.message||String(t)).includes("Timed out")&&m.flashProgress>=95)x("Flash complete (device rebooted)");else throw t}x("✓ Flash complete!","success");try{await A.disconnect()}catch{}A=null,m.flashStep="complete",m.flashComplete=!0,m.terminalCollapsed=!0,T()}catch(n){console.error("[firmware-panel] Flash error:",n),x(`Flash failed: ${n.message}`,"error"),m.flashStep="select";try{S&&await S.close()}catch{}S=null,A=null,T()}}async function To(){x("Proceeding to WiFi setup..."),m.flashStep="select",m.flashComplete=!1,m.flashProgress=0,m.view="change-wifi";try{if(!S){x("Please reconnect device"),m.view="connect",T();return}ft()}catch(e){x(`Error: ${e.message}`,"error")}T()}async function Po(){try{x("Reading credentials from device..."),await nn();const n=(await j(S,`
import json
...
Click OK to try again, or Cancel to abort.`))throw new Error("Connection cancelled")}}catch(e){x(`Error: ${e.message}`,"error"),T()}}async function No(){try{x("Starting re-provisioning..."),await nn(),x("Reading device settings...");const n=(await j(S,`
from lib.sys import settings
print("HOST:" + settings.get("device.hostname", "pydirect-xxxx"))
`)).match(/HOST:(.+)/),t=n?n[1].trim():"pydirect-new";x(`Generating new certificate for ${t}.local...`),window.forge||await new Promise((r,a)=>{const l=document.createElement("script");l.src="https://cdn.jsdelivr.net/npm/node-forge@1.3.1/dist/forge.min.js",l.onload=r,l.onerror=a,document.head.appendChild(l)});const o=t+".local",{certPem:i,keyPem:s}=Oo(o);x("Installing new certificate..."),await Fo(S,i,s),x("Enabling setup mode..."),await j(S,`
from lib.sys import settings
settings.set("setup_mode", True)
settings.save()
print("SETUP_MODE_SET")
`,5e3,!1),x("Resetting device..."),await j(S,"import machine; machine.reset()",1e3,!1);try{await S.close()}catch{}S=null,x("Device is restarting and connecting to WiFi...","success");const c=`https://${o}/setup`;Io(o,c)}catch(e){x(`Error: ${e.message}`,"error"),T()}}function Io(e,n){const t=document.createElement("div");t.className="fw-modal-overlay active",t.innerHTML=`
  ...
  `,document.body.appendChild(t),t.querySelector(".fw-modal-ok").addEventListener("click",()=>{t.remove(),m.view="scenarios",m.reblessComplete=!1,T(),window.open(n,"_blank")}),t.querySelector(".fw-modal-cancel").addEventListener("click",()=>{t.remove(),m.view="scenarios",m.reblessComplete=!1,T()})}function Oo(e,n=10){const t=forge.pki.rsa.generateKeyPair(2048),o=forge.pki.createCertificate();o.publicKey=t.publicKey,o.serialNumber="01",o.validity.notBefore=new Date,o.validity.notAfter=new Date,o.validity.notAfter.setFullYear(o.validity.notBefore.getFullYear()+n);const i=[{name:"commonName",value:e}];return o.setSubject(i),o.setIssuer(i),o.setExtensions([{name:"subjectAltName",altNames:[{type:2,value:e}]}]),o.sign(t.privateKey,forge.md.sha256.create()),{certPem:forge.pki.certificateToPem(o),keyPem:forge.pki.privateKeyToPem(t.privateKey)}}async function Fo(e,n,t){x("Creating /certs directory..."),await j(e,`
import os
try:
    os.mkdir('/certs')
except OSError:
...
print('KEY_OK')
`,5e3,!1),x("Certificates installed!")}function Do(e,n){ge=e;const t=m.view;return b`
    <div class="firmware-panel">
      ${t==="connect"?Ao():""}
      ${t==="scenarios"?Ro():""}
      ${t==="new-device"?Lo():""}
      ${t==="forgot-credentials"?Mo():""}
      ${t==="change-wifi"?Bo():""}
      ${t==="re-provision"?zo():""}
  ...
  `}function Ao(e,n){return b`
    <div class="fw-view active">
      ...
      <button class="fw-btn fw-btn-primary" onclick=${So}>
  ...
  `}function Ro(e,n){const t=m.deviceInfo,o=v(ie?"firmware.readyToFlash":"firmware.pyDirectDetected"),i=ie?"fw-status-warning":"fw-status-success";return b`
    <div class="fw-view active">
      ${t?b`
      ...
      <button class="fw-btn fw-btn-secondary" onclick=${Eo}>
        ${w.renderIcon("unlink",{className:"fw-btn-icon"})}
        ${v("firmware.disconnect")}
      </button>
    </div>
  `}function Lo(e,n){const t=m.deviceInfo,o=m.flashStep,i=m.flashProgress,s=m.selectedFirmware;if(o==="complete")return b`
      <div class="fw-view active">
        <div class="fw-device-card">
          ...
          <button class="fw-btn fw-btn-primary" onclick=${To}>
    ...
    <div class="fw-view active">
      ...
      <div class="fw-device-card" style="margin-top: 1rem;">
        ...
        ${s?b`
          ...
          <button class="fw-btn fw-btn-primary" onclick=${_o} style="margin-top: 1rem;">
  ...
  `}function Mo(e,n){const t=m.credentials;return b`
    <div class="fw-view active">
      <button class="fw-btn-back" onclick=${Fe}>
        ${v("firmware.back")}
      </button>
  ...
  `}function Bo(e,n){return m.setupComplete?b`
      <div class="fw-view active">
        <div class="fw-device-card">
          <div class="fw-device-card-header">
            <h3>
  ...
  `}function zo(e,n){const t=()=>{const i=`https://${m.reblessHostname}`;window.open(i,"_blank")},o=async()=>{await ko(),m.reblessComplete=!1,m.reblessHostname=null,m.view="connect",T()};return m.reblessComplete?b`
      <div class="fw-view active">
        <div class="fw-device-card">
          <div class="fw-device-card-header">
            <h3>
  ...
  `}function Uo(){const e=m.logs;return b`
  ...
  `}function jo(e,n){const t=e.aiAgent.settings;return html`
    <div class="panel-container">
      <div class="ai-agent-content">
        ...
        <div class="ai-agent-section">
          ...
          >
            ${Wo(t.provider,t.model,e.aiAgent.openRouterModels,e.aiAgent.isLoadingOpenRouterModels)}
          </select>
  ...
  `}function qo(e,n){let t="Connect to board",o=`${e.diskNavigationRoot}${e.diskNavigationPath}`;return e.isConnected&&(t=`${e.connectedPort}${e.boardNavigationPath}`),html`
    <div id="file-manager">
      <div id="board-files">
  ...
  `}function je(e){window.open(e,"_blank","noopener,noreferrer")}function Go(e,n){e.sidebarIconRotated===void 0&&(e.sidebarIconRotated=!1);const t=e.connectionMode==="webrepl"?"#00FF7F":e.connectionMode==="usb"?"#FF9500":"var(--text-secondary)",o=b`
  ...
  `,y=C=>{if(C.iconSvg)try{const z=new DOMParser().parseFromString(C.iconSvg,"image/svg+xml").querySelector("svg");if(z){const N=z.getAttribute("viewBox")||"0 0 24 24",D=Array.from(z.querySelectorAll("path"));return b`
            <svg 
          ...
          `}}catch(_){console.warn("[System] Failed to parse extension icon SVG:",_)}const L=C.icon||"settings";return w.renderIcon(L,{className:"",size:20})},E=(e.installedExtensions||[]).map(C=>{const L=e.expandedExtensions[C.id],_=e.activeExtension===C.id,W=C.devices===!0;let z=null;if(W&&L)if(!e.loadedExtensions[C.id])console.warn("[System] Extension not loaded:",C.id);else{const N=e.loadedExtensions[C.id];if(!N.instance)try{const D=N.data.content,oe=new Function("DeviceAPI","html","emit","state",`
            ...
            `)(DeviceAPI,b,n,e),De=new DeviceAPI(B),Ae=new oe(De,n,e,b);e.loadedExtensions[C.id].instance=Ae}catch(D){console.error(`[System] Error instantiating extension ${C.id}:`,D)}if(N.instance&&typeof N.instance.getMenuItems=="function")try{const D=N.instance.getMenuItems();D&&Array.isArray(D)&&(C.menu=D)}catch(D){console.error(`[System] Error getting menu items for ${C.id}:`,D)}if(N.instance&&typeof N.instance.renderSidebarDevices=="function")try{z=N.instance.renderSidebarDevices()}catch(D){console.error(`[System] Error rendering sidebar devices for ${C.id}:`,D)}else console.warn(`[System] renderSidebarDevices not available for ${C.id}`)}if(L&&e.loadedExtensions[C.id]?.instance){const N=e.loadedExtensions[C.id];if(N.instance&&typeof N.instance.getMenuItems=="function")try{const D=N.instance.getMenuItems();D&&Array.isArray(D)&&(C.menu=D)}catch(D){console.error(`[System] Error getting menu items for ${C.id}:`,D)}}return b`
      <div class="system-sidebar-extension">
        <div 
        ...
        ${L?b`
          <div class="system-sidebar-submenu">
            ${C.menu.map(N=>{const D=e.activeExtension===C.id&&e.activeExtensionPanel===N.id,q=N.disabled===!0;return b`
                <div 
  ...
  `}function Ko(e,n){const t=e.systemSection;if(t==="editor")return Ho(e,n);if(t==="file-manager")return qo(e,n);if(t==="landing")return _n(e,n);if(t?.startsWith("extension:"))return Si(e,n,b);if(t?.startsWith("network:"))switch(t.split(":")[1]){case"wifi":return kn(e,n);case"ethernet":return Xi(e,n);case"vpn":return Yi(e,n);case"btle":return Qi(e,n);case"wwan":return Zi(e,n);case"mqtt":return eo(e,n);case"ntp":return no(e,n);case"can":return to(e,n);default:return kn(e,n)}if(t?.startsWith("peripherals:"))switch(t.split(":")[1]){case"gps":return $n(e,n);case"4g-modem":return so(e,n);case"sdcard":return co(e,n);default:return $n(e,n)}if(t?.startsWith("system:"))switch(t.split(":")[1]){case"settings":return Sn(e,n);case"ai-agent":return jo(e,n);case"firmware":return Do(e);default:return Sn(e,n)}return e.isConnected?ji(e,n):_n(e,n)}const Ce=Date.now();window.i18nReady=Promise.all([fetch(`locales/en.json?v=${Ce}`).then(e=>e.json()),fetch(`locales/de.json?v=${Ce}`).then(e=>e.json()),fetch(`locales/es.json?v=${Ce}`).then(e=>e.json()),fetch(`locales/fr.json?v=${Ce}`).then(e=>e.json())]).then(([e,n,t,o])=>{if(window.i18n){window.i18n.initTranslations(e,n,t,o);const i=localStorage.getItem("locale")||"en";window.i18n.setLocale(i),console.log("[i18n] Translations loaded, locale set to:",i),window.appInstance&&window.appInstance.emitter.emit("render")}else console.warn("[i18n] window.i18n not available yet");return!0}).catch(e=>(console.error("[i18n] Failed to load translations:",e),!1));const Oe=`# This program was created in ScriptO Studio for MicroPython
...
`,c.write(u),c.write(G),c.scrollToBottom(),e.bannerDisplayed=!0,console.log("[Connection] Welcome banner displayed")}}catch(r){console.debug("[Connection] Failed to fetch banner:",r.message)}})}function ds(e,n,t,o,i,s,c){n.on("save",async()=>{if(console.log("save"),lt({isConnected:e.isConnected,openFiles:e.openFiles,editingFile:e.editingFile})==!1){console.log("can't save");return}e.isSaving=!0,I(e);let a=e.openFiles.find(g=>g.id===e.editingFile),l=!1;const d=a.parentFolder,p=d===null;p&&(a.source=="board"?a.parentFolder=e.boardNavigationPath:a.source=="disk"&&(a.parentFolder=e.diskNavigationPath));let u=!1;if(a.source=="board"?u=await te(B,k(e.boardNavigationRoot,a.parentFolder,a.fileName)):a.source=="disk"&&(u=await o.fileExists(o.getFullPath(e.diskNavigationRoot,a.parentFolder,a.fileName))),(p||!u)&&(a.source=="board"?(a.parentFolder=e.boardNavigationPath,l=await te(B,k(e.boardNavigationRoot,a.parentFolder,a.fileName))):a.source=="disk"&&(a.parentFolder=e.diskNavigationPath,l=await o.fileExists(o.getFullPath(e.diskNavigationRoot,a.parentFolder,a.fileName)))),l&&await M(`You are about to overwrite the file ${a.fileName} on your ${a.source}.
...
`,d+="Are you sure you want to proceed?",await M(d)!==0){e.isSaving=!1,e.renamingFile=null,I(e),n.emit("render");return}a.type=="folder"?await o.removeFolder(o.getFullPath(e.diskNavigationRoot,e.diskNavigationPath,r)):a.type=="file"&&await o.removeFile(o.getFullPath(e.diskNavigationRoot,e.diskNavigationPath,r))}try{e.renamingFile=="board"?await vn(t,k(e.boardNavigationRoot,e.boardNavigationPath,a.fileName),k(e.boardNavigationRoot,e.boardNavigationPath,r)):await o.renameFile(o.getFullPath(e.diskNavigationRoot,e.diskNavigationPath,a.fileName),o.getFullPath(e.diskNavigationRoot,e.diskNavigationPath,r));const l=e.openFiles.findIndex(d=>d.fileName===a.fileName&&d.source===a.source&&d.parentFolder===a.parentFolder);l>-1&&(e.openFiles[l].fileName=r,n.emit("render"))}catch{alert(`The file ${a.fileName} could not be renamed to ${r}`)}e.isSaving=!1,e.renamingFile=null,I(e),n.emit("refresh-files"),n.emit("render")}),n.on("rename-tab",r=>{console.log("rename-tab",r),e.renamingTab=r,n.emit("render")}),n.on("finish-renaming-tab",async r=>{console.log("finish-renaming-tab",r);const a=e.openFiles.find(g=>g.id===e.renamingTab);if(!r||a.fileName==r){e.renamingTab=null,e.isSaving=!1,I(e),n.emit("render");return}e.isSaving=!0,I(e);const l=a.parentFolder,d=a.fileName;a.fileName=r;const p=l===null;let u=!1;p||(a.source=="board"?u=await te(B,k(e.boardNavigationRoot,a.parentFolder,d)):a.source=="disk"&&(u=await o.fileExists(o.getFullPath(e.diskNavigationRoot,a.parentFolder,d)))),(p||!u)&&(a.source=="board"?a.parentFolder=e.boardNavigationPath:a.source=="disk"&&(a.parentFolder=e.diskNavigationPath));let f=!1;if(a.source=="board"?f=await te(B,k(e.boardNavigationRoot,a.parentFolder,a.fileName)):a.source=="disk"&&(f=await o.fileExists(o.getFullPath(e.diskNavigationRoot,a.parentFolder,a.fileName))),f&&await M(`You are about to overwrite the file ${a.fileName} on your ${a.source}.
...
${c.message}`)}})}console.log("[Stores] ES modules loaded");let We=null;async function ms(){return We||(We=(await R(()=>Promise.resolve().then(()=>Ht),void 0)).CodeMirrorEditor),We}const In=console.log,K=B,He=V,vs=new Jo;async function ys(e,n){async function t(s){const{source:c,parentFolder:r,fileName:a,content:l=Oe,hasChanges:d=!1}=s,p=ii(),u=await ms(),f=e.cache(u,`editor_${p}`);return f.content=l,f.fileName=a,{id:p,source:c,parentFolder:r,fileName:a,editor:f,hasChanges:d}}async function o(s,c=null,r=null,a=null){s=="board"?e.boardNavigationPath:e.diskNavigationPath;const l=await t({fileName:c===null?Ke():c,parentFolder:r,source:s,hasChanges:!0,content:a||Oe});let d=!1;return r!=null&&(s=="board"?d=await te(B,st(e.boardNavigationRoot,l.parentFolder,l.fileName)):s=="disk"&&(d=await He.fileExists(He.getFullPath(e.diskNavigationRoot,l.parentFolder,l.fileName)))),e.openFiles.find(u=>u.parentFolder===l.parentFolder&&u.fileName===l.fileName&&u.source===l.source)||d?(await M(`File ${l.fileName} already exists on ${s}. Please choose another name.`),!1):(l.editor.onChange=function(){l.hasChanges=!0,n.emit("render")},e.openFiles.push(l),e.editingFile=l.id,!0)}await Xo(e,n,o),Zo(e,n),es(e,n);const i=()=>{};ns(e,n,K),cs(e,n,K,i,Ie),ds(e,n,K,He,t,o),ts(e,n,K,H),gs(e,n,o),is(e,n),document.addEventListener("connection-mode-change",s=>{const c=s.detail?.mode||"none";e.connectionMode=c,n.emit("render")}),document.addEventListener("firmware-panel-update",()=>{n.emit("render")}),n.on("navigate",s=>{n.emit("change-system-section",s)}),n.on("change-view",async s=>{s==="file-manager"&&(e.isConnected&&B&&B.isCommandRunning()&&(n.emit("stop"),await gt(250)),e.filesLoadedOnce||(In("[File Manager] Loading files for first time..."),e.filesLoadedOnce=!0),e.isConnected?n.emit("refresh-files"):n.emit("refresh-disk-files")),e.systemSection!==s&&(e.selectedFiles=[],e.systemSection=s,e.activeNetworkPanel=null,e.activePeripheralsPanel=null,e.activeSystemPanel=null,e.activeExtension=null,e.activeExtensionPanel=null,s==="editor"&&e.isConnected&&setTimeout(()=>{Ie(e);const c=e.cache(H,"terminal");c&&c.term&&(n.emit("bind-terminal-input"),n.emit("terminal-focus"))},100),n.emit("render"))}),n.on("launch-app",async(s,c)=>{window.launchApp(s,c)}),n.on("change-system-section",s=>{if(In("change-system-section",s),e.systemSection=s,s.startsWith("network:")){const c=s.split(":")[1];e.activeNetworkPanel=c,e.activeSystemPanel=null,e.activePeripheralsPanel=null,e.activeExtension=null,e.activeExtensionPanel=null}else if(s.startsWith("peripherals:")){const c=s.split(":")[1];e.activePeripheralsPanel=c,e.activeNetworkPanel=null,e.activeSystemPanel=null,e.activeExtension=null,e.activeExtensionPanel=null}else if(s.startsWith("system:")){const c=s.split(":")[1];e.activeSystemPanel=c,e.activeNetworkPanel=null,e.activePeripheralsPanel=null,e.activeExtension=null,e.activeExtensionPanel=null}else e.activeNetworkPanel=null,e.activePeripheralsPanel=null,e.activeSystemPanel=null,e.activeExtension=null,e.activeExtensionPanel=null;(s==="ai-agent"||s==="system:ai-agent")&&e.aiAgent.settings.provider==="openrouter"&&e.aiAgent.settings.apiKey&&e.aiAgent.openRouterModels.length===0&&n.emit("ai-fetch-openrouter-models"),n.emit("render")}),ss(e,n,K),rs(e,n,K),os(e,n),fs(e,n,K),hs(e,n,vs,o),typeof Pn=="function"?Pn(e,n):console.warn("[Store] Debugger store not loaded"),n.on("change-locale",s=>{e.locale=s,localStorage.setItem("locale",s),window.i18n&&window.i18n.setLocale(s),n.emit("render")}),window.i18n?(window.i18n.setLocale(e.locale),console.log("[i18n] Locale initialized to:",e.locale)):console.warn("[i18n] i18n module not available")}const ws=45,bs=65,Cs=320;typeof window<"u"&&(window.PANEL_CLOSED=ws,window.PANEL_TOO_SMALL=bs,window.PANEL_DEFAULT=Cs);function xs(e,n){return e.isInitializing?b`
  ...
  `}async function On(){window.i18nReady&&(await window.i18nReady,console.log("[App] Translations loaded, starting app..."));let e=wt();e.use(ys),e.route("*",xs),e.mount("#app"),window.appState=e.state,window.appInstance=e,e.emitter.on("DOMContentLoaded",()=>{e.state.diskNavigationRoot&&e.emitter.emit("refresh-files");const n=new URLSearchParams(window.location.search),t=n.get("device");if(t){const i=`wss://${t}/webrepl`;localStorage.setItem("webrepl-url",i),console.log(`[App] Device URL from query param: ${i}`),setTimeout(()=>{e.emitter.emit("open-connection-dialog");const s=window.location.pathname;window.history.replaceState({},"",s)},500)}const o=n.get("configure");o&&setTimeout(()=>{e.emitter.emit("configure-scripto",o);const i=window.location.pathname;window.history.replaceState({},"",i)},500)})}document.readyState==="complete"?On():window.addEventListener("load",On);export{R as _};
