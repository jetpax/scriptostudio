...
`}),l+=`${n}_ds.st()
`,l}async function instrumentCodeForExec(n,i={}){const{watches:o={},conditionalBP:r={},fileName:s="main.py"}=i,a=performance.now(),c=await identifyCodeRows(n);console.log(`[Debugger] Identified ${c.size} code rows in ${performance.now()-a}ms`);let l=n.split(/\r?\n/);const u=new Map,d=o[""]||[],p=o[s]||[],h=[...new Set([...d,...p])],f=r[""]||[],m=r[s]||[],v=[...new Set([...f,...m])],y=Array.from(c.keys()).sort(($,b)=>$-b);for(const $ of y){const b=l[$],T=c.get($);if(b===void 0)continue;const x=/# ●/.test(b),I=i.breakpoints&&i.breakpoints[s]?i.breakpoints[s][$+1]:null;(x||I)&&console.log(`[Debugger] Breakpoint detected at line ${$+1}: ${b}`);const P=b.match(/^(\s*)/)[1];let k=generateDebugBlock(P,x,s,$+1,h,v,I);T==="function_definition"?k+=`${P}@_ds.wrap
`:T==="async_function_definition"&&(k+=`${P}@_ds.awrap
`),x&&console.log(`[Debugger] Generated instrumentation for row ${$+1} (${T})`),u.set($,k)}let w=[];for(let $=0;$<l.length;$++)u.has($)&&w.push(u.get($)),w.push(l[$]);const S=w.join(`
`),E=`${DEBUG_STATE_MODULE}

# Execute isolated code
_ds.exec("""${S.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/'/g,"\\'")}""")
`,C=performance.now()-a;return console.log(`[Debugger] Instrumentation complete in ${C.toFixed(0)}ms`),E}console.log("[Libs] ES modules loaded");function renderIcon(n,i={}){const{className:o="",size:r=24,color:s="currentColor"}=i;return html$1`
...
`}catch{return null}}toggleBreakpoint(i){if(!this.view)return;const o=this.view.state.doc.line(i+1),r=o.text,s=/# ●/.test(r);let a;s?a=r.replace(/\s*# ●.*/,""):a=r.trimEnd()+" # ●",this.view.dispatch({changes:{from:o.from,to:o.to,insert:a}})}syncBreakpointsFromStore(){if(!this.view||!this.fileName)return;const i=window.appState?.debugger?.breakpoints?.[this.fileName]||{},o=this.view.state.doc,r=[];for(let s=1;s<=o.lines;s++){const a=o.line(s),c=a.text,l=i[s],u=/# ●/.test(c);if(l&&!u)r.push({from:a.to,to:a.to,insert:" # ●"});else if(!l&&u){const d=c.match(/\s*# ●.*/);if(d){const p=a.from+c.indexOf(d[0]);r.push({from:p,to:a.to,insert:""})}}}r.length>0&&this.view.dispatch({changes:r})}}const editor=Object.freeze(Object.defineProperty({__proto__:null,CodeMirrorEditor},Symbol.toStringTag,{value:"Module"}));function Tab(n){const{text:i="undefined",icon:o="device-desktop",onSelectTab:r=()=>!1,onCloseTab:s=()=>!1,onStartRenaming:a=()=>!1,onFinishRenaming:c=()=>!1,active:l=!1,renaming:u=!1,hasChanges:d=!1}=n;if(l)if(u){let f=function(v){c(v.target.value)},m=function(v){v.key.toLowerCase()==="enter"&&v.target.blur(),v.key.toLowerCase()==="escape"&&(v.target.value=null,v.target.blur())};return html`
      ...
      `}else{let f=function(m){m.stopPropagation(),s(m)};return html`
        <div class="tab active" tabindex="0">
          ${IconSprite.renderIcon(o,{className:"icon"})}
      ...
      `}function p(f){f.target.classList.contains("close-tab")||r(f)}function h(f){f.stopPropagation(),s(f)}return html`
    <div
      class="tab"
      tabindex="1"
      onclick=${p}
      ...
      <div class="options close-tab">
        <button class="close-tab" onclick=${h}>
...
`)}}n.write(TERMINAL_PROMPT)}else if(r==="	"){if(i.isConnected&&o&&typeof o.requestCompletion=="function")try{const s=await o.requestCompletion(i.currentLine);if(!s||s.length===0){n.write("\x07");return}const a=d=>d&&d.length>=2&&/^[A-Z][A-Z0-9_]*$/.test(d),c=s.filter(d=>!a(d));if(c.length===0){n.write("\x07");return}let l="",u=[];if(c.length===1)l=c[0];else{const d=i.currentLine;let p=c[0];for(let h=1;h<c.length;h++){let f=0;for(;f<p.length&&f<c[h].length&&p[f]===c[h][f];)f++;p=p.slice(0,f)}p.length>d.length&&(l=p.slice(d.length)),u=c}if(l&&(i.currentLine=i.currentLine.slice(0,i.cursorPos)+l+i.currentLine.slice(i.cursorPos),i.cursorPos+=l.length,redrawLine(n,i.currentLine,i.cursorPos,o)),u.length>0){n.write(`\r
`);const d=80;let p="";for(const h of u){const f=p.length+h.length+(p?4:0);p&&f>d?(n.write(p+`\r
`),p=h):p=p?p+"    "+h:h}p&&n.write(p+`\r
...
`;if(n.isNewFileDialogOpen){const f=h.querySelector("#dialog-new-file .dialog-content > input");return f&&f.focus(),h}}function ScriptOsModal(n,i){return n.isScriptOsModalOpen?html`
  ...
  `:html`<div></div>`}function ScriptOsLibraryView(n,i){if(n.scriptOsCategoryCollapse||(n.scriptOsCategoryCollapse={}),n.isLoadingRegistry)return html`
    ...
    `;const o=f=>{if(f.slug){const m=f.tags?Object.keys(f.tags):[];return{name:f.displayName||f.slug,description:f.summary||"",tags:m,author:"",version:f.latestVersion?.version||"1.0.0"}}else return f.config&&f.config.info?{name:f.config.info.name||f.filename,description:f.config.info.description||"",tags:f.config.info.category?[f.config.info.category]:[],author:f.config.info.author||"",version:f.config.info.version||[1,0,0]}:{name:f.filename||"Unknown",description:"",tags:[],author:"",version:[1,0,0]}},r=new Set;n.scriptOsList.forEach(f=>{o(f).tags.forEach(v=>r.add(v))});const s=Array.from(r).sort(),a=(n.scriptOsSearchQuery||"").toLowerCase(),c=n.scriptOsFilterTags||[],l=n.scriptOsList.filter(f=>{const m=o(f),v=m.name.toLowerCase(),y=m.description.toLowerCase(),w=m.tags.join(" ").toLowerCase(),S=!a||v.includes(a)||y.includes(a)||w.includes(a),E=c.length===0||c.every(C=>m.tags.includes(C));return S&&E}),u={},d=[];l.forEach(f=>{const m=o(f),v=m.tags.length>0?m.tags[0]:null;v?(u[v]||(u[v]=[]),u[v].push(f)):d.push(f)});const p=Object.keys(u).sort(),h=f=>{n.scriptOsCategoryCollapse[f]=!n.scriptOsCategoryCollapse[f],i("render")};return html`
    <div class="scriptos-library">
          ...
          ${s.length>0?html`
            <div class="scriptos-tag-filters">
              ...
              <div class="scriptos-tag-filters-list">
                ${s.map(f=>{const m=c.includes(f);return html`
                    <button 
                      class="scriptos-tag-filter ${m?"active":""}"
                      onclick=${()=>i("scriptos-toggle-tag",f)}
                      title="${m?"Remove filter":"Filter by "+f}">
                      ${f}
                      ${m?html`<span class="scriptos-tag-check">✓</span>`:""}
                    </button>
                  `})}
              </div>
              ${c.length>0?html`
      ...
      <div class="scriptos-content">
        ...
        `:html`
          <div class="scriptos-categories">
            ${p.map(f=>{const m=n.scriptOsCategoryCollapse[f]===!0;return html`
                <div class="scriptos-category ${m?"collapsed":""}">
                  <h3 class="scriptos-category-title" onclick=${()=>h(f)}>
                    ...
                    <span>${f}</span>
                    <span class="scriptos-category-count">${u[f].length}</span>
                  </h3>
                  <div class="scriptos-grid" style="${m?"display: none;":""}">
                    ${u[f].map(v=>ScriptOsCard(v,i))}
                  </div>
                </div>
              `})}
            ${d.length>0?html`
              <div class="scriptos-category ${n.scriptOsCategoryCollapse.Other?"collapsed":""}">
                <h3 class="scriptos-category-title" onclick=${()=>h("Other")}>
                  <svg class="scriptos-category-chevron" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="6 9 12 15 18 9"/>
                  </svg>
                  <span>Other</span>
  ...
  `}function ScriptOsCard(n,i){let o,r,s,a,c;if(n.slug)o=n.displayName||n.slug,r=n.summary||"",a=n.tags?Object.keys(n.tags):[],s=n.latestVersion?.version?`v${n.latestVersion.version}`:"v1.0.0",c="";else if(n.config&&n.config.info){const l=n.config.info;o=l.name||n.filename,r=l.description||"",a=l.category?[l.category]:[],c=l.author||"",l.version?s=Array.isArray(l.version)?`v${l.version.join(".")}`:`v${l.version}`:s="v1.0.0"}else o=n.filename||"Unknown",r="",a=[],s="v1.0.0",c="";return html`
    <div 
      class="scriptos-card"
      ...
      <div class="scriptos-card-header">
        ...
        <h3>${o}</h3>
        <span class="scriptos-card-version">${s}</span>
      </div>
      ...
      ${a.length>0?html`
        <div class="scriptos-card-tags">
          ${a.slice(0,3).map(l=>html`<span class="scriptos-tag-badge">${l}</span>`)}
          ${a.length>3?html`<span class="scriptos-tag-badge">+${a.length-3}</span>`:""}
        </div>
      `:""}
  ...
  `}function ExtensionsLibraryView(n,i){const o=n.availableExtensions||[],r=n.installedExtensions||[],s=new Set(r.map(l=>l.id)),a=o.filter(l=>!s.has(l.id)),c=(l,u)=>{for(let d=0;d<3;d++){const p=l[d]||0,h=u[d]||0;if(p>h)return!0;if(p<h)return!1}return!1};return o.length===0?html`
      <div class="scriptos-library">
        <div class="scriptos-header-sticky">
           <div class="scriptos-header">
            <h2>Extensions</h2>
...
`;this.term.write(m),this.state.logs?.autoScroll!==!1&&this.term.scrollToBottom()}cleanup(){this.writeHandler&&(window.removeEventListener("log-terminal-write",this.writeHandler),this.writeHandler=null),this.clearHandler&&(window.removeEventListener("log-terminal-clear",this.clearHandler),this.clearHandler=null),this.resizeObserver&&(this.resizeObserver.disconnect(),this.resizeObserver=null),this.term&&(this.term.dispose(),this.term=null),this.fitAddon=null}}let logTerminalComponent=null;function LogSidebar(n,i){if(!n.logs.isOpen)return logTerminalComponent=null,null;logTerminalComponent||(logTerminalComponent=new LogTerminalComponent("log-terminal",n,i));const o=logTerminalComponent.render(n),r=n.logSidebarWidth||350;return html`
  ...
  `}const DiskFileList=generateFileList("disk"),BoardFileList=generateFileList("board");function generateFileList(n){return function(o,r){function s(f){f.key.toLowerCase()==="enter"&&f.target.blur(),f.key.toLowerCase()==="escape"&&(f.target.value=null,f.target.blur())}const a=html`
    ...
    `;function l(f,m){const v=html`
        <input type="text"
          ...
          ondblclick=${x=>!1}
          />
      `,y=o.selectedFiles.find(x=>x.fileName===f.fileName&&x.source===n);function w(x){return x.preventDefault(),r("rename-file",n,f),!1}function S(){o.renamingFile||r(`navigate-${n}-folder`,f.fileName)}function E(){o.renamingFile||r("open-file",n,f)}let C=f.fileName;const $=o.selectedFiles.find(x=>x.fileName===C);o.renamingFile==n&&$&&(C=v);function b(x){if(x==null)return"";if(x===0)return"0 B";const I=1024,P=["B","KB","MB","GB"],k=Math.floor(Math.log(x)/Math.log(I));return parseFloat((x/Math.pow(I,k)).toFixed(1))+" "+P[k]}const T=f.type==="file"?b(f.size):"";return f.type==="folder"?html`
          <div
        ...
        `}const u=o[`${n}Files`].sort((f,m)=>{const v=f.fileName.toUpperCase(),y=m.fileName.toUpperCase();if(f.type==="folder"&&m.type==="file")return-1;if(f.type===m.type){if(v<y)return-1;if(v>y)return 1}return 0}),d=html`<div class="item"
  ...
  ..
    ...
    `;return new MutationObserver(f=>{const m=p.querySelector("input");m&&m.focus()}).observe(p,{childList:!0,subtree:!0}),p}}function ReplPanel(n,i){const o=()=>{n.panelHeight>PANEL_CLOSED$1?i("close-panel"):i("open-panel")};n.isPanelOpen;const r=n.panelHeight>PANEL_TOO_SMALL$1?"visible":"hidden";let s="terminal-enabled";return(!n.isConnected||n.isNewFileDialogOpen)&&(s="terminal-disabled"),html$1`
  ...
  `:""}function EthernetPanel(n,i){if(!n.networksInfo&&n.isConnected&&!n.isLoadingNetworks&&i("refresh-networks"),n.isConnected&&!n.ethConfigLoaded&&!n.isLoadingEthConfig&&i("load-eth-config"),n.isConnected&&!n.networkInterfacesConfig&&!n.isLoadingNetworkInterfacesConfig&&i("load-network-interfaces-config"),!n.networksInfo)return html`
    ...
    `;const o=n.networksInfo.eth,r=n.ethStatus,s=r&&r.initialized?r:o&&o.mac?o:r||o,a=n.ethConfig||{},c=o!==null,l=n.networkInterfacesConfig?.ethernet===!1,u=s&&(s.mac||s.enabled||s.enable||s.initialized),d=s&&s.ip&&s.ip!=="0.0.0.0",p=s&&s.linkup===!0,h=d;if(!c)return html`
      <div class="panel-container">
        <div class="panel-header">
          <h2>Ethernet Configuration</h2>
        </div>
    ...
    `;const f=l?"status-disabled":h?"status-active":p?"status-warning":"status-inactive",m=l?"Disabled":h?"Connected":p?"Link Up (No IP)":u?"No Link":"Not Initialized";return html`
    <div class="panel-container">
      <div class="panel-header">
        <h2>Ethernet Configuration</h2>
      </div>
  ...
  `}function VPNPanel(n,i){n.isConnected&&!n.vpnConfigLoaded&&!n.isLoadingVpnConfig&&i("load-vpn-config");const o=n.vpnConfig||{hostname:"",join_code:"",auto_connect:!1},r=n.networksInfo?.vpn,s=n.isConnected,a=n.networksInfo&&r!==void 0;!a||r.available;const c=a&&r.active;return html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${l=>{l.preventDefault();const u=new FormData(l.target),d=u.get("hostname")||"",p=u.get("join_code")||"",h=u.get("auto_connect")==="on";if(!d.trim()){alert("Please enter a hostname for this device");return}if(!p.trim()){alert("Please enter a Husarnet join code");return}i("vpn-connect",{hostname:d.trim(),join_code:p.trim(),auto_connect:h})}}>
          <div class="form-group">
  ...
  `}function NTPPanel(n,i){n.isConnected&&!n.ntpConfigLoaded&&!n.isLoadingNtpConfig&&i("load-ntp-config");const o=n.ntpConfig||{server:"pool.ntp.org",tzOffset:0,timezone:"UTC",autoDetect:!1},r=n.ntpSyncResult||null,s=d=>{if(!d)return"--:--:--";const{year:p,month:h,day:f,hour:m,minute:v,second:y}=d;return`${String(m).padStart(2,"0")}:${String(v).padStart(2,"0")}:${String(y).padStart(2,"0")}`},a=d=>{if(!d)return"";const{year:p,month:h,day:f}=d;return`${p}-${String(h).padStart(2,"0")}-${String(f).padStart(2,"0")}`},c=[{value:"UTC",offset:0,label:"UTC (Coordinated Universal Time)"},{value:"EST",offset:-5,label:"EST (Eastern Standard Time)"},{value:"CST",offset:-6,label:"CST (Central Standard Time)"},{value:"MST",offset:-7,label:"MST (Mountain Standard Time)"},{value:"PST",offset:-8,label:"PST (Pacific Standard Time)"},{value:"GMT",offset:0,label:"GMT (Greenwich Mean Time)"},{value:"CET",offset:1,label:"CET (Central European Time)"},{value:"EET",offset:2,label:"EET (Eastern European Time)"},{value:"JST",offset:9,label:"JST (Japan Standard Time)"},{value:"AEST",offset:10,label:"AEST (Australian Eastern Standard Time)"}],l=o.timezone,u=[...c];return l&&!c.find(d=>d.value===l)&&u.unshift({value:l,offset:o.tzOffset||0,label:`${l} (Detected)`}),html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${async d=>{d.preventDefault();const p=new FormData(d.target),h=p.get("server")||"pool.ntp.org",f=p.get("timezone")||"UTC",m=p.get("auto_detect")==="on",v=u.find(S=>S.value===f),y=v?v.offset:0,w={server:h,tz_offset:y,timezone:f,auto_detect:m};try{await new Promise((S,E)=>{const C=setTimeout(()=>E(new Error("Save timeout")),1e4),$=()=>{clearTimeout(C),window.appInstance.emitter.removeListener("ntp-config-saved",b),window.appInstance.emitter.removeListener("ntp-config-save-error",T)},b=()=>{$(),S()},T=x=>{$(),E(x)};window.appInstance.emitter.once("ntp-config-saved",b),window.appInstance.emitter.once("ntp-config-save-error",T),i("save-ntp-config",w)}),i("sync-ntp-time",h,y,m)}catch(S){console.error("[NTP] Failed to save config before sync:",S),alert(`Failed to save NTP configuration: ${S.message}`)}}}>
          <div class="form-group">
    ...
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <div class="status-display">
          <span class="status-indicator ${h}"></span>
...
`)).match(/CREDS:(\{.*\})/);if(o){const r=JSON.parse(o[1]);panelState.credentials={hostname:r.h+".local",password:r.p,ssid:r.s},addLog(`Found: ${r.h}.local`,"success")}else throw new Error("Could not read settings - device may need firmware");triggerRerender()}catch(n){addLog(`Error: ${n.message}`,"error"),triggerRerender()}}async function performWifiSetup(){clearLog();try{let l=function(u){return new Promise((d,p)=>{const h=document.createElement("div");h.className="fw-modal-overlay active",h.innerHTML=`
        ...
        `,document.body.appendChild(h);const f=y=>{const w=y.target.closest(".fw-modal-network-item");if(w){const S=parseInt(w.dataset.index);v(),d(S)}},m=()=>{v(),p(new Error("Setup cancelled"))},v=()=>{h.remove()};h.querySelector(".fw-network-modal-list").addEventListener("click",f),h.querySelector(".fw-modal-cancel").addEventListener("click",m)})};addLog("Connecting to REPL...");const n=await transitionToREPL();if(!n.success){await handleBootFailure(n.error);return}addLog("Scanning for WiFi networks...");const o=(await sendREPLCommand(currentPort,`
import network
import json
...
`,15e3,!1)).match(/CONNECTED:(\d+\.\d+\.\d+\.\d+)/);if(f){a=f[1],s=!0,addLog(`Connected! IP: ${a}`),addLog("Saving settings...");const v=`pybot-${panelState.deviceInfo?.mac?panelState.deviceInfo.mac.replace(/:/g,"").slice(-4).toLowerCase():"xxxx"}`;if(await sendREPLCommand(currentPort,`
from lib.sys import settings
...
Click OK to try again, or Cancel to abort.`))throw new Error("Connection cancelled")}}catch(n){addLog(`Error: ${n.message}`,"error"),triggerRerender()}}async function continueToProvision(n){try{const i=n+".local";addLog("Generating HTTPS certificate..."),window.forge||await new Promise((c,l)=>{const u=document.createElement("script");u.src="https://cdn.jsdelivr.net/npm/node-forge@1.3.1/dist/forge.min.js",u.onload=c,u.onerror=()=>l(new Error("Failed to load forge")),document.head.appendChild(u)}),addLog(`Hostname: ${i}`);const{certPem:o,keyPem:r}=generateSelfSignedCert(i);if(addLog(`Certificate generated (${o.length} bytes)`),addLog("Installing certificate..."),await uploadCertsToDevice(currentPort,o,r),panelState.selectedBoard)try{addLog(`Installing board config: ${panelState.selectedBoard.name}...`);const c=await fetchBoardManifest(panelState.selectedBoard.id),l=JSON.stringify(c).replace(/\\/g,"\\\\").replace(/'/g,"\\'");await sendREPLCommand(currentPort,`
import json
try:
    import os
    os.mkdir('/settings')
...
with open('/settings/board.json', 'w') as f:
    f.write('${l}')
print("BOARD_OK")
`,5e3,!1),addLog("Board config installed","success")}catch(c){addLog(`Warning: Could not install board config: ${c.message}`,"error")}const s=panelState.selectedFirmware?.version||"unknown";addLog(`Firmware version: ${s}`),addLog("Enabling setup mode..."),await sendREPLCommand(currentPort,`
from lib.sys import settings
        ...
        ${a?html$1`
          ${panelState.firmwareOptions.length>1?html$1`
            <div class="fw-form-group">
              <select class="fw-select" onchange=${c=>{const l=panelState.firmwareOptions.find(u=>u.id===c.target.value);l&&(panelState.selectedFirmware=l,triggerRerender())}}>
                ${panelState.firmwareOptions.map(c=>html$1`
                  <option value="${c.id}" selected=${c.id===a.id}>
                    ${c.displayName}
                  </option>
                `)}
              </select>
          ...
          ${panelState.boardOptions.length>0?html$1`
            <div class="fw-form-group" style="margin-top: 1rem;">
              ...
              <select class="fw-select" onchange=${c=>{const l=panelState.boardOptions.find(u=>u.id===c.target.value);l&&(panelState.selectedBoard=l,triggerRerender())}}>
                ${panelState.boardOptions.map(c=>html$1`
                  <option value="${c.id}" selected=${panelState.selectedBoard?.id===c.id}>
                    ${c.name}
                  </option>
                `)}
              </select>
...
`)){const a=s.match(/^(\w+)\s*:\s*(.+)$/);a&&(i.meta[a[1].trim()]=a[2].trim())}i.body=n.slice(o[0].length)}return i.body=i.body.replace(/^ScriptO for\s+\[.*?\]\(.*?\)\.?\s*Run on device\s*\(MicroPython\)\.?\s*\n?/im,""),i.body=i.body.trim(),i}function SkillSidebar(n,i){if(!n.isSkillSidebarOpen||!n.skillSidebarContent)return html`<div></div>`;const{name:o,skillMd:r}=n.skillSidebarContent,{meta:s,body:a}=parseSkillMd(r);let c="";try{a&&(c=g.parse(a,{breaks:!0}))}catch(h){console.error("[SkillSidebar] Failed to parse markdown:",h),c=`<pre>${a}</pre>`}const l=[];s.description&&l.push(`<p class="skill-meta-description">${s.description}</p>`);const u=[];s.category&&u.push(`<span class="skill-meta-tag">${s.category}</span>`),s.author&&u.push(`<span class="skill-meta-tag">by ${s.author}</span>`),u.length&&l.push(`<div class="skill-meta-tags">${u.join("")}</div>`);const d=l.length?`<div class="skill-meta-block">${l.join("")}</div>`:"",p=document.createElement("div");return p.className="skill-sidebar-content skill-md-content",p.innerHTML=d+c,html`
  ...
  `;return html$1`
    ...
    ${ExtensionsModal(n,i)}
    ${BreakpointModal(n,i)}
  `}function SystemSidebar(n,i){const o=window.i18n?window.i18n.t:b=>b,r=[{id:"home",label:o("sidebar.home"),icon:"home"}],s=[{id:"editor",label:o("sidebar.editor"),icon:"code"},{id:"file-manager",label:o("sidebar.files"),icon:"folder"}],a=[{id:"about",label:o("sidebar.about"),icon:"info-circle"},{id:"settings",label:o("sidebar.settings"),icon:"adjustments-alt"},{id:"ai-agent",label:o("sidebar.aiAgent"),icon:"robot-face"},{id:"firmware",label:o("sidebar.firmware"),icon:"file-download"}],c=[{id:"wifi",label:o("sidebar.wifi"),icon:"wifi"},{id:"ethernet",label:o("sidebar.ethernet"),icon:"cloud-network"},{id:"vpn",label:o("sidebar.vpn"),icon:"shield-chevron"},{id:"btle",label:o("sidebar.btle"),icon:"bluetooth"},{id:"wwan",label:o("sidebar.wwan"),icon:"cell"},{id:"mqtt",label:o("sidebar.mqtt"),icon:"message-2"},{id:"ntp",label:o("sidebar.ntp"),icon:"clock-cog"},{id:"can",label:o("sidebar.can"),icon:"car-crash"}],l=[{id:"gps",label:o("sidebar.gps"),icon:"gps"},{id:"4g-modem",label:o("sidebar.modem"),icon:"signal-4g"},{id:"sdcard",label:o("sidebar.sdcard"),icon:"device-sd-card"}],u=r.map(b=>renderSidebarItem(b,n,i)),d=s.map(b=>renderSidebarItem(b,n,i)),p=n.expandedSystem!==!1,h=html$1`
    <div class="system-sidebar-extension">
      ...
      ${p?html$1`
        <div class="system-sidebar-submenu">
          ${a.map(b=>{const T=n.activeSystemPanel===b.id;return html$1`
              <div 
                class="system-sidebar-subitem ${T?"active":""}"
                onclick=${()=>i("change-system-panel",b.id)}
              >
  ...
  `,f=n.expandedNetworks!==!1,m=html$1`
    <div class="system-sidebar-extension">
      <div 
        class="system-sidebar-item system-sidebar-toggle"
      ...
      ${f?html$1`
        <div class="system-sidebar-submenu">
          ${c.map(b=>{const T=n.activeNetworkPanel===b.id;return html$1`
              <div 
                class="system-sidebar-subitem ${T?"active":""}"
                onclick=${()=>i("change-network-panel",b.id)}
              >
                ${IconSprite.renderIcon(b.icon,{className:"",size:16})}
                <span>${b.label}</span>
              </div>
  ...
  `,v=n.expandedPeripherals!==!1,y=html$1`
    <div class="system-sidebar-extension">
      ...
      ${v?html$1`
        <div class="system-sidebar-submenu">
          ${l.map(b=>{const T=n.activePeripheralsPanel===b.id;return html$1`
              <div 
                class="system-sidebar-subitem ${T?"active":""}"
                onclick=${()=>i("change-peripherals-panel",b.id)}
              >
                ${IconSprite.renderIcon(b.icon,{className:"",size:16})}
                <span>${b.label}</span>
              </div>
  ...
  `,w=b=>{if(b.iconSvg)try{const P=new DOMParser().parseFromString(b.iconSvg,"image/svg+xml").querySelector("svg");if(P){const k=P.getAttribute("viewBox")||"0 0 24 24",_=Array.from(P.querySelectorAll("path"));return html$1`
            <svg 
            ...
            >
              ${_.map(N=>{const O=N.getAttribute("d")||"",F=N.getAttribute("fill")||"none",D=N.getAttribute("stroke");return O==="M0 0h24v24H0z"||D==="none"?html$1`<path d="${O}" stroke="none" fill="none" />`:html$1`<path d="${O}" stroke="currentColor" fill="${F}" />`})}
            </svg>
          `}}catch(x){console.warn("[System] Failed to parse extension icon SVG:",x)}const T=b.icon||"settings";return IconSprite.renderIcon(T,{className:"",size:20})},S=(n.installedExtensions||[]).map(b=>{const T=n.expandedExtensions[b.id],x=n.activeExtension===b.id,I=b.devices===!0;let P=null;if(I&&T)if(!n.loadedExtensions[b.id])console.warn("[System] Extension not loaded:",b.id);else{const k=n.loadedExtensions[b.id];if(!k.instance)try{const _=k.data.content,O=new Function("DeviceAPI","html","emit","state",`
              ${_}
            ...
            `)(DeviceAPI,html$1,i,n),F=new DeviceAPI(BridgeDevice),D=new O(F,i,n,html$1);n.loadedExtensions[b.id].instance=D}catch(_){console.error(`[System] Error instantiating extension ${b.id}:`,_)}if(k.instance&&typeof k.instance.getMenuItems=="function")try{const _=k.instance.getMenuItems();_&&Array.isArray(_)&&(b.menu=_)}catch(_){console.error(`[System] Error getting menu items for ${b.id}:`,_)}if(k.instance&&typeof k.instance.renderSidebarDevices=="function")try{P=k.instance.renderSidebarDevices()}catch(_){console.error(`[System] Error rendering sidebar devices for ${b.id}:`,_)}else console.warn(`[System] renderSidebarDevices not available for ${b.id}`)}if(T&&n.loadedExtensions[b.id]?.instance){const k=n.loadedExtensions[b.id];if(k.instance&&typeof k.instance.getMenuItems=="function")try{const _=k.instance.getMenuItems();_&&Array.isArray(_)&&(b.menu=_)}catch(_){console.error(`[System] Error getting menu items for ${b.id}:`,_)}}return html$1`
      <div class="system-sidebar-extension">
        <div 
          class="system-sidebar-item system-sidebar-toggle ${x?"active-extension":""}"
          onclick=${()=>i("toggle-extension-menu",b.id)}
        >
          ${w(b)}
          <span>${b.name}</span>
          ${IconSprite.renderIcon("chevron-down",{className:`expand-icon ${T?"expanded":""}`,size:16})}
        </div>
        
        ${T?html$1`
          <div class="system-sidebar-submenu">
            ${b.menu.map(k=>{const _=n.activeExtension===b.id&&n.activeExtensionPanel===k.id,N=k.disabled===!0;return html$1`
                <div 
                  class="system-sidebar-subitem ${_?"active":""} ${N?"disabled":""}"
                  onclick=${N?null:()=>i("change-extension-panel",{extensionId:b.id,panelId:k.id})}
...
print('Hello, ')
print('ScriptO!') # ●
`;async function sleep(n){return new Promise(i=>setTimeout(i,n))}async function confirmDialog(n,i,o){return confirm(n)?0:1}function updateStatusBarDirectly(n,i){const o=document.getElementById("status-bar");if(!o)return;const r=i||localStorage.getItem("temperatureUnit")||"degC",s=buildStatusBarModel?buildStatusBarModel(n,r):null;if(!s||!s.connected){o.className="disconnected";const h=o.querySelector(".status-bar-center");h?h.textContent=s&&s.disconnectedText||DISCONNECTED_STATUS_TEXT:o.textContent=s&&s.disconnectedText||DISCONNECTED_STATUS_TEXT;return}o.className="";const a=o.querySelector(".status-bar-center")||o;function c(h){let f=a.querySelector(`.status-item.${h}`);if(!f){f=document.createElement("div"),f.className=`status-item ${h}`;const m=document.createElement("span");m.className="status-label";const v=document.createElement("span");v.className="status-value",f.appendChild(m),f.appendChild(v),a.appendChild(f)}return f}const l=c("ram");l.querySelector(".status-label").textContent="RAM",l.querySelector(".status-value").textContent=s.ram;const u=a.querySelector(".status-item.temp");if(s.temp){const h=c("temp");h.querySelector(".status-label").textContent="TEMP",h.querySelector(".status-value").textContent=s.temp}else u&&u.remove();const d=c("uptime");d.querySelector(".status-label").textContent="UPTIME",d.querySelector(".status-value").textContent=s.uptime;const p=a.querySelector(".status-item.wifi-rssi");if(s.rssi){const h=c("wifi-rssi");h.querySelector(".status-label").textContent="RSSI",h.querySelector(".status-value").textContent=s.rssi}else p&&p.remove()}function updateOverlayDirectly(n){const i=document.getElementById("overlay");if(!i)return;let o=!1,r="";if(n.diskFiles==null)o=!0,r="<p>Loading files...</p>";else if(n.isRemoving)o=!0,r="<p>Removing...</p>";else if(n.isConnecting)o=!0,r="<p>Connecting...</p>";else if(n.isLoadingFiles)o=!0,r="<p>Loading files...</p>";else if(n.isSaving)o=!0,r=`<p>Saving file... ${n.savingProgress||0}</p>`;else if(n.isTransferring){o=!0;const s=String(n.transferringProgress||""),a=s.match(/(\d+)%?$/),c=a?parseInt(a[1]):0,l=s.match(/^(.+?):/);r=`
      <div class="transfer-overlay-content">
        <div class="transfer-title">Transferring File</div>
...
`,u.forEach(m=>d+=`${m}
`),d+=`
`),d+="Are you sure you want to proceed?",await confirmDialog(d)!==0){i.emit("render");return}let h=!1,f=!1;for(let m in n.selectedFiles){const v=n.selectedFiles[m];v.type=="folder"?v.source==="board"?(await removeBoardFolder(getFullPath(n.boardNavigationRoot,n.boardNavigationPath,v.fileName)),h=!0):(await r.removeFolder(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,v.fileName)),f=!0):v.source==="board"?(await deleteFile(o,getFullPath(n.boardNavigationRoot,n.boardNavigationPath,v.fileName)),h=!0):(await r.removeFile(r.getFullPath(n.diskNavigationRoot,n.diskNavigationPath,v.fileName)),f=!0)}h&&f?i.emit("refresh-files"):h?i.emit("refresh-board-files"):f&&i.emit("refresh-disk-files"),n.selectedFiles=[],i.emit("render")}),i.on("rename-file",(l,u)=>{console.log("rename-file",l,u),n.renamingFile=l,i.emit("render")}),i.on("finish-renaming-file",async l=>{console.log("finish-renaming-file",l);const u=n.selectedFiles[0];if(!l||u.fileName==l){n.renamingFile=null,i.emit("render");return}if(n.isSaving=!0,updateOverlayDirectly(n),n.renamingFile=="board"&&n.isConnected){if((await checkOverwrite({fileNames:[l],parentPath:r.getFullPath(n.boardNavigationRoot,n.boardNavigationPath,""),source:"board"})).length>0){let p=`You are about to overwrite the following file/folder on your board:
...
${l.message}`)}});async function a(c,l,u,d,p){const h=c.id,f=Array.isArray(c.version)?c.version.join("."):String(c.version);s(`[Extensions] Running onInstall for ${c.name}...`),showStyledModal({variant:"",icon:"packages",title:"Installing Extension",subtitle:c.name,body:`<p>Installing extension files to device...</p>
             <div style="margin-top: 16px; color: var(--text-secondary);">
               <span class="install-spinner">◐</span> Preparing...
             </div>`,buttons:[]});try{stopStatusInfoPolling();const m=(y,...w)=>y.reduce((S,E,C)=>S+E+(w[C]||""),""),v=instantiateExtension(l.content,u,d.emit.bind(d),m);if(typeof v.onInstall=="function"){updateModalBody(`<p>Writing files to device...</p>
          <div style="margin-top: 16px; color: var(--text-secondary);">
