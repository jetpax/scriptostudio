...
`,C=performance.now()-a;return console.log(`[Debugger] Instrumentation complete in ${C.toFixed(0)}ms`),x}console.log("[Libs] ES modules loaded");function renderIcon(n,i={}){const{className:o="",size:r=24,color:s="currentColor"}=i;return html$1`
    ...
    <div class="panel-section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <span class="info-value info-mono">${"8ed1159"}</span>
        </div>
        <div class="info-item">
          <span class="info-label">${n("sysinfo.studioBuildDate")}</span>
          <span class="info-value">${"2026-03-02"}</span>
        </div>
      </div>
    </div>
...
`,5e3,!1),addLog("Certificates installed!")}function FirmwarePanel(n,i){appState=n;const o=panelState.view;return html$1`
  ...
  `}function getChipFeatures(n){const i={"ESP32-P4":["High-Performance MCU","RISC-V Dual Core"],"ESP32-S3":["WiFi","BLE 5.0","USB OTG"],"ESP32-S2":["WiFi","USB OTG"],"ESP32-C3":["WiFi","BLE 5.0","RISC-V"],"ESP32-C6":["WiFi 6","BLE 5.0","Thread"],ESP32:["WiFi","Bluetooth Classic","BLE"]};for(const[o,r]of Object.entries(i))if(n?.includes(o))return r;return["Microcontroller"]}let extensionDisplayNames={},extensionNamesFetched=!1;async function fetchExtensionDisplayName(n){if(!extensionNamesFetched){extensionNamesFetched=!0;try{const i=await fetch("https://cheerful-stoat-894.convex.site/api/v1/extensions");if(!i.ok)return;const o=await i.json();for(const r of o.extensions||[])extensionDisplayNames[r.id]=r.name;triggerRerender()}catch(i){console.warn("[firmware-panel] Could not fetch extension names:",i)}}}function renderConnectView(n,i){const o=sessionStorage.getItem("extension-install-active");if(o){extensionNamesFetched||fetchExtensionDisplayName();const r=extensionDisplayNames[o]||o.toUpperCase();return html$1`
      <div class="fw-view active" style="align-items: center; justify-content: center;">
        <div class="fw-ext-card">
          ...
          <h2 class="fw-ext-title">Installing ${r}</h2>
          ...
          <p class="fw-ext-note">
            We'll detect your hardware, flash firmware, and install the <strong>${r}</strong> extension on your ESP32 device.
          </p>
        ...
        ${s?html$1`
          ...
          `:html$1`
            <div class="fw-firmware-info">
              <p style="font-weight: 500;">${s.displayName} <span style="font-size: 0.8rem; color: var(--text-secondary);">
...
${l.message}`)}});async function a(c,l,d,u,p){const f=c.id,h=Array.isArray(c.version)?c.version.join("."):String(c.version);s(`[Extensions] Running onInstall for ${c.name}...`),showStyledModal({variant:"",icon:"packages",title:"Installing Extension",subtitle:c.name,body:`<p>Installing extension files to device...</p>
             ...
             </div>`,buttons:[]});try{stopStatusInfoPolling();const m=(y,...w)=>y.reduce((b,x,C)=>b+x+(w[C]||""),""),v=instantiateExtension(l.content,d,u.emit.bind(u),m);if(typeof v.onInstall=="function"){updateModalBody(`<p>Writing files to device...</p>
          <div style="margin-top: 16px; color: var(--text-secondary);">
...
${c.message}`)}})}async function getSkillVersionFiles(n,i,o){const r=i.latestVersion?.version;if(!r)throw new Error(`Skill "${n}" has no version`);const s=await fetch(`${o}/api/v1/skills/${encodeURIComponent(n)}/versions/${encodeURIComponent(r)}`);if(!s.ok)throw new Error(`Failed to fetch version files: ${s.status}`);const a=await s.json();return a.version||a}console.log("[Stores] ES modules loaded");let CodeMirrorEditorClass=null;async function getCodeMirrorEditor(){return CodeMirrorEditorClass||(CodeMirrorEditorClass=(await __vitePreload(()=>Promise.resolve().then(()=>editor),void 0)).CodeMirrorEditor),CodeMirrorEditorClass}const log=console.log,device=BridgeDevice,disk=BridgeDisk,registryCache=new RegistryCache;async function store(n,i){async function o(a){const{source:c,parentFolder:l,fileName:d,content:u=newFileContent,hasChanges:p=!1}=a,f=generateHash(),h=await getCodeMirrorEditor(),m=n.cache(h,`editor_${f}`);return m.content=u,m.fileName=d,{id:f,source:c,parentFolder:l,fileName:d,editor:m,hasChanges:p}}async function r(a,c=null,l=null,d=null){a=="board"?n.boardNavigationPath:n.diskNavigationPath;const u=await o({fileName:c===null?generateFileName():c,parentFolder:l,source:a,hasChanges:!0,content:d||newFileContent});let p=!1;return l!=null&&(a=="board"?p=await fileExists(BridgeDevice,getFullPath$1(n.boardNavigationRoot,u.parentFolder,u.fileName)):a=="disk"&&(p=await disk.fileExists(disk.getFullPath(n.diskNavigationRoot,u.parentFolder,u.fileName)))),n.openFiles.find(h=>h.parentFolder===u.parentFolder&&h.fileName===u.fileName&&h.source===u.source)||p?(await confirmDialog(`File ${u.fileName} already exists on ${a}. Please choose another name.`),!1):(u.editor.onChange=function(){u.hasChanges=!0,i.emit("render")},n.openFiles.push(u),n.editingFile=u.id,!0)}await initializeState(n,i,r),initializeTheme(n,i),registerThemeHandlers(n,i);const s=()=>{};registerDialogHandlers(n,i,device),registerConnectionHandlers(n,i,device,s,bindTerminalOutput),registerFileOperationHandlers(n,i,device,disk,o,r),registerTerminalHandlers(n,i,device,XTerm),registerAIAgentHandlers(n,i,r),registerLogHandlers(n,i),document.addEventListener("connection-mode-change",a=>{const c=a.detail?.mode||"none";n.connectionMode=c,i.emit("render")}),document.addEventListener("firmware-panel-update",()=>{i.emit("render")}),i.on("navigate",a=>{i.emit("change-system-section",a)}),i.on("change-view",async a=>{a==="file-manager"&&(n.isConnected&&BridgeDevice&&BridgeDevice.isCommandRunning()&&(i.emit("stop"),await sleep(250)),n.filesLoadedOnce||(log("[File Manager] Loading files for first time..."),n.filesLoadedOnce=!0),n.isConnected?i.emit("refresh-files"):i.emit("refresh-disk-files")),n.systemSection!==a&&(n.selectedFiles=[],n.systemSection=a,n.activeNetworkPanel=null,n.activePeripheralsPanel=null,n.activeSystemPanel=null,n.activeExtension=null,n.activeExtensionPanel=null,a==="editor"&&n.isConnected&&setTimeout(()=>{bindTerminalOutput(n);const c=n.cache(XTerm,"terminal");c&&c.term&&(i.emit("bind-terminal-input"),i.emit("terminal-focus"))},100),i.emit("render"))}),i.on("launch-app",async(a,c)=>{window.launchApp(a,c)}),i.on("change-system-section",a=>{if(log("change-system-section",a),n.systemSection=a,a.startsWith("network:")){const c=a.split(":")[1];n.activeNetworkPanel=c,n.activeSystemPanel=null,n.activePeripheralsPanel=null,n.activeExtension=null,n.activeExtensionPanel=null}else if(a.startsWith("peripherals:")){const c=a.split(":")[1];n.activePeripheralsPanel=c,n.activeNetworkPanel=null,n.activeSystemPanel=null,n.activeExtension=null,n.activeExtensionPanel=null}else if(a.startsWith("system:")){const c=a.split(":")[1];n.activeSystemPanel=c,n.activeNetworkPanel=null,n.activePeripheralsPanel=null,n.activeExtension=null,n.activeExtensionPanel=null}else n.activeNetworkPanel=null,n.activePeripheralsPanel=null,n.activeSystemPanel=null,n.activeExtension=null,n.activeExtensionPanel=null;(a==="ai-agent"||a==="system:ai-agent")&&n.aiAgent.settings.provider==="openrouter"&&n.aiAgent.settings.apiKey&&n.aiAgent.openRouterModels.length===0&&i.emit("ai-fetch-openrouter-models"),i.emit("render")}),registerNetworkConfigHandlers(n,i,device),registerHardwareConfigHandlers(n,i,device),registerSystemConfigHandlers(n,i,device),registerExtensionHandlers(n,i,device),registerScriptOsHandlers(n,i,registryCache,r),typeof debuggerStore=="function"?debuggerStore(n,i):console.warn("[Store] Debugger store not loaded"),i.on("change-locale",a=>{n.locale=a,localStorage.setItem("locale",a),window.i18n&&window.i18n.setLocale(a),i.emit("render")}),window.i18n?(window.i18n.setLocale(n.locale),console.log("[i18n] Locale initialized to:",n.locale)):console.warn("[i18n] i18n module not available")}const PANEL_CLOSED=45,PANEL_TOO_SMALL=65,PANEL_DEFAULT=320;typeof window<"u"&&(window.PANEL_CLOSED=PANEL_CLOSED,window.PANEL_TOO_SMALL=PANEL_TOO_SMALL,window.PANEL_DEFAULT=PANEL_DEFAULT);function App(n,i){return n.isInitializing?html$1`
  ...
  `}async function initApp(){window.i18nReady&&(await window.i18nReady,console.log("[App] Translations loaded, starting app...")),console.log("%c ScriptO Studio v1.0.0 ","background: #9b59b6; color: #fff; font-size: 11px; padding: 2px 6px; border-radius: 3px;","build 8ed1159 (2026-03-02)");let n=Choo();n.use(store),n.route("*",App),n.mount("#app"),window.appState=n.state,window.appInstance=n,n.emitter.on("DOMContentLoaded",()=>{n.state.diskNavigationRoot&&n.emitter.emit("refresh-files");const i=new URLSearchParams(window.location.search),o=i.get("device");if(o){const c=`wss://${o}/webrepl`;localStorage.setItem("webrepl-url",c),console.log(`[App] Device URL from query param: ${c}`),setTimeout(()=>{n.emitter.emit("open-connection-dialog");const l=window.location.pathname;window.history.replaceState({},"",l)},500)}const r=i.get("configure");r&&setTimeout(()=>{n.emitter.emit("configure-scripto",r);const c=window.location.pathname;window.history.replaceState({},"",c)},500);const s=i.get("extension");if(s){localStorage.setItem("pending-extension-id",s),sessionStorage.setItem("extension-install-active",s),console.log(`[App] Extension from query param: ${s}`),i.delete("extension");const c=i.toString(),l=window.location.pathname+(c?"?"+c:"");window.history.replaceState({},"",l),o||setTimeout(()=>{console.log("[App] Fresh extension install — navigating to Device Setup"),n.emitter.emit("change-system-panel","firmware")},500)}const a=localStorage.getItem("pending-extension-id");if(a&&o&&setTimeout(async()=>{try{const d=(await new ExtensionRegistry().getInstalledExtensions()).find(u=>u.id===a);d&&d.menu&&d.menu.length>0&&(console.log(`[App] Auto-navigating to extension: ${a}`),localStorage.removeItem("pending-extension-id"),n.emitter.emit("change-extension-panel",{extensionId:a,panelId:d.menu[0].id}))}catch(c){console.warn("[App] Could not auto-navigate to extension:",c)}},1e3),!n.state.isConnected){let c=document.getElementById("hand-click-hint");if(!c){c=document.createElement("div"),c.id="hand-click-hint";const u=document.createElementNS("http://www.w3.org/2000/svg","svg");u.setAttribute("width","36"),u.setAttribute("height","36"),u.setAttribute("viewBox","0 0 24 24"),u.setAttribute("fill","none"),u.setAttribute("stroke","currentColor"),u.setAttribute("stroke-width","2"),u.setAttribute("stroke-linecap","round"),u.setAttribute("stroke-linejoin","round"),u.innerHTML=`
          ...
          <path d="M14 3l1 -1" />
          <path d="M15 6h1" />
        `,c.appendChild(u),document.body.appendChild(c)}const l=()=>{c.classList.add("animate"),setTimeout(()=>{c.classList.remove("animate")},7500)};setTimeout(l,1e3);const d=setInterval(l,1e4);n.emitter.on("connect",()=>{clearInterval(d);const u=document.getElementById("hand-click-hint");u&&u.remove()})}})}document.readyState==="complete"?initApp():window.addEventListener("load",initApp);export{__vitePreload as _};
