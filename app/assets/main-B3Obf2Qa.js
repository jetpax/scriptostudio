...
`}),a+=`${e}_ds.st()
`,a}async function instrumentCodeForExec(e,n={}){const{watches:i={},conditionalBP:s={},fileName:o="main.py"}=n,r=performance.now(),c=await identifyCodeRows(e);console.log(`[Debugger] Identified ${c.size} code rows in ${performance.now()-r}ms`);let a=e.split(/\r?\n/);const d=new Map,l=i[""]||[],p=i[o]||[],u=[...new Set([...l,...p])],f=s[""]||[],h=s[o]||[],m=[...new Set([...f,...h])],v=Array.from(c.keys()).sort((k,b)=>k-b);for(const k of v){const b=a[k],$=c.get(k);if(b===void 0)continue;const x=/# ●/.test(b),I=n.breakpoints&&n.breakpoints[o]?n.breakpoints[o][k+1]:null;(x||I)&&console.log(`[Debugger] Breakpoint detected at line ${k+1}: ${b}`);const T=b.match(/^(\s*)/)[1];let E=generateDebugBlock(T,x,o,k+1,u,m,I);$==="function_definition"?E+=`${T}@_ds.wrap
...
`;if(e.isNewFileDialogOpen){const f=u.querySelector("#dialog-new-file .dialog-content > input");return f&&f.focus(),u}}function ScriptOsModal(e,n){return e.isScriptOsModalOpen?html`
  ...
  `}function ExtensionsLibraryView(e,n){const i=e.availableExtensions||[],s=e.installedExtensions||[],o=new Set(s.map(a=>a.id)),r=i.filter(a=>!o.has(a.id)),c=(a,d)=>{for(let l=0;l<3;l++){const p=a[l]||0,u=d[l]||0;if(p>u)return!0;if(p<u)return!1}return!1};return i.length===0?html`
      <div class="scriptos-library">
        <div class="scriptos-header-sticky">
           <div class="scriptos-header">
            <h2>Extensions</h2>
      ...
      <div class="scriptos-content">
        ${s.length>0?html`
          <div class="scriptos-section" style="margin-bottom: 32px">
            ...
            <div class="scriptos-grid">
              ${s.map(a=>{const d=i.find(p=>p.id===a.id),l=d&&c(d.version,a.version);return html`
                  <div class="scriptos-card installed ${l?"has-update":""}">
                    ...
                    <div class="scriptos-card-actions" style="margin-top: auto; padding-top: 12px; border-top: 1px solid var(--border-color); display: flex; gap: 8px;">
                      ${l?html`
                        <button 
                          class="scriptos-update-btn"
                          onclick=${p=>{p.stopPropagation(),n("update-extension",{extension:a,newVersion:d})}}
...
`;this.term.write(h),this.state.logs?.autoScroll!==!1&&this.term.scrollToBottom()}cleanup(){this.writeHandler&&(window.removeEventListener("log-terminal-write",this.writeHandler),this.writeHandler=null),this.clearHandler&&(window.removeEventListener("log-terminal-clear",this.clearHandler),this.clearHandler=null),this.resizeObserver&&(this.resizeObserver.disconnect(),this.resizeObserver=null),this.term&&(this.term.dispose(),this.term=null),this.fitAddon=null}}let logTerminalComponent=null;function LogSidebar(e,n){if(!e.logs.isOpen)return logTerminalComponent=null,null;logTerminalComponent||(logTerminalComponent=new LogTerminalComponent("log-terminal",e,n));const i=logTerminalComponent.render(e),s=e.logSidebarWidth||350;return html`
    ...
    <div class="panel-section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <span class="info-value info-mono">${"a82da68"}</span>
        </div>
        <div class="info-item">
  ...
  `:""}function EthernetPanel(e,n){if(!e.networksInfo&&e.isConnected&&!e.isLoadingNetworks&&n("refresh-networks"),e.isConnected&&!e.ethConfigLoaded&&!e.isLoadingEthConfig&&n("load-eth-config"),e.isConnected&&!e.networkInterfacesConfig&&!e.isLoadingNetworkInterfacesConfig&&n("load-network-interfaces-config"),!e.networksInfo)return html`
    ...
    `;const i=e.networksInfo.eth,s=e.ethStatus,o=s&&s.initialized?s:i&&i.mac?i:s||i,r=e.ethConfig||{},c=i!==null,a=e.networkInterfacesConfig?.ethernet===!1,d=o&&(o.mac||o.enabled||o.enable||o.initialized),l=o&&o.ip&&o.ip!=="0.0.0.0",p=o&&o.linkup===!0,u=l;if(!c)return html`
      <div class="panel-container">
        <div class="panel-header">
          <h2>Ethernet Configuration</h2>
        </div>
    ...
    `;const f=a?"status-disabled":u?"status-active":p?"status-warning":"status-inactive",h=a?"Disabled":u?"Connected":p?"Link Up (No IP)":d?"No Link":"Not Initialized";return html`
    <div class="panel-container">
      <div class="panel-header">
        <h2>Ethernet Configuration</h2>
      </div>
  ...
  `:html`
    ...
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${d=>{d.preventDefault();const l=new FormData(d.target),p={apn:l.get("apn")||"",username:l.get("username")||"",password:l.get("password")||"",auto_init_modem:l.get("auto_init_modem")==="on",mobile_data_enabled:s.mobile_data_enabled||!1};n("save-wwan-config",p)}}>
          <div class="form-group">
            <label for="wwan-apn">
              GPRS APN
              <span class="label-tooltip">
  ...
  `}function NTPPanel(e,n){e.isConnected&&!e.ntpConfigLoaded&&!e.isLoadingNtpConfig&&n("load-ntp-config");const i=e.ntpConfig||{server:"pool.ntp.org",tzOffset:0,timezone:"UTC",autoDetect:!1},s=e.ntpSyncResult||null,o=l=>{if(!l)return"--:--:--";const{year:p,month:u,day:f,hour:h,minute:m,second:v}=l;return`${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}:${String(v).padStart(2,"0")}`},r=l=>{if(!l)return"";const{year:p,month:u,day:f}=l;return`${p}-${String(u).padStart(2,"0")}-${String(f).padStart(2,"0")}`},c=[{value:"UTC",offset:0,label:"UTC (Coordinated Universal Time)"},{value:"EST",offset:-5,label:"EST (Eastern Standard Time)"},{value:"CST",offset:-6,label:"CST (Central Standard Time)"},{value:"MST",offset:-7,label:"MST (Mountain Standard Time)"},{value:"PST",offset:-8,label:"PST (Pacific Standard Time)"},{value:"GMT",offset:0,label:"GMT (Greenwich Mean Time)"},{value:"CET",offset:1,label:"CET (Central European Time)"},{value:"EET",offset:2,label:"EET (Eastern European Time)"},{value:"JST",offset:9,label:"JST (Japan Standard Time)"},{value:"AEST",offset:10,label:"AEST (Australian Eastern Standard Time)"}],a=i.timezone,d=[...c];return a&&!c.find(l=>l.value===a)&&d.unshift({value:a,offset:i.tzOffset||0,label:`${a} (Detected)`}),html`
    <div class="panel-container">
      ...
      <div class="panel-section">
        ...
        <form class="config-form" onsubmit=${async l=>{l.preventDefault();const p=new FormData(l.target),u=p.get("server")||"pool.ntp.org",f=p.get("timezone")||"UTC",h=p.get("auto_detect")==="on",m=d.find(w=>w.value===f),v=m?m.offset:0,y={server:u,tz_offset:v,timezone:f,auto_detect:h};try{await new Promise((w,S)=>{const C=setTimeout(()=>S(new Error("Save timeout")),1e4),k=()=>{clearTimeout(C),window.appInstance.emitter.removeListener("ntp-config-saved",b),window.appInstance.emitter.removeListener("ntp-config-save-error",$)},b=()=>{k(),w()},$=x=>{k(),S(x)};window.appInstance.emitter.once("ntp-config-saved",b),window.appInstance.emitter.once("ntp-config-save-error",$),n("save-ntp-config",y)}),n("sync-ntp-time",u,v,h)}catch(w){console.error("[NTP] Failed to save config before sync:",w),alert(`Failed to save NTP configuration: ${w.message}`)}}}>
          <div class="form-group">
...
`)){const r=o.match(/^(\w+)\s*:\s*(.+)$/);r&&(n.meta[r[1].trim()]=r[2].trim())}n.body=e.slice(i[0].length)}return n.body=n.body.replace(/^ScriptO for\s+\[.*?\]\(.*?\)\.?\s*Run on device\s*\(MicroPython\)\.?\s*\n?/im,""),n.body=n.body.trim(),n}function SkillSidebar(e,n){if(!e.isSkillSidebarOpen||!e.skillSidebarContent)return html`<div></div>`;const{name:i,skillMd:s}=e.skillSidebarContent,{meta:o,body:r}=parseSkillMd(s);let c="";try{r&&(c=g.parse(r,{breaks:!0}))}catch(u){console.error("[SkillSidebar] Failed to parse markdown:",u),c=`<pre>${r}</pre>`}const a=[];o.description&&a.push(`<p class="skill-meta-description">${o.description}</p>`);const d=[];o.category&&d.push(`<span class="skill-meta-tag">${o.category}</span>`),o.author&&d.push(`<span class="skill-meta-tag">by ${o.author}</span>`),d.length&&a.push(`<div class="skill-meta-tags">${d.join("")}</div>`);const l=a.length?`<div class="skill-meta-block">${a.join("")}</div>`:"",p=document.createElement("div");return p.className="skill-sidebar-content skill-md-content",p.innerHTML=l+c,html`
...
`,a=await i.exec(c);i.onSdcardUnmount&&i.onSdcardUnmount(a),e.sdcardInfo={error:"SD card unmounted"},e.isUnmountingSDCard=!1,n.emit("render")}catch(o){console.error("[SD Card] Failed to unmount:",o),e.isUnmountingSDCard=!1,alert(`Failed to unmount SD card: ${o.message}`),n.emit("render")}}),n.on("sdcard-mount",async()=>{if(s("sdcard-mount"),!e.isConnected){console.warn("[SD Card] Not connected to device");return}if(e.isMountingSDCard){console.log("[SD Card] Already mounting, skipping");return}try{e.isMountingSDCard=!0,n.emit("render");const c=`
import json
import time
...
try:
    ...
    if not board.has("sdcard"):
        ...
        sd_hw_norm = {
            ...
            'slot': getattr(sd_bus, 'slot', 0),  # Read from manifest (P4: slot 1 for SD, slot 0 for C6 SDIO)
            'width': width,
...
`)}}catch(a){console.debug("[Connection] Failed to fetch version info:",a.message)}c.write(TERMINAL_PROMPT),c.scrollToBottom(),e.bannerDisplayed=!0,console.log("[Connection] Welcome banner displayed")})}function registerFileOperationHandlers(e,n,i,s,o,r,c){n.on("save",async()=>{if(console.log("save"),canSave({isConnected:e.isConnected,openFiles:e.openFiles,editingFile:e.editingFile})==!1){console.log("can't save");return}e.isSaving=!0,updateOverlayDirectly(e);let d=e.openFiles.find(m=>m.id===e.editingFile),l=!1;const p=d.parentFolder,u=p===null;u&&(d.source=="board"?d.parentFolder=e.boardNavigationPath:d.source=="disk"&&(d.parentFolder=e.diskNavigationPath));let f=!1;if(d.source=="board"?f=await fileExists(BridgeDevice,getFullPath(e.boardNavigationRoot,d.parentFolder,d.fileName)):d.source=="disk"&&(f=await s.fileExists(s.getFullPath(e.diskNavigationRoot,d.parentFolder,d.fileName))),(u||!f)&&(d.source=="board"?(d.parentFolder=e.boardNavigationPath,l=await fileExists(BridgeDevice,getFullPath(e.boardNavigationRoot,d.parentFolder,d.fileName))):d.source=="disk"&&(d.parentFolder=e.diskNavigationPath,l=await s.fileExists(s.getFullPath(e.diskNavigationRoot,d.parentFolder,d.fileName)))),l&&await confirmDialog(`You are about to overwrite the file ${d.fileName} on your ${d.source}.

 Are you sure you want to proceed?`)!==0){e.isSaving=!1,d.parentFolder=p,updateOverlayDirectly(e),n.emit("render");return}let h=d.editor.content||"";if(d.fileName&&d.fileName.toLowerCase().endsWith(".json"))try{const m=h.trim();if(m&&(m[0]==="{"||m[0]==="[")){const v=JSON.parse(m),y=JSON.stringify(v,null,2)+`
...
${a.message}`)}});async function r(c,a,d,l,p){const u=c.id,f=Array.isArray(c.version)?c.version.join("."):String(c.version);o(`[Extensions] Running onInstall for ${c.name}...`),showStyledModal({variant:"",icon:"packages",title:"Installing Extension",subtitle:c.name,body:`<p>Installing extension files to device...</p>
...
Please check your internet connection and try again.`)}}),n.on("select-scriptos",async r=>{if(o("select-scriptos:",r.displayName||r.slug),r.slug&&!r.content){const c=r.slug;console.log("[Registry] Fetching skill from ScriptoHub:",c);try{const a=await getSkill(c,e.scriptohubApiUrl);if(!a||!a.latestVersion)throw new Error(`Skill "${c}" has no published version`);const l=(await getSkillVersionFiles(c,a,e.scriptohubApiUrl)).files.find(h=>h.path.endsWith(".py"));if(!l)throw new Error(`Skill "${c}" has no .py file`);const[p,u]=await Promise.all([getSkillFile(c,l.path,e.scriptohubApiUrl),getSkillFile(c,"SKILL.md",e.scriptohubApiUrl).catch(()=>null)]),f=parseScriptOsConfig(p);if(!f)throw new Error("Failed to parse ScriptO configuration");console.log("[Registry] Fetched skill:",c,"py:",l.path,"skill.md:",!!u),r={slug:c,filename:l.path,displayName:r.displayName||a.skill?.displayName,content:p,config:f,skillMd:u}}catch(a){console.error("[Registry] Error fetching skill:",a),alert(`Failed to load skill:
