...
class OpenInverterExtension {
   ...
   */
  renderDevicePanel() {
    // Track that we're rendering device panel and when
    this.state._lastRenderedPanel = 'devicePanel'
    this.state._devicePanelRenderTime = Date.now()
    ...
    return this.html`
      <div style="display: flex; flex-direction: column; height: 100%;">
        <!-- Device Header -->
        <div style="background: var(--oi-beige); border-bottom: 1px solid var(--border-color); padding: 20px;">
...
`
  ...
  renderDeviceselector() {
    // This always shows the device selector - no special logic needed
    // Navigation to device panels is handled by the menu system
    ...
    return this.html`
      <div class="system-panel">
        <div class="panel-header" style="padding: 20px; border-bottom: 1px solid var(--border-color);">
          <h2 
            style="margin: 0; font-size: 24px; cursor: ${this.state.selectedDeviceSerial ? 'pointer' : 'default'};"
            onclick=${this.state.selectedDeviceSerial ? () => {
              this.state.selectedDeviceSerial = null
          ...
          >
            Device Manager
            ${this.state.selectedDeviceSerial ? this.html`<span style="font-size: 14px; font-weight: normal; margin-left: 12px; opacity: 0.8;">← Click to go back</span>` : ''}
          </h2>
      ...
      >
        <div 
          ...
          onclick=${(e) => {
            ...
            this.selectDeviceFromCard(device)
          }}
        >
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
  ...
  updateExtensionMenu() {
    ...
    // Don't emit render here - it causes issues with device selection
    // The sidebar will update on next natural render
  }
  ...
  selectDeviceFromCard(device) {
    ...
    // Find the device index to get the correct panel ID
    const deviceIndex = this.state.discoveredDevices.findIndex(d => d.serial === device.serial)
    if (deviceIndex === -1) return
    
    // Navigate to the device panel using the menu system (same as clicking menu item)
    const panelId = `device${deviceIndex}`
